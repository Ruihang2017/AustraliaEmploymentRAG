/**
 * FND-10 — the deterministic generator the property tests draw from.
 *
 * WHY HAND-ROLLED (plan open question OQ-1): the ticket's Test plan mentions "the property-testing
 * library declared in packages/domain/package.json", but `tools/tests/skeleton.test.mjs` and FND-08's
 * `test/workflow/purity.test.ts` both assert that `packages/domain/package.json` declares NO dependency
 * of any kind — the latter even greps the manifest text for `fast-check`. The acceptance checklist
 * requires ">= 10,000 cases", not a particular library, so the standing gate wins. FND-07 and FND-08
 * already set this convention in `packages/domain`; this file mirrors `test/answers/arbitrary.ts`.
 *
