...
async function attempt(
  ...
  const held = reservation as HeldReservation | null | undefined;
  if (held === null || held === undefined) return unavailable('NO_RESERVATION', 'RESERVATION_MISSING');
  if (typeof held.expiresAt !== 'number' || held.expiresAt <= deps.clock.now()) {
    return unavailable('NO_RESERVATION', 'RESERVATION_EXPIRED');
  }
  if (held.profileId !== profile.id) return unavailable('NO_RESERVATION', 'RESERVATION_WRONG_PROFILE');
...
export async function generate(
  ...
  return attempt(fallback.profile, call, reservation, deps);
}
