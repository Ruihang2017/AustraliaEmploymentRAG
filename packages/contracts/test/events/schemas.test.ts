...
import { UUID_V7_PATTERN, newId } from '../../src/ids/index.js';
import {
  type JsonObject,
  type JsonValue,
  loadAllSchemas,
...
describe('id patterns agree with the real minter (acceptance item 10)', () => {
  ...
  it('spells the uuid half with FND-03’s own regex, so the two cannot drift', () => {
    // UUID_V7_PATTERN is `^…$`; a schema pattern is `^<prefix>_…$`, so the shared middle is its
    // source minus the two anchors. If FND-03 ever tightens the uuid form, this fails here first.
    const uuidPart = UUID_V7_PATTERN.source.replace(/^\^/, '').replace(/\$$/, '');
    for (const [path, node] of cases.map(([p, n]) => [p, n] as const)) {
      expect(String(node['pattern']), `${path} does not embed UUID_V7_PATTERN`).toContain(uuidPart);
    }
  });
