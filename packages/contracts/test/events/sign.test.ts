...
/**
 * Static checks over `src/events/sign.ts` itself (acceptance items 5 and 12).
 *
...
describe('static properties of the implementation', () => {
  const SOURCE_PATH = 'src/events/sign.ts';
  const source = readFileSync(join(PACKAGE_ROOT, SOURCE_PATH), 'utf8').replace(/\r\n/g, '\n');
  ...
  it('compares in constant time and never with an equality operator', () => {
    expect(stripComments(source)).toContain('timingSafeEqual');
    expect(unsafeComparisons(source)).toEqual([]);
  });

  it('guards the length before timingSafeEqual, which throws on a mismatch', () => {
    const guard = /function equalsInConstantTime[\s\S]*?\n}/.exec(stripComments(source));
    expect(guard).not.toBeNull();
    const bodyText = guard?.[0] ?? '';
    expect(bodyText.indexOf('a.length')).toBeLessThan(bodyText.indexOf('timingSafeEqual('));
  });
  ...
  it('imports nothing but node:crypto and sources no secret (acceptance item 12)', () => {
    const code = stripComments(source);
    const specifiers = [...code.matchAll(/\bfrom\s*['"]([^'"]+)['"]/g)].map((match) => match[1]);
    expect(specifiers).toEqual(['node:crypto']);
    for (const forbidden of ['process.env', 'node:fs', 'node:http', 'fetch(', 'require(', 'import(']) {
      expect(code, `${SOURCE_PATH} references ${forbidden}`).not.toContain(forbidden);
    }
  });
