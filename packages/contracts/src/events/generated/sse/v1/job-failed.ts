...
// source: schemas/events/sse/v1/job.failed.json

/** Emitted once on terminal failure. The error code is fetched with the job over REST (PRD §34.9); the stream carries no message text. PRD §34.4 evidences a frame for `stage.changed` and `job.completed` only. Every other allowed type therefore carries exactly the three fields all §34.4 frames share — inventing more would exceed the PRD, and any content-bearing field is forbidden outright (FND-05 deliverable 2; PRD §16.2 "MUST NOT contain hidden reasoning or raw provider payloads"; §22). */
export interface JobFailedSseEvent {
