---
name: builder-codex
description: ARCHIVED — the Codex-delegating Builder, preserved verbatim and NOT wired into the pipeline (the workflows resolve `builder`). Thin Claude wrapper that frames the ticket, loads real repo context, and delegates implementation to OpenAI Codex headless (`codex exec -m gpt-5.6-sol`); then runs the tests itself and iterates until green. Restore by renaming this file and its `name:` back to `builder`.
model: sonnet
