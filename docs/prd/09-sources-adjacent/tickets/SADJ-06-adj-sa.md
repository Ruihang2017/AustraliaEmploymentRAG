...
already made in PRD §40.5; this is build ticket 6 of 9 against it.
...
`SLEG-07` — `LEG-SA`, module `06-sources-legislation` (`docs/prd/06-sources-legislation/tickets/`).
...
**PRD §6.3 fixes the full state/territory checklist** — binding on South Australia in addition to the
§40.5 row:
...
**Two of this group's statutory authorities publish outside `.gov.au`** — the return-to-work
authority and the construction-industry portable long-service-leave board. Sub-PRD **D8** governs:
they are admissible only as the named statutory authority for their scheme, recorded in
`registry.yaml.authority` with the statutory basis, and explicitly allowlisted. PRD §6.1 admits only
official public sources; the exception is expressed as data in `allowlist.yaml`, never as a code path.

**Why `blocked_by SLEG-07`.** The Acts behind these regimes — the *Work Health and Safety Act 2012*
(SA), *Equal Opportunity Act 1984* (SA), the return-to-work legislation, *Labour Hire Licensing Act
2017* (SA), *Long Service Leave Act 1987* (SA), the construction industry portable LSL legislation,
*Public Interest Disclosure Act 2018* (SA), *Public Sector Act 2009* and the child-employment
provisions of the child-protection legislation — belong to `LEG-SA` (PRD §40.2, wave 1). This adapter
never re-ingests them (sub-PRD **D4**); it emits `node_relation` records targeting their `LEG-SA` node
...
"regime does not exist" reason code — a gap-level code only; it never sets `adapter_status` and never
produces a `limitation` block, sub-PRD **D12**), **M7** (per-collection fixture namespacing).
...
No mandatory source group is pre-selected for omission or reduced implementation; `ADJ-SA` must be
...
Deliver the `ADJ-SA` source adapter at `pipelines/adapters/adj-sa/**`: a `registry.yaml` that
decomposes the group into the eight named source groups of Deliverable 1 with an exact official
endpoint and PRD §40.5 material classification per collection, an `allowlist.yaml` covering exactly
those hosts (including the two non-`.gov.au` statutory authorities), a `licence.yaml` plus captured
snapshots, and an `adapter.py` exposing `ADAPTER: SourceAdapter` implementing all eight PRD §40.7
boundaries across those collections — such that
`python -m <root>.conformance check pipelines/adapters/adj-sa` exits 0 with all twelve PRD §40.8
items `PASS` per collection (item 11 `DEFERRED(GOLD-16)` only while `evals/cases/**` is absent), the
composed Source Coverage Registry reports `ADJ-SA` as `ACTIVE` or an explicit PRD §7 limited status
...
- **No South Australian legislation.** Acts, regulations, rules, proclamations and notices belong to
  `SLEG-07` (`LEG-SA`, module `06-sources-legislation`) — including Commissioner's Determinations if
  they are published as instruments in the register.
- **No court or tribunal decisions.** SA courts, the SA Employment Tribunal and the Employment Court
  belong to `SCAS-10` (`CASE-SA`, module `08-sources-cases`). Only the **regulator's own** published
  decisions, register entries, enforcement outcomes and decision summaries are in scope here
  (PRD §6.4 last bullet, §9.1 level 6).
- **No bills, drafts or consultations.** `SFUT-07` (`FUTURE-SA`, module `10-sources-future`).
- **No payroll tax.** `SINS-11` (`PT-SA`, module `07-sources-instruments`).
...
- `pipelines/adapters/adj-sa/**` — `registry.yaml`, `allowlist.yaml`, `licence.yaml`,
  `licence-snapshots/**`, `conformance.yaml`, `adapter.py`, internal modules of the Builder's choice,
  `fixtures/**`, `tests/**`.
- Does not touch: `pipelines/adapters/adj-{cth,nsw,vic,qld,wa,tas,act,nt}/**` — sibling tickets
  `SADJ-01`…`SADJ-05`, `SADJ-07`…`SADJ-09`.
...
`SLEG-07` have landed before this ticket starts.

## Deliverables

1. **Registry decomposition — the eight named source groups of `ADJ-SA`.** Each becomes one
   ...
   |---|---|---|---|---|---|---|
   | 1 | `whs-safework-sa` | SafeWork SA (`REGULATOR`) | `safework.sa.gov.au` | WHS/OHS | `CODE` (approved codes of practice), `GUIDANCE`, `DECISION` (prosecutions, enforceable undertakings), `NEWS` | T1 |
   | 2 | `ir-lsl-child-employment-safework-sa` | SafeWork SA — industrial relations function (`REGULATOR`) | `safework.sa.gov.au` industrial-relations pages | ordinary long service leave; state industrial relations; child employment | `GUIDANCE`, `POLICY` | T1 |
   | 3 | `discrimination-eoc-sa` | Equal Opportunity Commission SA (`COMMISSION`) | `equalopportunity.sa.gov.au` | discrimination and equal opportunity | `GUIDANCE`, `POLICY`, `DECISION` (exemptions and published commission decisions) | T1 |
   | 4 | `workers-comp-rtwsa` | ReturnToWorkSA (`REGULATOR`) | ReturnToWorkSA official site (**non-`.gov.au`** — sub-PRD **D8**) | workers compensation / return to work | `OPERATIVE_INSTRUMENT` (return-to-work guidelines and publications made under the Act), `GUIDANCE`, `POLICY` | T1 |
   | 5 | `labour-hire-cbs` | Consumer and Business Services (`REGULATOR`) | `cbs.sa.gov.au` | labour hire licensing | `DECISION` (public licence register entries and licensing decisions), `GUIDANCE` | T2 |
   | 6 | `portable-lsl-construction-sa` | South Australian construction industry portable long service leave authority (`REGULATOR`) | the authority's official site (**verify; may be non-`.gov.au`** — sub-PRD **D8**) | portable long service leave | `GUIDANCE`, `POLICY`, `DECISION` (registration/scheme decisions) | T2 |
   | 7 | `public-sector-ocpse` | Office of the Commissioner for Public Sector Employment (`COMMISSION`) | `publicsector.sa.gov.au` | public-sector employment | `OPERATIVE_INSTRUMENT` (Commissioner's Determinations and Guidelines), `CODE` (code of ethics), `GUIDANCE` | T2 |
   | 8 | `pid-privacy-sa` | Office for Public Integrity and the SA Ombudsman (`REGULATOR`) | `pid.sa.gov.au`, `ombudsman.sa.gov.au` | whistleblowing / public interest disclosure; employment-related privacy in the public sector | `GUIDANCE`, `CODE` (PID standards and model procedures) | T2 |

   `authority.jurisdiction` is `SA` for all eight. Lead authority in `registry.yaml.authority` is
   **SafeWork SA**; every other authority is named on its endpoint entries — the interim rule of
...
2. **`pipelines/adapters/adj-sa/registry.yaml`** — one file validating against `INGF-07`'s
   `registry.schema.json`: `group_id: ADJ-SA`, `wave: 4`, lead `authority`, one `official_endpoints`
   ...
   `python -m <root>.registry validate pipelines/adapters/adj-sa` exits 0.
...
3. **`allowlist.yaml`** — `INGF-02`'s schema: `schemes: [https]` only, one `hosts` entry per domain in
   Deliverable 1 with the narrowest `path_prefixes` covering the declared collections. The two
   non-`.gov.au` statutory-authority hosts are allowlisted **explicitly**, each with a `notes` line
   naming the statutory basis (sub-PRD **D8**). Every `official_endpoints` URL must pass this file.
...
4. **`licence.yaml` + `licence-snapshots/`** — `INGF-04`'s schema: a captured snapshot per authority
   ...
   (PRD §11.1). The two non-`.gov.au` sites are the likeliest to carry restrictive terms — assess each
   independently, never by analogy with the state's default open licence.

5. **`adapter.py` — `ADAPTER: SourceAdapter`** with `meta = AdapterMeta(group_id="ADJ-SA",
   adapter_key="adj-sa", jurisdiction="SA", …)` implementing all eight PRD §40.7 boundaries. All I/O
...
7. **`identify()` — stable identity and deletion behaviour.** `stable_source_key` scheme
   `adj-sa:<collection_key>:<authority-assigned-id-or-normalised-path>`, documented with one worked
   example per collection. Deterministic across runs, stable across versions, collision-free across
   collections — explicitly between collections 1 and 2, which share a host. A disappeared descriptor
   yields `REMOVED` and retains prior versions; a de-listed labour-hire licence is a **status change
   with history**, never a deletion (PRD §40.8 item 3).

8. **`parse()` + `normalise()` — node hierarchy and exact-text round-trip.** Approved codes of
   practice and Commissioner's Determinations normalise to a chapter/section/clause `NodeVersion` tree
   (one root, contiguous sibling ordinals); short guidance and register entries normalise to a shallow
   structure. `text[start_offset:end_offset]` reproduces every block and every `text_hash` recomputes
   from `canonical_text` (PRD §15.3, `INGF-06`'s `assert_roundtrip()`).

9. **`document_type` — subordinate-authority typing (sub-PRD D3).** Closed vocabulary distinguishing
   at least `APPROVED_CODE_OF_PRACTICE`, `COMMISSIONER_DETERMINATION`, `SCHEME_GUIDELINE`,
   `REGULATOR_GUIDANCE`, `REGULATOR_POLICY`, `REGULATOR_DECISION`, `PUBLIC_REGISTER_ENTRY` and
   `REGULATOR_NEWS`, each mapped to its PRD §40.5 `material_class`. This makes PRD §9.1's "Guidance
   MUST NOT silently override legislation" enforceable downstream and `SRCH-002`'s type/authority
   filters meaningful.
...
10. **`extractEvents()` — evidenced status only (sub-PRD D7).** Issue, revision, replacement and
    ...
    default. Return-to-work guidelines and Commissioner's Determinations are dated and superseded
    explicitly — that history must be captured, not flattened to "current". Versions of one
    consolidated series must not overlap (PRD §35.2, §40.9).

11. **`extractRelations()` — links into `LEG-SA`, never re-ingestion (sub-PRD D4).** A code of
    practice approved under a named section, a determination citing the *Public Sector Act*, or a
    return-to-work guideline citing the return-to-work legislation emits a `node_relation` targeting
    the `LEG-SA` node identity with `derivation` recording deterministic extraction, exact evidence
...
12. **PRD §6.3 topic coverage map**, materialised both in `registry.yaml` and as a committed table in
    ...
    |---|---|---|
    | payroll tax legislation, rates, guidance | `PT-SA` (`SINS-11`, module `07`) | out of this group's scope |
    | employment and industrial-relations legislation | `LEG-SA` (`SLEG-07`, module `06`) | guidance side covered by collection 2 |
    | long service leave | collection 2 | ordinary LSL |
    | WHS/OHS | collection 1 | |
    | discrimination and equal opportunity | collection 3 | |
    | workers compensation | collection 4 | non-`.gov.au` statutory authority — sub-PRD **D8** |
    | labour hire licensing | collection 5 | |
    | portable long service leave | collection 6 | construction industry scheme |
    | workplace surveillance and employment-related privacy | collection 8, **partially** | South Australia's information-privacy regime is administrative rather than statutory for much of the public sector, and there is no general private-sector employment-privacy regulator collection; record a `known_gaps` entry with `customer_visible: true` and point to `ADJ-CTH`'s OAIC collection for federally-regulated employers. Surveillance-devices law itself is `LEG-SA`. |
    | whistleblowing | collection 8 | |
    | child employment | collection 2, **verify** | South Australia has no general child-employment permit scheme comparable to Victoria's; if no regulator collection exists, record a `known_gaps` entry with `customer_visible: true` rather than leaving the topic unmentioned |
    | public-sector employment | collection 7 | |
    | relevant regulators, courts and tribunals | this group (regulators) + `CASE-SA` (`SCAS-10`, module `08`) | boundary stated in *Non-goals* |
...
15. **Adapter-local unit tests** beyond the inherited kit: per-collection discovery parsing (including
    register pagination and shared-host separation), identity stability/collision properties, the
    `document_type` mapping table, the `LEG-SA` relation extractor including its drop-on-ambiguity
    path, and the gap/status derivation of Deliverable 12.

16. **Failure codes** registered with `register_failure_codes("adj-sa", …)`, each with a non-empty
...
- [ ] `[machine]` **DoD 1 — registry row, allowlist, licence.** All three files validate; `group_id`
      is `ADJ-SA` and in `MANDATORY_SOURCE_GROUPS`; directory name equals `group_id.lower()`; each of
      the eight collections has its own `official_endpoints` row with a PRD §40.5 `material_class`;
      every endpoint URL passes `allowlist.yaml`; both non-`.gov.au` hosts carry a recorded statutory
      basis; the licence snapshot's SHA-256 equals `snapshot.terms_sha256` (PRD §40.8 item 1, §6.1,
      §11.1; sub-PRD D8).
...
- [ ] `[fixture]` **DoD 3 — stable identity, versions, deletion/unavailability.** Deterministic across
      two calls, stable across two versions, collision-free across collections (explicitly 1 vs 2); a
      removed descriptor yields `REMOVED` and deletes no prior state, including a de-listed licence.
...
- [ ] `[fixture]` **DoD 6 — three time points.** Three declared legal dates per collection resolve to
      ...
      `event_date` and `effective_date` distinguished; a superseded guideline or determination keeps
      its history; no overlapping consolidated intervals (PRD §6.6, §15.2, §35.2).
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/adj-sa` exits **0** in strict
      mode with `summary.fail == 0` and `summary.not_available == 0`; a `--lenient` report is not
      acceptable evidence (PRD §45.4).
- [ ] `[machine]` **Source Coverage Registry row.** `INGF-07`'s composer includes `ADJ-SA` with all
...
- [ ] `[machine]` **PRD §6.3 topic coverage.** A test asserts Deliverable 12's map: every §6.3 topic
      ...
      named other group — with the **employment-privacy** and **child-employment** rows required to
      resolve one way or the other, never to be absent (sub-PRD D5, PRD §44.4).
...
- [ ] `[machine]` **Relations, not duplicates.** Every `node_relation` targets a `LEG-SA` identity
      with retained evidence offsets and `parser_version`, `confidence_state` is never
      `MODEL_SUGGESTED`, and no emitted `DocumentVersion` duplicates a `LEG-SA` stable identity
      (PRD §9.3, §35.2, §40.9).
- [ ] `[machine]` **Boundary tests.** `INGF-01`'s architecture scan passes for
      `pipelines/adapters/adj-sa/**`: no HTTP library, no `sqlite3`, no corpus-database module, no
...
- [ ] `[machine]` The adapter suite runs fully **offline** — a session fixture asserts no outbound
      network during `uv run pytest pipelines/adapters/adj-sa`.
...
- [ ] `[human]` **Founder/Architect review of the registry decomposition** — whether the eight named
      collections and their material classifications discharge PRD §40.5 for South Australia, and
      whether the two non-`.gov.au` statutory authorities are correctly admitted under PRD §6.1, are
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**ADM-001**, **SRCH-002**); UAT
      ...
      **source/licence/provenance impact** (eight authorities, two of them publishing outside
      `.gov.au`); cost/memory/latency impact (DoD item 12 measurements against PRD §39.2's 2 GiB
      budget); rollback path (delete the group directory — `INGF-07` then fails loudly with
      `MANDATORY_GROUP_MISSING`); known gaps (employment privacy; child employment if unresolved;
...
Harness: `uv run pytest pipelines/adapters/adj-sa -q` plus the conformance CLI. Every step is
...
1. `uv sync --frozen` then `uv run pytest pipelines/adapters/adj-sa -q` — all green.
2. `python -m <root>.registry validate pipelines/adapters/adj-sa` — exit 0; then delete one required
   attribute from a scratch copy and confirm a non-zero exit.
3. `python -m <root>.conformance check pipelines/adapters/adj-sa --report /tmp/adj-sa.json` — exit 0,
...
5. **Identity** (`tests/test_identity.py`): determinism, cross-version stability, no cross-collection
   collision (explicitly 1 vs 2), `REMOVED` with prior versions retained including a de-listed
   licence.
...
7. **Temporal** (`tests/test_events.py`): three time points per collection; a superseded return-to-work
   guideline keeps its history; `event_date` vs `effective_date`; `STATUS_UNCONFIRMED` where no status
   is published; an overlapping-interval fixture must fail.
8. **Relations** (`tests/test_relations.py`): a code-of-practice fixture citing a named WHS Act 2012
   (SA) section emits a `node_relation` into `LEG-SA` with evidence offsets and `parser_version`; an
   ambiguous citation emits nothing; `confidence_state` is never `MODEL_SUGGESTED`.
9. **Topics, status and gaps** (`tests/test_coverage.py`): Deliverable 12's map, with the
   employment-privacy and child-employment rows explicitly resolved; `capability: NONE` forces
   `FRESHNESS_LIMITED`; `UNCLEAR_RESTRICTED` forces the metadata/link-only permission set; both
   non-`.gov.au` hosts are allowlisted with a statutory-basis note.
10. `uv run pytest` (whole repo) and `pnpm test` — green.

**Reviewer focus.** (a) Each of the eight collections must have a real, specific official endpoint and
a defensible `material_class`; collections 1 and 2 sharing a host must still be two rows. (b) The two
non-`.gov.au` sources must be justified in `registry.yaml` and assessed independently in
`licence.yaml` — this is the ticket's main licensing risk. (c) The conformance report must name every
collection under every item. (d) No Act text may be emitted — check `LEG-SA` stable keys appear only
as relation targets. (e) SAET and Employment Court decisions must **not** appear here — they belong to
`CASE-SA`. (f) Re-run step 3 after deleting one licence snapshot file — DoD item 1 must fail.
...
1. **A non-`.gov.au` statutory authority's terms forbid storage or indexing** (sub-PRD **D8**, **M2**).
   → Set that collection's assessment accordingly (`METADATA_AND_LINK_ONLY` or `UNCLEAR_RESTRICTED`),
   let `INGF-04` collapse the permission set, record a `known_gaps` entry with
   `customer_visible: true`, and add a writeback line to `docs/prd/09-sources-adjacent/README.md`.
   Never widen a permission locally and never treat a non-`.gov.au` site as open by analogy with the
   state's `.gov.au` sites.
2. **`registry.yaml` cannot express eight authorities in one group** (sub-PRD **M1**). → Update
...
4. **An expected official entry has moved, or the portable-LSL authority's official site differs from
   the one assumed here.** → Record the resolved entry in `registry.yaml`/`allowlist.yaml` **and**
   update Deliverable 1's table in this ticket in the same PR. The table is the cold-start contract for
   reviewers and for `GOLD-16`.
5. **The child-employment or employment-privacy topic has no SA regulator collection.** → Record a
   `known_gaps` entry with `customer_visible: true` and the interim reason code of sub-PRD **M3**, and
   update Deliverable 12 in this ticket. Leaving the row unresolved in either direction is the PRD
   §44.4 failure.
...
classification per named authority — cannot be met for South Australia, that is not a local
