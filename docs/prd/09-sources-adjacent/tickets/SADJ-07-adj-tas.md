...
already made in PRD §40.5; this is build ticket 7 of 9 against it.
...
`SLEG-08` — `LEG-TAS`, module `06-sources-legislation` (`docs/prd/06-sources-legislation/tickets/`).
...
Note that the §40.5 row names two of the four authorities **generically** ("workers-compensation and
portable-LSL authorities"). Resolving those names to concrete authorities and concrete collections is
precisely this ticket's job; a registry that repeats the generic phrase has not discharged §40.5.

**PRD §6.3 fixes the full state/territory checklist** — binding on Tasmania in addition to the §40.5
row:
...
MUST produce customer-visible warnings when relevant." Tasmania is a small jurisdiction with
correspondingly small regulator sites; `FRESHNESS_LIMITED` is a realistic outcome for at least one
collection and must be an honest status, not a workaround.
...
**Why `blocked_by SLEG-08`.** The Acts behind these regimes — the *Work Health and Safety Act 2012*
(Tas), *Anti-Discrimination Act 1998* (Tas), the workers rehabilitation and compensation legislation,
*Long Service Leave Act 1976* (Tas), the construction industry portable LSL legislation, the youth
employment provisions of the education legislation, *Personal Information Protection Act 2004* (Tas),
*Public Interest Disclosures Act 2002* (Tas) and *State Service Act 2000* — belong to `LEG-TAS`
(PRD §40.2, wave 1). This adapter never re-ingests them (sub-PRD **D4**); it emits `node_relation`
records targeting their `LEG-TAS` node identities, which must exist first. PRD §9.3: "Deterministic
...
No mandatory source group is pre-selected for omission or reduced implementation; `ADJ-TAS` must be
...
Deliver the `ADJ-TAS` source adapter at `pipelines/adapters/adj-tas/**`: a `registry.yaml` that
decomposes the group into the seven named source groups of Deliverable 1 — resolving §40.5's generic
"workers-compensation and portable-LSL authorities" to concrete authorities and collections — with an
exact official endpoint and PRD §40.5 material classification per collection, an `allowlist.yaml`
covering exactly those hosts (including the non-`.gov.au` portable-LSL authority), a `licence.yaml`
plus captured snapshots, and an `adapter.py` exposing `ADAPTER: SourceAdapter` implementing all eight
PRD §40.7 boundaries across those collections — such that
`python -m <root>.conformance check pipelines/adapters/adj-tas` exits 0 with all twelve PRD §40.8
items `PASS` per collection (item 11 `DEFERRED(GOLD-16)` only while `evals/cases/**` is absent), the
composed Source Coverage Registry reports `ADJ-TAS` as `ACTIVE` or an explicit PRD §7 limited status
...
- **No Tasmanian legislation.** Acts, statutory rules and point-in-time versions belong to `SLEG-08`
  (`LEG-TAS`, module `06-sources-legislation`) — including Employment Directions if they are
  published as instruments in the register.
- **No court or tribunal decisions.** Tasmanian courts, TASCAT and the industrial collections belong
  to `SCAS-11` (`CASE-TAS`, module `08-sources-cases`), which also owns "operative instruments" from
  those bodies. Only the **regulator's own** published decisions, register entries, enforcement
  outcomes and decision summaries are in scope here (PRD §6.4 last bullet, §9.1 level 6).
- **No bills, drafts or consultations.** `SFUT-08` (`FUTURE-TAS`, module `10-sources-future`).
- **No payroll tax.** `SINS-12` (`PT-TAS`, module `07-sources-instruments`).
...
- `pipelines/adapters/adj-tas/**` — `registry.yaml`, `allowlist.yaml`, `licence.yaml`,
  `licence-snapshots/**`, `conformance.yaml`, `adapter.py`, internal modules of the Builder's choice,
  `fixtures/**`, `tests/**`.
- Does not touch: `pipelines/adapters/adj-{cth,nsw,vic,qld,wa,sa,act,nt}/**` — sibling tickets
  `SADJ-01`…`SADJ-06`, `SADJ-08`, `SADJ-09`.
...
`SLEG-08` have landed before this ticket starts.

## Deliverables

1. **Registry decomposition — the seven named source groups of `ADJ-TAS`.** Each becomes one
   ...
   |---|---|---|---|---|---|---|
   | 1 | `whs-worksafe-tas` | WorkSafe Tasmania (`REGULATOR`) | `worksafe.tas.gov.au` | WHS/OHS | `CODE` (approved codes of practice), `GUIDANCE`, `DECISION` (prosecutions, enforceable undertakings), `NEWS` | T1 |
   | 2 | `workers-comp-workcover-tas` | WorkCover Tasmania Board, administered through WorkSafe Tasmania (`REGULATOR`) | WorkCover/workers-compensation pages under `worksafe.tas.gov.au` | workers compensation | `OPERATIVE_INSTRUMENT` where made under the Act, `GUIDANCE`, `POLICY` | T1 |
   | 3 | `discrimination-eot` | Equal Opportunity Tasmania (`COMMISSION`) | `equalopportunity.tas.gov.au` | discrimination and equal opportunity | `GUIDANCE`, `DECISION` (exemption decisions), `POLICY` | T1 |
   | 4 | `portable-lsl-construction-tas` | The Tasmanian construction industry portable long service leave authority (`REGULATOR`) | the authority's official site (**non-`.gov.au`** — sub-PRD **D8**) | portable long service leave | `GUIDANCE`, `POLICY`, `DECISION` (registration/scheme decisions) | T2 |
   | 5 | `ir-lsl-youth-employment-tas` | WorkSafe Tasmania and the Department of Justice (`DEPARTMENT`) | industrial-relations and employment-standards pages under `worksafe.tas.gov.au` / `justice.tas.gov.au` | ordinary long service leave; state industrial relations; youth/child employment | `GUIDANCE`, `POLICY` | T1 |
   | 6 | `public-sector-ssmo` | State Service Management Office, Department of Premier and Cabinet (`DEPARTMENT`) | `dpac.tas.gov.au` | public-sector employment | `OPERATIVE_INSTRUMENT` (Employment Directions issued under the *State Service Act*), `GUIDANCE`, `POLICY` | T2 |
   | 7 | `pid-privacy-ombudsman-tas` | Ombudsman Tasmania (`REGULATOR`) | `ombudsman.tas.gov.au` | whistleblowing / public interest disclosures; employment-related personal-information protection | `GUIDANCE`, `CODE` (PID guidelines and model procedures) | T2 |

   `authority.jurisdiction` is `TAS` for all seven. Lead authority in `registry.yaml.authority` is
   **WorkSafe Tasmania**; every other authority is named on its endpoint entries — the interim rule of
   sub-PRD **M1**. Collections 1, 2 and 5 share a host but are distinct collections with distinct
   discovery, baselines and tiers; they must not be collapsed into one endpoint row.

2. **`pipelines/adapters/adj-tas/registry.yaml`** — one file validating against `INGF-07`'s
   `registry.schema.json`: `group_id: ADJ-TAS`, `wave: 4`, lead `authority`, one `official_endpoints`
   ...
   `python -m <root>.registry validate pipelines/adapters/adj-tas` exits 0.
...
3. **`allowlist.yaml`** — `INGF-02`'s schema: `schemes: [https]` only, one `hosts` entry per domain in
   ...
   statutory basis (sub-PRD **D8**). Every `official_endpoints` URL must pass this file.
...
4. **`licence.yaml` + `licence-snapshots/`** — `INGF-04`'s schema: a captured snapshot per authority
   ...
   `UNCLEAR_RESTRICTED`, not an implied permission (PRD §11.1: "Unclear rights default to metadata,
   limited quotation and official links").

5. **`adapter.py` — `ADAPTER: SourceAdapter`** with `meta = AdapterMeta(group_id="ADJ-TAS",
   adapter_key="adj-tas", jurisdiction="TAS", …)` implementing all eight PRD §40.7 boundaries. All
...
6. **`discover()` — per-collection strategies.** One strategy per `collection_key` declaring its
   ...
   `collection_key` in `hints`. Collections published under the shared `worksafe.tas.gov.au` host must
   be discovered from their own listing pages, not from a site-wide crawl. A collection with no delta
...
7. **`identify()` — stable identity and deletion behaviour.** `stable_source_key` scheme
   `adj-tas:<collection_key>:<authority-assigned-id-or-normalised-path>`, documented with one worked
   example per collection. Deterministic across runs, stable across versions, collision-free across
   collections — the three shared-host collections are the risk case. A disappeared descriptor yields
...
8. **`parse()` + `normalise()` — node hierarchy and exact-text round-trip.** Approved codes of
   practice and Employment Directions normalise to a chapter/section/clause `NodeVersion` tree (one
...
9. **`document_type` — subordinate-authority typing (sub-PRD D3).** Closed vocabulary distinguishing
   at least `APPROVED_CODE_OF_PRACTICE`, `EMPLOYMENT_DIRECTION`, `REGULATOR_GUIDANCE`,
...
10. **`extractEvents()` — evidenced status only (sub-PRD D7).** Issue, revision, replacement and
    ...
    default — a rule that bites hardest on small regulator sites that publish undated pages.
    Employment Directions are numbered, dated and superseded explicitly: that history must be
    captured, not flattened to "current". Versions of one consolidated series must not overlap
    (PRD §35.2, §40.9).

11. **`extractRelations()` — links into `LEG-TAS`, never re-ingestion (sub-PRD D4).** A code of
    practice approved under a named section, an Employment Direction citing the *State Service Act*,
    or guidance citing a numbered provision emits a `node_relation` targeting the `LEG-TAS` node
    identity with `derivation` recording deterministic extraction, exact evidence offsets and
    `parser_version`, and a `confidence_state` that is never `MODEL_SUGGESTED` (PRD §9.3, §35.2).
...
12. **PRD §6.3 topic coverage map**, materialised both in `registry.yaml` and as a committed table in
    ...
    |---|---|---|
    | payroll tax legislation, rates, guidance | `PT-TAS` (`SINS-12`, module `07`) | out of this group's scope |
    | employment and industrial-relations legislation | `LEG-TAS` (`SLEG-08`, module `06`) | guidance side covered by collection 5 |
    | long service leave | collection 5 | ordinary LSL under the *Long Service Leave Act 1976* (Tas) |
    | WHS/OHS | collection 1 | |
    | discrimination and equal opportunity | collection 3 | |
    | workers compensation | collection 2 | resolves §40.5's generic "workers-compensation authorities" |
    | labour hire licensing | **gap** | Tasmania operates no labour-hire licensing scheme as at the recorded date. `known_gaps` entry, `customer_visible: true`, naming the jurisdictions that do (`ADJ-VIC`, `ADJ-QLD`, `ADJ-SA`, `ADJ-ACT`) — sub-PRD **D5**, interim reason code per **M3** |
    | portable long service leave | collection 4 | resolves §40.5's generic "portable-LSL authorities" |
    | workplace surveillance and employment-related privacy | collection 7, **partially** | Tasmania's personal-information protection regime covers the public sector; there is no general private-sector employment-privacy regulator collection. Record a `known_gaps` entry with `customer_visible: true` and point to `ADJ-CTH`'s OAIC collection for federally-regulated employers. Surveillance-devices law itself is `LEG-TAS`. |
    | whistleblowing | collection 7 | |
    | child employment | collection 5 | youth-employment provisions administered through the employment-standards and education portfolios; if no regulator collection exists, record a `known_gaps` entry rather than silence |
    | public-sector employment | collection 6 | |
    | relevant regulators, courts and tribunals | this group (regulators) + `CASE-TAS` (`SCAS-11`, module `08`) | boundary stated in *Non-goals* |
...
14. **`conformance.yaml` + `tests/test_conformance.py`** — the five-line `ConformanceTestCase`
    ...
    `anomaly_overrides` (PRD §40.9, sub-PRD **M6**). Small collections make percentage-based count
    anomalies noisy: tighten thresholds deliberately and record the reasoning, never loosen them.
    `deferred_items` may contain only `11`, with a reason.
...
15. **Adapter-local unit tests** beyond the inherited kit: per-collection discovery parsing (including
    ...
    table, the `LEG-TAS` relation extractor including its drop-on-ambiguity path, and the gap/status
    derivation of Deliverable 12 — with an explicit test that the labour-hire gap is **present** and
    customer-visible.

16. **Failure codes** registered with `register_failure_codes("adj-tas", …)`, each with a non-empty
...
- [ ] `[machine]` **DoD 1 — registry row, allowlist, licence.** All three files validate; `group_id`
      is `ADJ-TAS` and in `MANDATORY_SOURCE_GROUPS`; directory name equals `group_id.lower()`; each of
      the seven collections has its own `official_endpoints` row with a PRD §40.5 `material_class`;
      every endpoint URL passes `allowlist.yaml`; the non-`.gov.au` host carries a recorded statutory
...
- [ ] `[fixture]` **DoD 3 — stable identity, versions, deletion/unavailability.** Deterministic across
      two calls, stable across two versions, collision-free across collections including the three
...
- [ ] `[fixture]` **DoD 6 — three time points.** Three declared legal dates per collection resolve to
      ...
      `event_date` and `effective_date` distinguished; a superseded Employment Direction keeps its
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/adj-tas` exits **0** in
      strict mode with `summary.fail == 0` and `summary.not_available == 0`; a `--lenient` report is
      not acceptable evidence (PRD §45.4).
- [ ] `[machine]` **Source Coverage Registry row.** `INGF-07`'s composer includes `ADJ-TAS` with all
...
- [ ] `[machine]` **§40.5's generic authority names are resolved.** A test asserts that no
      ...
      "workers-compensation authorities" or "portable-LSL authorities": each is resolved to a named
      authority with a concrete endpoint (PRD §40.5).
...
- [ ] `[machine]` **Relations, not duplicates.** Every `node_relation` targets a `LEG-TAS` identity
      with retained evidence offsets and `parser_version`, `confidence_state` is never
      `MODEL_SUGGESTED`, and no emitted `DocumentVersion` duplicates a `LEG-TAS` stable identity
      (PRD §9.3, §35.2, §40.9).
- [ ] `[machine]` **Boundary tests.** `INGF-01`'s architecture scan passes for
      `pipelines/adapters/adj-tas/**`: no HTTP library, no `sqlite3`, no corpus-database module, no
...
- [ ] `[machine]` The adapter suite runs fully **offline** — a session fixture asserts no outbound
      network during `uv run pytest pipelines/adapters/adj-tas`.
- [ ] `[machine]` `uv run pytest` green (PRD §45.3, §20.3).
- [ ] `[machine]` `pnpm test` green — standing item; no TypeScript here, so "unchanged and green".
- [ ] `[human]` **Founder/Architect review of the registry decomposition** — whether the seven named
      collections discharge PRD §40.5 for Tasmania, whether the generic authority names have been
      resolved correctly, and whether any `FRESHNESS_LIMITED` status reflects a genuine source
      limitation rather than unfinished work, are irreducibly judgment calls; PRD §43.4 item 4 puts
      source-adapter anomalies in the founder review queue.
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**ADM-001**, **SRCH-002**); UAT
      ...
      known gaps (labour hire licensing; employment privacy; sub-PRD M1, M2, M3, M6, M7; plan §8
      **Q9** is baseline-selected — the ±10% / >2% figures are initial defaults this group may tighten
      from its own baseline — and the plan §8 **Q10**
...
Harness: `uv run pytest pipelines/adapters/adj-tas -q` plus the conformance CLI. Every step is
...
1. `uv sync --frozen` then `uv run pytest pipelines/adapters/adj-tas -q` — all green.
2. `python -m <root>.registry validate pipelines/adapters/adj-tas` — exit 0; then delete one required
   attribute from a scratch copy and confirm a non-zero exit.
3. `python -m <root>.conformance check pipelines/adapters/adj-tas --report /tmp/adj-tas.json` —
...
6. **Identity** (`tests/test_identity.py`): determinism, cross-version stability, no cross-collection
   collision across the three `worksafe.tas.gov.au` collections, `REMOVED` with prior versions
   retained.
...
8. **Temporal** (`tests/test_events.py`): three time points per collection; a superseded Employment
   Direction keeps its history; an undated guidance page resolves to `STATUS_UNCONFIRMED`;
...
9. **Relations and gaps** (`tests/test_relations.py`, `tests/test_coverage.py`): a code-of-practice
   fixture citing a named WHS Act 2012 (Tas) section emits a `node_relation` into `LEG-TAS` with
   evidence offsets and `parser_version`; an ambiguous citation emits nothing; Deliverable 12's map
   holds, including the labour-hire gap; `capability: NONE` forces `FRESHNESS_LIMITED`;
...
labour-hire gap must be present and customer-visible. (c) Undated regulator pages must resolve to
`STATUS_UNCONFIRMED`, never to `IN_FORCE`. (d) The non-`.gov.au` portable-LSL source must be justified
in `registry.yaml` and assessed independently in `licence.yaml`. (e) TASCAT decisions and operative
instruments must **not** appear here — they belong to `CASE-TAS`. (f) Re-run step 3 after deleting one
licence snapshot file — DoD item 1 must fail.
...
1. **§40.5's generic authority names resolve differently than Deliverable 1 assumes** — for example
   the workers-compensation function sits with a separate board site, or the portable-LSL authority
   has been restructured. → Record the resolved authority and endpoint in
   `registry.yaml`/`allowlist.yaml` **and** update Deliverable 1's table in this ticket in the same
   PR. The table is the cold-start contract for reviewers and for `GOLD-16`.
2. **A regulator lacks the assumed change-detection capability, or publishes no terms at all.** →
   ...
   PRD §12.1, §44.4.
3. **`registry.yaml` cannot express seven authorities in one group** (sub-PRD **M1**). → Update
...
4. **A regime this ticket records as absent turns out to exist, or one recorded as present has been
   ...
   in force belongs to `FUTURE-TAS` (`SFUT-08`), never mislabelled as current law (PRD §6.5).
...
classification per named authority — cannot be met for Tasmania, that is not a local implementation
