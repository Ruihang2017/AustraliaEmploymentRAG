...
already made in PRD §40.5; this is build ticket 2 of 9 against it.
...
`SLEG-03` — `LEG-NSW`, module `06-sources-legislation` (`docs/prd/06-sources-legislation/tickets/`).
...
**PRD §6.3 fixes the full state/territory checklist** — binding on NSW in addition to the §40.5 row:
...
Deliverable 12 maps every one of those topics to a named collection **or** to an explicit gap, or to
the module that owns it. PRD §44.4: "It is not permitted to silently call an unimplemented source
category covered."
...
summaries and impact materials" — and: "Guidance MUST NOT silently override legislation, an
operative instrument or binding authority." PRD §6.1: "Official regulator summaries MAY supplement
but MUST NOT replace primary decisions or operative instruments."

**Why `blocked_by SLEG-03`.** The Acts behind these regimes — *Work Health and Safety Act 2011*
(NSW), *Anti-Discrimination Act 1977*, the workers-compensation Acts, *Long Service Leave Act 1955*,
the building and construction industry LSL payments Act, *Workplace Surveillance Act 2005*,
*Privacy and Personal Information Protection Act 1998*, *Public Interest Disclosures Act 2022*,
*Government Sector Employment Act 2013* — belong to `LEG-NSW` (PRD §40.2, wave 1). This adapter never
re-ingests them (sub-PRD **D4**); it emits `node_relation` records targeting their `LEG-NSW` node
...
No mandatory source group is pre-selected for omission or reduced implementation; `ADJ-NSW` must be
...
Deliver the `ADJ-NSW` source adapter at `pipelines/adapters/adj-nsw/**`: a `registry.yaml` that
decomposes the group into the ten named source groups of Deliverable 1 with an exact official
...
that `python -m <root>.conformance check pipelines/adapters/adj-nsw` exits 0 with all twelve PRD
§40.8 items `PASS` per collection (item 11 `DEFERRED(GOLD-16)` only while `evals/cases/**` is
absent), the composed Source Coverage Registry reports `ADJ-NSW` as `ACTIVE` or an explicit PRD §7
...
- **No NSW legislation.** Acts, regulations and statutory instruments belong to `SLEG-03`
  (`LEG-NSW`, module `06-sources-legislation`) — including instruments that a regulator republishes.
- **No court or tribunal decisions.** NSW Caselaw, the Industrial Relations Commission, NCAT and the
  Personal Injury Commission belong to `SCAS-06` (`CASE-NSW`, module `08-sources-cases`). Only the
  **regulator's own** published decisions, determinations, register entries, enforcement outcomes and
  decision summaries are in scope here (PRD §6.4 last bullet, §9.1 level 6).
- **No bills, drafts or consultations.** `SFUT-03` (`FUTURE-NSW`, module `10-sources-future`).
- **No payroll tax.** `SINS-07` (`PT-NSW`, module `07-sources-instruments`).
...
- `pipelines/adapters/adj-nsw/**` — `registry.yaml`, `allowlist.yaml`, `licence.yaml`,
  `licence-snapshots/**`, `conformance.yaml`, `adapter.py`, internal modules of the Builder's choice,
  `fixtures/**`, `tests/**`.
- Does not touch: `pipelines/adapters/adj-{cth,vic,qld,wa,sa,tas,act,nt}/**` — sibling tickets
  `SADJ-01`, `SADJ-03`…`SADJ-09`.
...
min waves 1, nine useful lanes) — all nine may run as concurrent lanes. `pipelines/adapters/pyproject.toml`
stays untouched (sub-PRD **D10**). Upstream `INGF-09` and `SLEG-03` have landed before this ticket
starts.

## Deliverables

1. **Registry decomposition — the ten named source groups of `ADJ-NSW`.** Each becomes one
   ...
   |---|---|---|---|---|---|---|
   | 1 | `whs-safework-nsw` | SafeWork NSW (`REGULATOR`) | `safework.nsw.gov.au` | WHS/OHS | `CODE` (codes of practice approved under the WHS Act), `GUIDANCE`, `DECISION` (prosecutions, enforceable undertakings), `NEWS` (safety alerts) | T1 |
   | 2 | `discrimination-adnsw` | Anti-Discrimination NSW (`COMMISSION`) | `antidiscrimination.nsw.gov.au` | discrimination and equal opportunity | `GUIDANCE`, `DECISION` (exemption decisions), `POLICY` | T1 |
   | 3 | `workers-comp-sira` | State Insurance Regulatory Authority (`REGULATOR`) | `sira.nsw.gov.au` | workers compensation | `OPERATIVE_INSTRUMENT` (workers compensation guidelines made under the Act), `GUIDANCE`, `POLICY` | T1 |
   | 4 | `workers-comp-icare` | icare NSW (`REGULATOR`) | `icare.nsw.gov.au` | workers compensation (scheme/insurer material) | `GUIDANCE`, `POLICY` | T2 |
   | 5 | `portable-lsl-lsc` | Long Service Corporation (`REGULATOR`) | `longservice.nsw.gov.au` | portable long service leave | `GUIDANCE`, `POLICY`, `DECISION` (scheme/registration decisions) | T2 |
   | 6 | `ir-lsl-surveillance-nswir` | NSW Industrial Relations (`DEPARTMENT`) | `nsw.gov.au/industrial-relations` | ordinary long service leave; state industrial relations; workplace surveillance guidance | `GUIDANCE`, `POLICY` | T1 |
   | 7 | `privacy-ipc-nsw` | Information and Privacy Commission NSW (`REGULATOR`) | `ipc.nsw.gov.au` | employment-related privacy | `GUIDANCE`, `CODE` (statutory guidelines), `DECISION` (published review reports) | T2 |
   | 8 | `child-employment-ocg` | Office of the Children's Guardian (`REGULATOR`) | `ocg.nsw.gov.au` | child employment (employer authorities, working-with-children requirements) | `GUIDANCE`, `CODE` (child safe standards), `DECISION` | T2 |
   | 9 | `public-sector-psc-nsw` | NSW Public Service Commission (`COMMISSION`) | `psc.nsw.gov.au` | public-sector employment | `OPERATIVE_INSTRUMENT` (directions made under the GSE Act), `GUIDANCE`, `POLICY` | T2 |
   | 10 | `whistleblowing-ombo-nsw` | NSW Ombudsman (`REGULATOR`) | `ombo.nsw.gov.au` | whistleblowing / public interest disclosures | `GUIDANCE`, `CODE` (PID standards and model policies) | T2 |

   `authority.jurisdiction` is `NSW` for all ten. Lead authority in `registry.yaml.authority` is
   **SafeWork NSW**; every other authority is named on its endpoint entries — the interim rule of
   sub-PRD **M1**.

2. **`pipelines/adapters/adj-nsw/registry.yaml`** — one file validating against `INGF-07`'s
   `registry.schema.json`: `group_id: ADJ-NSW`, `wave: 4`, lead `authority`, one `official_endpoints`
   ...
   `python -m <root>.registry validate pipelines/adapters/adj-nsw` exits 0.
...
4. **`licence.yaml` + `licence-snapshots/`** — `INGF-04`'s schema: a captured snapshot per authority
   (`terms_url`, `terms_sha256`, stored artifact) and one `assessment` covering all nine PRD §11.1
   decision axes plus `attribution_text`, `max_quote_chars` and `status`. Divergent terms resolve to
   the most restrictive for the group (sub-PRD **M2**); unclear terms are `UNCLEAR_RESTRICTED`,
   collapsing to metadata/link-only (PRD §11.1).

5. **`adapter.py` — `ADAPTER: SourceAdapter`** with `meta = AdapterMeta(group_id="ADJ-NSW",
   adapter_key="adj-nsw", jurisdiction="NSW", …)` implementing all eight PRD §40.7 boundaries. All
...
6. **`discover()` — per-collection strategies.** One strategy per `collection_key` declaring its
   ...
   `collection_key` in `hints`. A collection whose authority publishes no delta mechanism declares
   `capability: NONE` and drives `FRESHNESS_LIMITED` (sub-PRD **D6**, PRD §12.1) rather than
   inheriting a daily cadence it cannot support.

7. **`identify()` — stable identity and deletion behaviour.** `stable_source_key` scheme
   `adj-nsw:<collection_key>:<authority-assigned-id-or-normalised-path>`, documented with one worked
   example per collection. Deterministic across runs, stable across versions, collision-free across
   collections; a disappeared descriptor yields `REMOVED` and retains prior versions (PRD §40.8
   item 3).

8. **`parse()` + `normalise()` — node hierarchy and exact-text round-trip.** Codes of practice and
   long guidance normalise to a chapter/section/clause `NodeVersion` tree (one root, contiguous
   sibling ordinals); short guidance normalises to a shallow heading tree.
...
9. **`document_type` — subordinate-authority typing (sub-PRD D3).** Closed vocabulary distinguishing
   at least `APPROVED_CODE_OF_PRACTICE`, `REGULATOR_GUIDELINE`, `REGULATOR_GUIDANCE`,
...
11. **`extractRelations()` — links into `LEG-NSW`, never re-ingestion (sub-PRD D4).** A code of
    practice approved under a named section, or guidance citing a numbered provision, emits a
    `node_relation` targeting the `LEG-NSW` node identity with `derivation` recording deterministic
    extraction, exact evidence offsets and `parser_version`, and a `confidence_state` that is never
    `MODEL_SUGGESTED` (PRD §9.3, §35.2). Unresolvable citations are dropped, not guessed.

12. **PRD §6.3 topic coverage map**, materialised both in `registry.yaml` (`known_gaps` +
    `official_endpoints`) and as a committed table in the adapter's module docstring:

    | PRD §6.3 topic | Covered by | Note |
    |---|---|---|
    | payroll tax legislation, rates, guidance | `PT-NSW` (`SINS-07`, module `07`) | out of this group's scope |
    | employment and industrial-relations legislation | `LEG-NSW` (`SLEG-03`, module `06`) | guidance side covered by collection 6 |
    | long service leave | collections 5, 6 | portable and ordinary |
    | WHS/OHS | collection 1 | |
    | discrimination and equal opportunity | collection 2 | |
    | workers compensation | collections 3, 4 | |
    | labour hire licensing | **gap** | NSW operates no general labour-hire licensing scheme as at the recorded date: `known_gaps` entry, `customer_visible: true`, naming the jurisdictions that do (`ADJ-VIC`, `ADJ-QLD`, `ADJ-SA`, `ADJ-ACT`) — sub-PRD **D5**, interim reason code per **M3** |
    | portable long service leave | collection 5 | |
    | workplace surveillance and employment-related privacy | collections 6, 7 | |
    | whistleblowing | collection 10 | |
    | child employment | collection 8 | |
    | public-sector employment | collection 9 | |
    | relevant regulators, courts and tribunals | this group (regulators) + `CASE-NSW` (`SCAS-06`, module `08`) | boundary stated in *Non-goals* |
...
15. **Adapter-local unit tests** beyond the inherited kit: per-collection discovery parsing, identity
    stability/collision properties, the `document_type` mapping table, the `LEG-NSW` relation
    extractor including its drop-on-ambiguity path, and the gap/status derivation of Deliverable 12.

16. **Failure codes** registered with `register_failure_codes("adj-nsw", …)`, each with a non-empty
    operator action (PRD §40.8 item 10, ADM-001): at minimum unparseable collection listing, missing
    collection listing, unexpected content type, identity conflict, unresolvable citation target and
    licence-terms change detected.

## Acceptance checklist (classified)

**Per named source group.** Items 1–12 are PRD §40.8's twelve-item Definition of Done and each must
hold for **every one of the ten named source groups** in Deliverable 1. A report in which an item
...
- [ ] `[machine]` **DoD 1 — registry row, allowlist, licence.** All three files validate; `group_id`
      is `ADJ-NSW` and in `MANDATORY_SOURCE_GROUPS`; directory name equals `group_id.lower()`; each of
      the ten collections has its own `official_endpoints` row with a PRD §40.5 `material_class`;
...
- [ ] `[fixture]` **DoD 3 — stable identity, versions, deletion/unavailability.** Deterministic across
      ...
      yields `REMOVED` and deletes no prior state.
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/adj-nsw` exits **0** in
      strict mode with `summary.fail == 0` and `summary.not_available == 0`; a `--lenient` report is
      not acceptable evidence (PRD §45.4).
- [ ] `[machine]` **Source Coverage Registry row.** `INGF-07`'s composer includes `ADJ-NSW` with all
...
- [ ] `[machine]` **PRD §6.3 topic coverage.** A test asserts Deliverable 12's map: every §6.3 topic
      ...
      named other group — and that **labour hire licensing** is present as an explicit
      customer-visible gap rather than absent (sub-PRD D5, PRD §44.4).
...
- [ ] `[machine]` **Relations, not duplicates.** Every `node_relation` targets a `LEG-NSW` identity
      with retained evidence offsets and `parser_version`, `confidence_state` is never
      `MODEL_SUGGESTED`, and no emitted `DocumentVersion` duplicates a `LEG-NSW` stable identity
      (PRD §9.3, §35.2, §40.9).
- [ ] `[machine]` **Boundary tests.** `INGF-01`'s architecture scan passes for
      `pipelines/adapters/adj-nsw/**`: no HTTP library, no `sqlite3`, no corpus-database module, no
...
- [ ] `[machine]` The adapter suite runs fully **offline** — a session fixture asserts no outbound
      network during `uv run pytest pipelines/adapters/adj-nsw`.
- [ ] `[machine]` `uv run pytest` green (PRD §45.3, §20.3).
- [ ] `[machine]` `pnpm test` green — standing item; no TypeScript here, so "unchanged and green".
- [ ] `[human]` **Founder/Architect review of the registry decomposition** — whether the ten named
      collections and their material classifications discharge PRD §40.5 for NSW is irreducibly a
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**ADM-001**, **SRCH-002**); UAT
      ...
      impact (none — public source material; fixtures scanned for customer data); **source/licence/
      provenance impact** (ten authorities and their licence assessments); cost/memory/latency impact
      (DoD item 12 measurements against PRD §39.2's 2 GiB budget); rollback path (delete the group
      directory — `INGF-07` then fails loudly with `MANDATORY_GROUP_MISSING`); known gaps (labour hire
      licensing; sub-PRD M1, M2, M6, M7; plan §8 **Q9** is baseline-selected — the ±10% / >2% figures are
...
Harness: `uv run pytest pipelines/adapters/adj-nsw -q` plus the conformance CLI. Every step is
...
1. `uv sync --frozen` then `uv run pytest pipelines/adapters/adj-nsw -q` — all green.
2. `python -m <root>.registry validate pipelines/adapters/adj-nsw` — exit 0; then delete one required
   attribute from a scratch copy and confirm a non-zero exit.
3. `python -m <root>.conformance check pipelines/adapters/adj-nsw --report /tmp/adj-nsw.json` —
   exit 0, `"strict": true`, `summary.fail == 0`, `summary.not_available == 0`, and every item's
   `evidence` naming each of the ten `collection_key`s. Item 11 is `PASS` or `DEFERRED` with a
   reason; nothing else is deferred.
4. **Per-collection matrix** (`tests/test_collections.py`): parametrised over the ten
   ...
   this is what makes "per named source group" mechanical.
5. **Identity** (`tests/test_identity.py`): determinism, cross-version stability, no cross-collection
   collision, `REMOVED` with prior versions retained.
...
8. **Relations** (`tests/test_relations.py`): a code-of-practice fixture citing a named WHS Act
   section emits a `node_relation` into `LEG-NSW` with evidence offsets and `parser_version`; an
   ambiguous citation emits nothing; `confidence_state` is never `MODEL_SUGGESTED`.
9. **Topics, status and gaps** (`tests/test_coverage.py`): Deliverable 12's map, including the
   labour-hire-licensing gap; `capability: NONE` forces `FRESHNESS_LIMITED`; `UNCLEAR_RESTRICTED`
   forces the metadata/link-only permission set.
10. `uv run pytest` (whole repo) and `pnpm test` — green.

**Reviewer focus.** (a) Each of the ten collections must have a real, specific official endpoint and a
defensible `material_class` — a generic authority home page is the failure PRD §40.5 warns about.
(b) The conformance report must name every collection under every item; an item passing on one
collection and silently skipping another is the PRD §44.4 failure. (c) No Act text may be emitted by
this adapter — check that `LEG-NSW` stable keys appear only as relation targets, never as emitted
identities. (d) The labour-hire-licensing gap must be present and customer-visible, not absent.
(e) `FRESHNESS_LIMITED`/`LICENSING_RESTRICTED` must reflect a genuine source limitation, never an
unfinished collection. (f) Re-run step 3 after deleting one licence snapshot file — DoD item 1 must
fail.
...
1. **`registry.yaml` cannot express ten authorities in one group** (sub-PRD **M1**). → Update
   ...
   `docs/prd/09-sources-adjacent/README.md` **M1**, then implement. `INGF-07`'s feedback obligation
   item 1 names exactly this case and requires the multiplicity to stay **inside** the group's file.
   Never create a cross-group registry file — that falsifies plan §2.1 **A2**.
2. **Authorities in this group have materially different licence terms** (sub-PRD **M2**). → Interim
   ...
   collection, raise a per-collection override *inside* `licence.yaml` against `INGF-04` (its ticket
   plus `docs/prd/05-ingestion-framework/README.md`) and record the outcome in
...
3. **A regulator lacks the assumed change-detection capability, or its terms are unclear/restrictive.**
   ...
   `docs/prd/09-sources-adjacent/README.md` naming the collection and the limitation. Never a silent
   downgrade, never a quiet cadence change, never grounds for dropping the collection — PRD §44.4.
...
5. **A PRD §6.3 topic has no NSW regulator collection** (labour hire licensing is the known case). →
   Record a `known_gaps` entry with `customer_visible: true` naming the jurisdictions that do have the
   regime (sub-PRD **D5**), using the interim reason code of **M3**. If `GOLD-16` or `INTL-02` cannot
   distinguish "no regime" from "regulator offline", the writeback is a `REGIME_NOT_APPLICABLE` value
   raised against `INGF-07`.
6. **A shared helper looks necessary across two or more `SADJ` tickets.** → Forbidden here: module 09
   ...
   `06-sources-legislation` (which owns `pipelines/adapters/_shared/legislation/**` via `SLEG-01`),
   add the `blocked_by` edge in `docs/prd/breakdown-plan.md` §5.10 and §6.2, then implement. Never
   copy a helper into two adapter directories and never create `_shared/adjacent/**`.
...
8. **A hard-to-reverse choice arises that this ticket does not settle** (for example a general
   convention for multi-authority groups across all nine wave-4 tickets). → ADR candidate under plan
...
classification per named authority — cannot be met for NSW, that is not a local implementation
