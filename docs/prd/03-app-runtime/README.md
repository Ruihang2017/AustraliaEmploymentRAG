...
| QR12 | `tools/tests/skeleton.test.mjs` asserted `Object.keys(tsconfig).sort() === ['extends', 'include']` for every pnpm member, forbidding any `compilerOptions`. With `tsconfig.base.json` setting `lib: ["ES2024"]` and no `jsx`, that made **every `.tsx` file in the repository** fail `pnpm typecheck` (TS17004) and every DOM type unreachable — while PRD §18.2 fixes React + Vite for web/admin/widget. `RUNT-05`/`RUNT-06` ship the minimal relaxation inline (a member may add `compilerOptions`; still no `files`, no `references`, no second `extends`) and flag it. Confirm the relaxation, or replace it with a `00-foundation` repair? | `00-foundation` (`tools/**` is its allocated row) | a small repair ticket in the `FND-11`/`FND-14` family | `packages/ui/tsconfig.json` (`jsx` + DOM lib) and `apps/web/tsconfig.json`; every later React member | tools/tests/skeleton.test.mjs; PRD §18.2, §20.1, §20.3 |
| QR13 | `packages/contracts`'s generated `Citation` carries `role` (`SUPPORTS`/`QUALIFIES`/…) but **no authority reference**, while PRD §32.3 requires the evidence panel to show the citation's "authority role". `RUNT-06` models it as caller-supplied panel data (`CitationView.authority`, whose `authority_level` is the contracts `AuthorityLevel`) rather than declaring a vocabulary. Should `Citation` gain an authority reference in the OpenAPI contract instead? | `00-foundation` (`FND-04`, OpenAPI + generated bindings) with the **Founder** for the API change (PRD §45.5) | a PRD/OpenAPI amendment, then `FND-04` | `packages/ui/src/evidence/types.ts` becomes a re-export; no component logic changes | PRD §32.3, §34.5, §45.5 |
| QR14 | `JurisdictionCode` and `SearchResult.freshness` are deliberately **not** `ENUM_REGISTRY` families (sub-PRD Q-F10) and are typed `string`. `RUNT-06`'s `JurisdictionBadge`/`FreshnessBadge` therefore render the open string they are given (text plus a non-colour shape) and declare no local vocabulary. Is that sufficient for the PRD §41.1 "jurisdiction, legal status and source freshness use text plus badge/icon" row? | `00-foundation` (`FND-03`) with the Founder | confirmation on Q-F10, or a new enum family | the two badges swap `string` for the generated type and gain an exhaustive label map; no other change | PRD §41.1, §35.1; sub-PRD Q-F10 |
