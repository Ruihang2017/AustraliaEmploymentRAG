...
| D21 | **`packages/domain` and `packages/contracts` property tests use an in-repo seeded generator, not a property-testing library; and `FND-09`'s admission denial reasons reach PRD §34.9 through a declared mapping, not a 1:1 name match.** (a) `tools/tests/skeleton.test.mjs` asserts every workspace member manifest declares no `dependencies`/`devDependencies`, and it runs on **every** branch; the domain/contracts ticket file-scopes also forbid touching root manifests and lockfiles, while CI installs with `--frozen-lockfile`. A ticket whose Test plan calls for "the property-testing library declared in `<package>/package.json`" therefore satisfies it with a deterministic seeded generator inside its own `test/**` leaf (reproducible per seed, seed printed on failure, interleaved sequences, boundary-value inputs). The acceptance bars name counts, not libraries, and are unchanged. If a real dependency ever becomes necessary, that is an `FND-01` (`tools/**`) change plus a lockfile change and must be raised there first. (b) `FND-09` deliverable 6's five reasons are **not** all §34.9 codes: `PRICE_DATA_UNAVAILABLE` and `CONCURRENCY_LIMIT` are absent from the seventeen codes `FND-03` transcribed. They are finer-grained *causes* of codes that do exist and are mapped by the exported total map `ADMISSION_REASON_TO_ERROR_CODE` — `PRICE_DATA_UNAVAILABLE → GENERATION_UNAVAILABLE` (PRD §36.8 final row, *"Provider/budget unavailable → Job unavailable; Search and saved records remain available"*) and `CONCURRENCY_LIMIT → RATE_LIMITED` (PRD §38.5 puts concurrency in the same limits table; §34.9 has no concurrency code). **No new §34.9 code is introduced**: that would be a public-API change under PRD §16.1/§45.5 touching `FND-03`, `FND-04` and the PRD, and is escalated rather than decided in a build ticket. | `FND-09` Feedback obligation 3 (the mapping route it authorises) and the General rule; `tools/tests/skeleton.test.mjs` (`FND-01`); PRD §34.9, §36.8, §38.5, §16.1, §45.5 |
...
| v0.7 | 2026-08-08 | `FND-09` implementation writeback (ticket Feedback obligation, General rule + obligation 3) — done **before** code, as the rule requires. Two ticket defects found while planning, both recorded as new decision **D21**. (a) The Test plan's Harness line required *"the property-testing library declared in `packages/domain/package.json`"*, which this repository forbids: `tools/tests/skeleton.test.mjs` asserts every workspace member manifest declares no `dependencies`/`devDependencies` and runs on every branch, the ticket File-scope forbids root manifests and lockfiles, and CI installs `--frozen-lockfile`. The Harness line now names a deterministic seeded generator inside `packages/domain/test/budget/**`; the acceptance bar (*"≥10,000 generated reserve/settle sequences"*, interleaved, cancellations) is unchanged and is met. (b) Deliverable 6 claimed the five admission denial reasons *"map 1:1 onto PRD §34.9 codes"*; checked against the merged `FND-03` registry, `PRICE_DATA_UNAVAILABLE` and `CONCURRENCY_LIMIT` are **not** among the seventeen §34.9 codes. Deliverable 6 and the PR-contract acceptance item now describe the exported total map `ADMISSION_REASON_TO_ERROR_CODE` (five reasons onto three codes; the two extras are finer-grained causes of `GENERATION_UNAVAILABLE` per PRD §36.8 and `RATE_LIMITED` per PRD §38.5). **No new §34.9 code was introduced** — that would be a PRD §16.1/§45.5 public-API change touching `FND-03`, `FND-04` and the PRD, and is left to the Architect/Founder. |
