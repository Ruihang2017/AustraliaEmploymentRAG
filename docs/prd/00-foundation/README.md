...
| D21 | **Generation scripts fan out from a per-artifact name; the aggregate name is a regex fan-out, identical in every generating package.** A ticket that generates registers `generate:<artifact>` and `generated:check:<artifact>` in its package manifest (`FND-05` registers `generate:events` / `generated:check:events`; `FND-04` registers `generate:openapi` / `generated:check:openapi`). The aggregate entries, when they can be registered at all (see D22), are byte-identically `"generate": "pnpm run \"/^generate:/\""` and `"generated:check": "pnpm run \"/^generated:check:/\""` in every package — two identical additions to a shared append-only manifest merge trivially, whereas two different `"generate"` commands are a real conflict in which one ticket's generator silently disappears. (`^generate:` does not match `generated:check:events`.) | D16 (append-only shared manifests); D9; PRD §20.1; the same reasoning that produced D19 for the CI gate script names |
| D22 | **`generate` and `generated:check` cannot be registered by any workspace member until `tools/tests/scripts.test.mjs` is repaired — recorded as a gap, not waived.** That merged test asserts `expect(membersProviding(name)).toEqual([])` for **every** key of `tools/fixtures/script-owners.json#owners`, which includes `generate` and `generated:check`. The moment a member manifest declares either name, the repo-wide suite fails — and the fix lives in `tools/**` (`FND-01`'s file-scope) and the root README's "Known gaps" list, neither of which `FND-04` or `FND-05` may write. This is the same class of defect as the one `FND-11` repaired: a repository-wide invariant that encoded one ticket's transient state. Until it is repaired, a generating ticket registers **only** its own `generate:<artifact>` / `generated:check:<artifact>` entries (D21) and makes the generated-diff guarantee real by asserting the emitter-vs-committed-files comparison inside its own package test suite, which `pnpm test` and the `ts-type-unit` CI job run on every PR. **Proposed repair (Architect to ticket):** split the per-owner test in `tools/tests/scripts.test.mjs` — assert the owner-line behaviour only for names with no provider, assert the recursive-delegation path for the rest, and keep it non-vacuous by requiring at least one owner name to still have no provider. The fixture entries stay: they are documented *fallbacks*, and `workspace-script.mjs` already prefers a real provider. | `tools/tests/scripts.test.mjs` (merged, `FND-01`); `tools/workspace-script.mjs` step 3; `FND-05` deliverable 4 and acceptance item 11; PRD §20.1 |
...
| Q-F2 | Are SSE event payload schemas (PRD §34.4) part of `schemas/events/**`? `FND-05`'s file-scope has no carve-out and no other module may write the tree (D8). **Answered in the affirmative by `FND-05` (v0.7): nine SSE schemas ship under `schemas/events/sse/v1/**`. Residual sub-question, still open and owned by the Architect with the Builder of `RUNT-03`:** PRD §34.4 calls `answer.section` *"provisional UI content"* but shows no frame for it, while `FND-05` deliverable 2 forbids any property named `answer`, `content`, `text`, `excerpt`, … in any schema under `schemas/events/**`. Inventing a content field would exceed the PRD's evidence and breach the denylist; inventing an evasive name (`section_markdown`) would breach its intent. `answer.section` therefore ships with the same three common fields as every other unevidenced type (`schema_version`, `job_id`, `occurred_at`). `RUNT-03` may **not** resolve this by defining a payload schema under `apps/api/src/sse/**` — that is exactly what D8 forbids; it comes back here through `FND-05` Feedback obligation 1. **Decide before `RUNT-03` starts.** | Builder of `FND-05`, confirmed by the Builder of `RUNT-03` when it consumes them | `FND-05`; re-checked at `RUNT-03` | Nothing | This README (D8) and `docs/prd/breakdown-plan.md` §5.1 if `RUNT-03` needs a different split |
...
| v0.7 | 2026-08-08 | `FND-05` implementation writeback (ticket Feedback obligation, General rule) — done **before** schemas or the helper, as the rule requires. **(a) Harness.** The ticket's Test-plan harness sentence says *"JSON Schema validator and code generator declared in `packages/contracts/package.json`"*; `tools/tests/skeleton.test.mjs` (merged, `FND-01`) asserts every member manifest declares **no** dependency of any kind, so neither can be a declared dependency. Both are repo-local instead: a purpose-built emitter under `packages/contracts/src/events/codegen/**` and a subset JSON Schema 2020-12 validator under `packages/contracts/test/events/support/**` whose supported-keyword allowlist is asserted **closed over the whole schema corpus**, so the validity reading discharged is *structural validity against the declared 2020-12 vocabulary subset, with the subset proven complete for these schemas* — not "a validator that happens to pass". **(b) `answer.section`** and every other PRD-unevidenced SSE type carries only `schema_version`, `job_id`, `occurred_at` — recorded against **Q-F2/D8** above. **(c)** New decisions **D21** (per-artifact `generate:<artifact>` names plus the identical regex fan-out for the aggregate) and **D22** (the aggregate names cannot be registered by any member until `tools/tests/scripts.test.mjs` is repaired — an **escalation**, with the repair specified; the generated-diff guarantee runs under `pnpm test` meanwhile). **(d) Deliverable 6** ships **two** fixtures per artifact: the PRD-verbatim text (which contains placeholders — `evt_...`, `job_...`, `<lowercase hex HMAC-SHA256>` — and therefore cannot itself validate or be signed) plus a concrete instance derived from it by a committed substitution map, with a test proving the derivation byte-for-byte. The §34.4 fixtures are transcribed from `docs/PRD.md` lines 1945–1951, which carry a second (`job.completed`) frame the ticket's Background quote omits but its deliverable 6 requires. **(e)** The fixed test secret is a **lower-case** JSON key with a lower-case value: `FND-02`'s CI secret scan applies the seven name-shaped patterns to every git-tracked file outside `docs/**`, so `TEST_SECRET`/`WEBHOOK_SIGNING_KEY`-shaped identifiers would fail the scan job (D20b). **(f) Deliverable 3** spells the parameter `secret` and then requires an *ordered list of secrets*; `verifyWebhook` accepts `string \| readonly string[]` so both readings hold without a second function, and the timestamp window is **symmetric** (a far-future timestamp is rejected too — PRD §34.8 only says "older than five minutes"; the symmetric reading closes a replay-with-future-timestamp hole and is strictly safer). |
