...
| D21 | **The access decision input shape is settled here, once, by `FND-06`, and one §38.1 condition is renamed.** `evaluate()` takes `{ principal: { kind, id, organizationId, role?, grants, scopes }, action, resource?: { organizationId, id?, ownerId?, sharedWith?, assignedReviewerId? } \| null, context: { intent, ownerCount?, target?, usageView?, auditView? } }`. A **`Grant`** is `{ permission, resourceId? }` — an explicit recorded row, never inferred from a role (PRD §8.1 "Developer MUST NOT automatically gain Research Record content access"); `resourceId` absent means organisation-wide. `resource: null` means *the request named an opaque id and nothing was found*, distinct from an omitted `resource` (*no resource in play*), so the domain — not the caller — produces the not-found decision and `isIndistinguishableNotFound` collapses `RESOURCE_ABSENT` with `CROSS_ORGANIZATION` (PRD §16.5). Four §38.1 cells are undecidable without the added fields: "read shared"/"read-only export" need `intent`, the two membership rows need `target`, "own usage"/"API/service usage subset" need `usageView`, "✓ limited"/"credential events only" need `auditView`. `DATA-04` (membership storage) and `RUNT-02` (the admission chain) read this shape and must not re-derive it. **Rename:** the §38.1 service-account condition is `SCOPED_CREDENTIAL_REQUIRED`, not `SCOPED_CREDENTIAL` — the latter matches the `credential` pattern of `tools/fixtures/secret-patterns.json` that `.github/workflows/checks/secret-scan.mjs` applies to every git-tracked file outside `docs/**` (D20b), so it would fail the PRD §20.2 gate on `FND-06`'s own source; widening the one-literal allowlist was rejected as out of file-scope and gate-weakening. | `FND-06` Feedback obligation 1 and General rule; PRD §38.1, §16.5, §8.1; D20b (the scan scope that forces the rename) |
| D22 | **`packages/domain` property tests use in-repo seeded generators, not a property-testing library.** `tools/tests/skeleton.test.mjs` asserts every pnpm member manifest declares `dependencies == {}` and `devDependencies == {}` (D2, D16), so `FND-06`'s Test-plan sentence "the property-testing library declared in `packages/domain/package.json`" is unsatisfiable without breaking a repository-wide `FND-01` invariant. The pattern the other domain tickets follow is therefore `test/<leaf>/arbitrary.ts`: a `mulberry32` PRNG with an exported constant seed and a printed case index, so ≥10,000-case properties stay deterministic and any counterexample is reproducible. Adding a library later is a `FND-01`-owned lockfile change plus an amendment to the skeleton assertion, not a domain ticket's to make. | `FND-06` Feedback obligation, General rule; `tools/tests/skeleton.test.mjs` (D2); D16 |
| D23 | **The `Permission → ApiScope` mapping lives in `packages/domain/src/access/scopes.ts` and nowhere else** (refines **D6**: `FND-03` owns the scope *vocabulary*, `FND-06` owns which scope a §38.1 action row demands). `CORPUS_SEARCH_READ → search:read`; `ANSWER_CREATE → answers:create, coverage:create`; `RESEARCH_RECORD_READ_WRITE_OWN → records:read, records:write`; `RESEARCH_RECORD_REVIEW_COMMENT → records:write`; `EXPORT_CREATE → exports:create`; `WATCHLIST_CREATE → monitor:write`; `USAGE_VIEW → usage:read`; every other permission maps to the empty set, because its §38.1 service-account cell is `—`. **Residue:** PRD §16.3's `monitor:read` maps to no §38.1 action row — the matrix has no "read watchlists" row (see **Q-F9**); the scope is recorded as broader than the matrix rather than answered by inventing a row or a permission. `02-auth-core`/`AUTC-04` (credential issuance) consumes this table through a writeback, never a second copy in `packages/auth` — a second mapping is the failure mode `FND-06`'s Feedback obligation 4 names. | `FND-06` Feedback obligation 4; PRD §16.3, §38.1; D6 |
...
| Q-F8 | PRD §38.1 row 7 (manage members/invitations) gives Admin *"✓ except Owner constraints"* while row 8 (change roles/remove members) gives Admin *"✓ cannot remove/change last Owner"*. Transcribed literally the two cells say different things: an Admin may demote or remove a **non-last** Owner under row 8, but is blocked from any Owner target under row 7. Intended, or is one cell shorthand for the other? | **Founder** (product ambiguity, PRD §45.5); drafted by the Builder of `FND-06` | `FND-06` implements the literal reading of each cell (the fixture carries both cell texts verbatim); confirmed or corrected at `13-identity-surface`/`IDNT-02` when the members screen exists | Nothing — both readings are buildable and the last-Owner invariant (PRD §8.1) holds either way | This README + the `FND-06` ticket and its `prd-38-1-matrix.json` fixture; a change in customer-visible admin behaviour also needs a PRD §45.5 update |
| Q-F9 | PRD §16.3's `monitor:read` scope maps to no PRD §38.1 action row — the matrix has no "read watchlists" row, so no service-account cell can demand that scope (**D23**). Does §38.1 need the row, or is the scope simply broader than the matrix? | **Architect**, with the founder on the product half (PRD §45.5) | Recorded by `FND-06`; confirmed by `02-auth-core`/`AUTC-04` (credential issuance) and `16-monitor-alerts` when watchlist reads get an endpoint | Nothing — an unmapped scope denies rather than over-grants | This README (**D23**) plus `FND-03`'s `ApiScope` enum and the `FND-06` ticket; never a second mapping table in `packages/auth` |
| Q-F10 | Nobody owns `packages/{contracts,domain}/src/index.ts`: `tools/tests/skeleton.test.mjs` freezes every pnpm member entry file at `export {};` (D2), so the `@taxrag/*` path aliases in `tsconfig.base.json` resolve to an empty module and every consumer must deep-import a leaf barrel by relative path (`FND-06` imports `../../../contracts/src/enums/index.js`). Which ticket un-freezes the root barrels, and does the skeleton assertion then need amending? | **Architect** (a breakdown plan §4 / sub-PRD allocation, same class as Q-F6) | Not `FND-06` — the relative import typechecks and runs today; `DATA-02` and `RUNT-02` hit it next | Nothing yet; it becomes friction once several packages consume `packages/domain` | `docs/prd/breakdown-plan.md` §4 (allocate the entry files to a ticket) plus this README and `tools/tests/skeleton.test.mjs` |
...
| v0.7 | 2026-08-08 | `FND-06` implementation writeback (ticket Feedback obligation 1 and General rule) — done **before** code, as the rule requires. New decision **D21**: the access decision input shape (`principal.id`, explicit `Grant` rows, `context.intent`/`target`/`usageView`/`auditView`, `resource: null` = named-but-not-found) is settled here once because `DATA-04` and `RUNT-02` both read it, and four PRD §38.1 conditional cells are undecidable without it; the same row records the rename of the service-account condition `SCOPED_CREDENTIAL` → `SCOPED_CREDENTIAL_REQUIRED`, forced by the D20b CI secret scan, which the `FND-06` ticket now carries with its reason. New decision **D22**: `packages/domain` property tests use in-repo seeded generators (`test/<leaf>/arbitrary.ts`, `mulberry32`, exported seed) rather than a property-testing library, because `tools/tests/skeleton.test.mjs` forbids any member dependency (D2/D16) — the ticket's Test-plan Harness sentence is amended accordingly and the ≥10,000-case acceptance items are unchanged. New decision **D23** (refines D6): the `Permission → ApiScope` mapping lives only in `packages/domain/src/access/scopes.ts`, with `monitor:read` recorded as unmapped residue. Three open questions added: **Q-F8** (PRD §38.1 row 7 vs row 8 — an Admin may act on a non-last Owner under the literal reading; founder), **Q-F9** (`monitor:read` maps to no matrix row; Architect/founder), **Q-F10** (nobody owns the frozen `src/index.ts` root barrels, so consumers deep-import a leaf barrel; Architect). |
