...
- **Implementation writeback (v1.1, 2026-08-08 — sub-PRD v0.9).** Eight things this ticket asserted that
  ...
  2. **`generate` / `generated:check` cannot be registered by a workspace member yet.**
     ...
     (sub-PRD **D22**) and discharges acceptance item 11's substance — a hand-edit is detected — inside
     the package's own vitest suite, which `pnpm test` runs on every PR. Recorded as sub-PRD **D23**,
  ...
  8. **`crypto.timingSafeEqual` is not reachable from this package, so the "or equivalent" branch of
     deliverable 3 is taken.** `packages/contracts` declares no dependency, so there is no
     `@types/node`, and `tsconfig.base.json` sets `lib: ["ES2024"]`: `import … from 'node:crypto'` is
     `TS2307` unless the package ships an ambient `declare module`, which every downstream consumer's
     program would then need too. Web Crypto needs no declaration but is async and offers no
     constant-time comparator, so it cannot back deliverable 3's synchronous signatures. SHA-256,
     HMAC-SHA256 and the comparator are therefore implemented in `src/events/{sha256,hmac,bytes}.ts`
     with **zero imports** (the shape `FND-03`'s `uuidv7.ts` established) and pinned byte-for-byte
     against Node's OpenSSL-backed `node:crypto` from `test/events/hmac.test.ts`, across every SHA-256
     block and padding boundary and both RFC 2104 key branches. Recorded as sub-PRD **D24**, with an
     ADR candidate (`docs/adr/NNNN-webhook-signing.md`) raised for the Architect — the signing input
     is unchanged, so nothing blocks on it.
...
4. **Generated TypeScript types** from the JSON Schemas into
   ...
   `packages/contracts/package.json`, sub-PRD **D22**). The aggregate `generate` / `generated:check`
   names are **not** registered here — see the writeback note above and sub-PRD **D23**; the
...
- [ ] `[machine]` `pnpm --filter @taxrag/contracts generate:events && pnpm --filter @taxrag/contracts
      ...
      hand-edit is detected, and the same comparison runs under `pnpm test` (PRD §20.1; sub-PRD D22/D23
...
5. **Constant-time check.** Read `sign.ts` and `bytes.ts`; confirm the *equivalent* comparator
   `equalsInConstantTime` (writeback note 8 — `crypto.timingSafeEqual` is unreachable from this
   package) on equal-length buffers, with an explicit length guard before it and no early `return`
   inside the loop. Confirm the static `===` check exists and fails when reverted on a scratch branch.
   Then read `test/events/hmac.test.ts`: the hand-written SHA-256/HMAC is only defensible while that
   differential run against `node:crypto` is intact.
...
8. **Generated check.** `pnpm --filter @taxrag/contracts generate:events` then
   ...
   both it and `pnpm test` fail; restore. (The aggregate root names are unregistered — sub-PRD D23.)
