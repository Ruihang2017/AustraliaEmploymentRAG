...
status: draft
date: 2026-08-15
blocked_by: [FND-01, FND-20, FND-25, FND-26, FND-27, FND-28]
blocks: []
---
...
| v1.2 | 2026-08-16 | Adds `FND-25` to `blocked_by`. Basis: v1.1's `overrides.nanoid` key in `pnpm-workspace.yaml` — the single carve-out this ticket owns — is rejected by a guard belonging to another module. `packages/database/test/migrate/file-scope.test.ts` (`DATA-01`'s, delivered) asserts the root workspace file's **entire top-level key set** exhaustively (`expect(topLevelKeys(workspace)).toEqual(['packages', 'allowBuilds'])`), which forbids not only a second `DATA-01` key but any key added by **`00-foundation`, the module that owns the file**; and the same test computes the `allowBuilds` entry list by slicing from the `allowBuilds:` line to the **end of the file**, so with `overrides:` appended it also evaluates to `['  better-sqlite3: false', '  nanoid: 3.3.18']` and fails a second time. Two independent failures, and no placement of the key avoids the first — key order is not a fix. That suite exists to keep `DATA-01`'s own out-of-file-scope exception (**M-Q4**) narrow, `packages/**` is outside this ticket's file-scope, and loosening another module's guard from inside this ticket is a rejected outcome, so the Builder stopped and escalated rather than editing it — the correct outcome under the Feedback obligation's general rule. The repair is **[`FND-25`](FND-25-narrow-data01-workspace-key-guard.md)**, which narrows `DATA-01`'s assertion to `DATA-01`'s own key (`toContain('allowBuilds')` plus a bounded entry computation) and updates the M-Q4 declaration to record that `00-foundation` now also holds a key in that file; this ticket is now `blocked_by` it and goes green when it lands. Nothing else here changes. This ticket's own deliverables 1–7 are complete and green on `ticket/FND-23` @ `fea02b1`: acceptance item 8 (`corepack pnpm audit --audit-level=high`, **unscoped**, exits 0 where `main` reports GHSA-2v37-7h3g-55p8), item 9 (`corepack pnpm install --frozen-lockfile` clean, the lockfile regenerated by the package manager), item 10 (`ci:local` in `rootScripts`, absent from `owners`, every root script still `node tools/workspace-script.mjs <name>`) and item 11 (the diff confined to the `ci:local` entry in `package.json` and the single `overrides.nanoid` key in `pnpm-workspace.yaml`, no pin moved). |
| v1.3 | 2026-08-16 | Adds `FND-26`, `FND-27` and `FND-28` to `blocked_by`. Basis: this ticket's headline acceptance is **`pnpm ci:local` exits 0**, and `ci:local` is *derived* from `.github/workflows/ci.yml` (deliverable 2) rather than transcribed — so it runs every command the nine CI jobs run, and it cannot exit 0 while **any** of them fails. That is not a scheduling nicety: a derived entry point that reported green while a derived command was red would be this phase's own root cause reproduced one level up. On `main` @ `5ac25c2`, six of the nine CI jobs pass and **three** fail, and all three failures are outside this ticket's file-scope. **(A)** `TypeScript type/unit tests` — `packages/domain/test/answers/decide-answer-status.property.test.ts:140` fails with `Error: Test timed out in 5000ms` (observed 7746 ms and 7859 ms) because nothing in the repository configures `testTimeout` anywhere, so every test runs on Vitest's 5000 ms default; the same missing setting is the long-running "wandering flake" in which different heavy files time out on different whole-suite parallel runs while each passes in isolation and the serial run is green. Repaired by **[`FND-26`](FND-26-global-vitest-test-timeout.md)**. **(B)** `Python builds/tests` — `pipelines/corpus-builder/tests/manifest/test_no_private_keys_committed.py:128` flags `pipelines/corpus-builder/tests/chunking/conftest.py: STABLE_SOURCE_KEY` because that guard *transcribes* the repository scan's patterns (its own docstring says so) and never reads `FND-24`'s exclusion fixture, so a name `FND-24` justified keeps failing one implementation of the scan and not the other — the `FND-11`/`FND-12`/`FND-19` transcription defect class. Repaired by **[`FND-27`](FND-27-python-guard-derives-shared-secret-patterns.md)**. **(C)** `Dependency, secret, container and artifact scans` — `node .github/workflows/checks/secret-scan.mjs` exits 1 on `packages/database/test/migrate/file-scope.test.ts` (four occurrences of `TOP_LEVEL_KEY`, an ALL-CAPS identifier ending in `_KEY` that is a YAML regex constant, not a credential), a **regression introduced by `FND-25`** and merged because that context is not in the branch-protection required set (**OPS-004**, still open). Repaired by **[`FND-28`](FND-28-rename-top-level-key-constant.md)**. **This ticket's own deliverables 1–7 are complete and verified green on `ticket/FND-23` @ `f298e6e`**: unscoped `corepack pnpm audit --audit-level=high` exits 0 on the branch and 1 on `main` (acceptance item 8); the `FND-25`-narrowed `DATA-01` guard passes with `overrides.nanoid` present; and 4325 tests pass in a serial run. The Builder therefore **escalated rather than reaching outside its declared file-scope** to repair three failures owned by `tools/vitest.config.mjs`, a `04-corpus-contract` Python guard and a `packages/database` test file — the correct outcome under the Feedback obligation's general rule, and the same boundary this ticket's Builder respected at v1.1. Nothing else here changes; this ticket goes green when the three repairs land. |
