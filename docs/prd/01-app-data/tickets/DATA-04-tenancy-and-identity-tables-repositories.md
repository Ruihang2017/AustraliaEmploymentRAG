...
status: draft
date: 2026-08-03
blocked_by: [DATA-02, DATA-03, DATA-10]
...
| v1.2 | 2026-08-19 | **The migration this ticket authorises breaks 17 tests it does not own; the repair is `DATA-10`, which is added to `blocked_by`.** Adding `packages/database/migrations/20260818093522_tenancy.sql` — exactly what this ticket's File-scope authorises — turns **17 tests red, all inside `packages/database/test/migrate/**` and none inside `packages/database/test/tenancy/**`**, because `DATA-01`'s migrate suite **pins the repository's migration list**: it asserts an exact set of migration filenames, and so encodes "the world has exactly these N migrations". Representative failure: `AssertionError: expected [ '0001_baseline.sql', ...(3) ] to deeply equal [ '0001_baseline.sql', ...(2) ]` at `test/migrate/runner.test.ts:388`; the affected files are `runner.test.ts`, `manifest.test.ts`, `conventions-lint.test.ts`, `concurrency.test.ts`, `cli.test.ts` and the shared harness `helpers.ts`. `packages/database/test/migrate/**` is **`DATA-01`'s declared write-owns** (*"this ticket's own test area, sub-PRD D8"*) and appears nowhere in this ticket's write-owns, so it is **not** `DATA-04`'s to repair — sub-PRD **D8**: *"No ticket writes into another ticket's test directory."* A Reviewer correctly blocked the attempted repair as a file-scope violation, and this ticket reached the **2-bounce cap on 2026-08-18**. The repair is the new **[DATA-10 — The migrate suite asserts framework properties, not the repository's migration count](DATA-10-migrate-suite-is-migration-count-agnostic.md)**, authored in `01-app-data` on 2026-08-19, which makes that suite assert the framework's properties instead of the repository's inventory; `DATA-10` is added to this ticket's `blocked_by` so `DATA-04` merges after it and lands green. It is a general repair rather than a one-off because `DATA-05`, `DATA-06` and `DATA-07` each own a different group-suffixed migration file and would each hit the same break. **One correction, recorded for the accuracy of the record:** the Reviewer's blocker text asserted that an authorising *"Anticipated forward references"* clause had been added to this ticket on the unmerged branch — that is not what happened; the ticket file on `ticket/DATA-04` is byte-identical to `main` (a `git diff` between them on this path is empty) and no such clause exists in either, so the Builder appears to have acted on an authorization that was never in the ticket. The file-scope violation was real; the self-amendment was not. `DATA-04`'s own work is otherwise **complete and green on `ticket/DATA-04` @ `6350421`**: `packages/database/test/tenancy/**` is 13 files / 73 tests passing, a mutation check confirms the AUTH-002 matrix can actually fail rather than passing vacuously, and the SSO `enforce()` negative-age hole was found and fixed with its own non-vacuity check. No id, title, other frontmatter field, File-scope entry, Non-goal, deliverable, acceptance item, test-plan step or earlier changelog row is changed by this revision. |
