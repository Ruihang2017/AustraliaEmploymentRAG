...
| D14 | **A table-group ticket may build and execute its own tenant-scoped statements, through the same chokepoint the factory uses.** `DATA-02`'s `defineTenantRepository` cannot express three behaviours PRD §35.4 requires of the tenancy group: an `APPEND_ONLY` table has no `update` member yet must stamp `accepted_at`/`revoked_at`/`last_used_at`; invitation single-use is a *conditional* update (`… AND accepted_at IS NULL AND expires_at > ?`); and `row_version` compare-and-swap needs `… AND row_version = ?` in the WHERE clause rather than a read-compare-write in JavaScript. `DATA-04` therefore ships `src/repos/tenancy/internal/statement.ts`, which reproduces the factory's own path exactly — `requireOpenTx` → build with `db.qb` → `.compile()` → `assertTenantScoped` → `db.sqlite.prepare(...)` → `recordTxChange` — and never calls `.execute()` on a Kysely builder. This widens nothing about who may reach a connection (D11/D12 hold: the handle is still passed, still absent from the `exports` map, and every statement still passes the chokepoint); it widens *who may compose a statement* from `src/tenant/**` to a repository module inside `packages/database/src/repos/**`. `DATA-05`…`DATA-07` inherit the pattern and must reuse this helper rather than copy its body; promoting it into `src/tenant/**` is a later `DATA-02`-owned ticket's call. | `DATA-04` implementation (2026-08-18), under the ticket's Feedback obligation; PRD §35.4 (single-use invitation, credential revocation stamps, `row_version`), §34.1; plan §8 **Q13** and **D11**/**D12**, neither of which this changes |
...
| M-Q4 | **Who owns the workspace dependency-build policy?** `DATA-01` declares the workspace's first native dependency (`better-sqlite3`, ADR `0002` (vi)), and pnpm 11 refuses to install it unless the **root** `pnpm-workspace.yaml` names it under `allowBuilds` — `strictDepBuilds` defaults to true and the package ships a `binding.gyp`, so an install that neither allows nor denies it exits `ERR_PNPM_IGNORED_BUILDS`. pnpm reads that policy from the workspace root only: `dependenciesMeta.built` and a `pnpm` block in `packages/database/package.json` were both measured and neither suppresses it, and both candidate versions trip it (`12.11.1` needs `true`, `13.0.3` needs `false`). So `DATA-01`'s own deliverable cannot be satisfied inside `DATA-01`'s file-scope, whose "Does not touch" list assigns root manifests to `00-foundation`. `DATA-01` shipped the minimal form — one key, one entry, `better-sqlite3: false` — under this entry rather than silently. Since `FND-23`, this file is **no longer single-tenant**: it also carries a `00-foundation`-owned `overrides.nanoid` key, added under `FND-23`'s own single-key carve-out to close GHSA-2v37-7h3g-55p8. `FND-25` consequently narrowed the `DATA-01` guard in `packages/database/test/migrate/file-scope.test.ts` to `DATA-01`'s own key: a borrower may assert its exception stays narrow, not that the file's owner never changes the file. **M-Q4 remains open** — `allowBuilds` is still where `DATA-01` put it. | **`00-foundation`**, with the Architect: either allocate the `allowBuilds` key to `00-foundation` and move it there, or add it to `DATA-01`'s file-scope by a ticket edit | A `00-foundation` ticket, or a `DATA-01` ticket edit + `publish-tickets.mjs --sync`. Until then the key stays where it is: removing it makes every install in the repository fail, which is strictly worse than a declared exception. | Nothing today. Every later module that declares a native dependency hits the same root file, which is the contention the disjoint-file-scope decomposition exists to prevent — so it should be settled before a second one lands |
| M-Q5 | **Composite tenant foreign keys are not feasible for three tenancy links** (PRD §15.4 says "where feasible"), recorded here under **D1** with the compensating application check `DATA-09` must cover at the repository level for invariant 4: (a) `invitation.invited_by_actor_id → actor` — `actor` is `GLOBAL` (PRD §35.4 "stable audit identity", no `organization_id`), so no composite key exists; compensating check: `invitations.create` resolves the actor inside the caller's transaction and refuses one whose `USER`/`SERVICE_ACCOUNT` linkage does not belong to the context's organisation. (b) `membership.user_id → user` — `user` is `GLOBAL` by design (AUTH-002: a user belongs to several organisations); the tenant boundary is `membership`'s own `organization_id`. (c) `actor.service_account_id → service_account` — a `GLOBAL` child pointing at a tenant row; plain single-column FK. The one link where it *is* feasible, `api_credential.(organization_id, service_account_id) → service_account.(organization_id, id)`, is structural and asserted at the database level. | `01-app-data` — the `DATA-09` Builder, with the Architect | `DATA-09` invariant 4's property test covering (a)–(c) at the repository level | Nothing — `DATA-04` ships the compensating check for (a) |
| M-Q6 | **`membership`'s primary key departs from the normative PRD §35.4 constraint.** §35.4 says "composite PK" for `(organization_id, user_id)`. `DATA-02`'s repository factory addresses every row by a single `id` (it refuses a spec without `id` in `requiredColumns`, and every read/update/delete is `WHERE id = ? AND organization_id = ?`), so `DATA-04` ships a synthetic `id TEXT PRIMARY KEY` **plus** `UNIQUE (organization_id, user_id)`. The uniqueness §35.4 asks for is enforced identically; only the addressing key differs. | **Architect**, with the PRD owner — either accept the synthetic key in §35.4 or change the repository addressing contract | A PRD §35.4 amendment, or a `DATA-02`/`DATA-04` ticket edit | Nothing — the constraint is enforced either way |
| M-Q7 | **Seven controlled-value columns have no canonical `packages/contracts` enum.** `membership.role`, `invitation.role` (`Role`) and `sso_connection.state` (`SsoConnectionState`) are registered and generated from the registry. `organization.plan`, `organization.default_legal_date_policy`, `user.status` and `service_account.status` have **no vocabulary anywhere in the PRD**, so `DATA-04` declares them `TEXT NOT NULL` with **no CHECK** rather than inventing one. `organization.status`, `membership.status`, `actor.actor_type` and `sso_connection.protocol` do need a value set because this ticket's own acceptance depends on distinguishing values (closure blocks writes; last `ACTIVE` `OWNER`; `DATA-02`'s `ActorType` union; PRD §16.3/§38.3's SAML/OIDC), so they carry a module-local frozen constant in `src/schema/tenancy.ts` and a pasted CHECK, and are deliberately **absent** from the manifest's `enumColumns` (that map is checked against the contracts registry and an unregistered family throws). | **`00-foundation` / `FND-03` owner**, raised by `DATA-04`'s Feedback obligation | `FND-03` canonicalising the families, then a `DATA-04`-successor ticket moving the CHECKs onto `enumCheck` and the columns into `enumColumns` | `IDNT-*` rendering these values; nothing in this module today |
...
| M-Q11 | **RESOLVED by `DATA-10`** — *Migrate suite asserts framework properties, not the repository's migration count* — authored 2026-08-19 and merged to `main` in PR #313 (`b564953`). The route taken is the one this row itself named as preferred: a **`DATA-01`-area repair ticket**, **not** a `DATA-04` File-scope amendment — so `DATA-04`'s File-scope is unchanged and the reverted repair described below stays reverted. `DATA-10` is now in `DATA-04`'s `blocked_by` (ticket v1.2), and `DATA-05`…`DATA-07`, which hit the same four files identically, are unblocked by the same repair. The original diagnosis is kept below as the record of why the repair exists. **Diagnosis (as recorded 2026-08-18): `DATA-04` could not go green without changing four `DATA-01`-owned files, and no ticket authorized that at the time.** `DATA-04` ships the repository's **first migration after the baseline** and the **first** `src/schema/` manifest, and four files in `DATA-01`'s test area (sub-PRD **D8**) were written while neither existed: (i) `test/migrate/manifest.test.ts` asserts `discoverTableManifests()` is `[]`, in a test titled *"defaults to packages/database/src/schema, which DATA-04 creates"*; (ii) `test/migrate/conventions-lint.test.ts` asserts the same, commented *"Vacuous by construction — `src/schema/` does not exist until DATA-04 lands"*; (iii) `test/migrate/helpers.ts`'s `withTempMigrations` copies the **whole** shipped `migrations/` directory into every fixture directory, so each fixture-based test silently inherits `*_tenancy.sql` — against its own comment, *"The fixtures deliberately do NOT carry their own copy of the baseline"*; (iv) three assertions in `test/migrate/runner.test.ts` pin the shipped sequence to exactly `['0001_baseline.sql']`. Measured on `ticket/DATA-04`: **17 failures across 5 files**, every one inside `test/migrate/**`, none inside `test/tenancy/**`. (i), (ii) and (iv) are the over-broad shape `FND-25`'s governing principle names — a module may assert its own exception stays narrow, it may not assert that another module never creates its own file — and (iii) is a harness that does not do what its own comment says. `DATA-05`…`DATA-07` each hit all four identically, so this is not `DATA-04`-specific. An earlier `DATA-04` build widened its own File-scope by writing an "Anticipated forward references" block into its ticket **on the implementing branch**; review round 4 bounced that as a blocker (CLAUDE.md's spec-change path is ticket edit → docs PR → merge → `publish-tickets.mjs --sync` → then execute, and the block also carried no version bump or Changelog row), and both the block and the four file edits have been reverted. **The reverted repair is recorded here so it is not re-derived from scratch**, but it is not applied. | **Decided**: the Architect, with the `DATA-01` owner, chose the `DATA-01` repair ticket over a `DATA-04` File-scope amendment | **Done** — `DATA-10` landed on `main` (PR #313, `b564953`); `DATA-04` takes it by merging `main` and changes nothing under `test/migrate/**` itself | **No longer blocking.** `DATA-04` merges `main` and is green; `DATA-05`, `DATA-06` and `DATA-07` inherit the same repair |
...
- **v0.8 — 2026-08-19** — open question **M-Q11** moves to **resolved**, by `DATA-10` (*Migrate
  ...
  PR #313 (`b564953`). The route taken is the `DATA-01`-area repair ticket the row itself named as
  preferred, **not** a `DATA-04` File-scope amendment, so `DATA-04`'s File-scope is unchanged and
  the reverted `test/migrate/**` edits stay reverted; `DATA-05`…`DATA-07` are unblocked by the same
  repair. The original diagnosis is kept in the row as the record of why the repair exists. No
  change to module scope, ticket set, dependency edges, decisions D1–D14, the §35.8 invariants or
  PRD traceability.

- **v0.7 — 2026-08-18** — `DATA-04` review round 3, under the ticket's Feedback obligation. The
...
- **v0.6 — 2026-08-18** — `DATA-04` implementation, under the ticket's Feedback obligation. Adds
  ...
  helper `DATA-05`…`DATA-07` inherit) and open questions **M-Q5**–**M-Q10** (the three infeasible
  composite tenant FKs and their compensating checks, recorded under **D1** and flagged to `DATA-09`
  ...
  **unresolved** cross-ticket dependency and not something this implementation was entitled to
  ...
  §35.8 invariants or D1–D13. (This entry and `v0.7` carry `2026-08-18` dates that precede
  `v0.5`’s `2026-08-19`: both were authored on the `ticket/DATA-04` branch and landed on `main`
  after `DATA-10`, so the non-monotonic dates are deliberate, not an error.)

- **v0.5 — 2026-08-19** — adds `DATA-10` to the ticket set: count 9 → 10, a work-breakdown row,
  and wave 1 alongside `DATA-01` — it is `blocked_by: []` and blocks `DATA-04`…`DATA-07`, so it
  precedes the four schema-group tickets and peak lane width stays two. Resolves that ticket’s
  **Q-D10-A**, which asked whether this README gains the row now or at the next module pass.
  Bookkeeping only — no change to scope, goals/non-goals, decisions D1–D13, any other ticket’s row
  or dependency edges, the §35.8 invariants or PRD traceability.
