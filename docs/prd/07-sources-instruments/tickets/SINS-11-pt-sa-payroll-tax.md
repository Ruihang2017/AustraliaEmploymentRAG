...
status: draft
date: 2026-08-03
blocked_by: [SINS-01, SLEG-07]
blocks: [GOLD-08, GOLD-16]
---

# SINS-11 — `PT-SA` payroll tax

Implements PRD §40.3 (wave-2 source group `PT-SA`), PRD §6.3 (state and territory scope) and PRD
§40.8 (adapter Definition of Done) <`ADM-001`, `SRCH-002`> — **No ADR — the decision is already made
in PRD §40.3; this is build ticket 11 of 14 against it.**
...
`SLEG-07` — `LEG-SA`, module `06-sources-legislation`
...
**The row names two distinct guidance instrument classes — rulings *and* circulars.** They are not
interchangeable: each is typically numbered, dated and capable of superseding an earlier one. PRD
§35.2 gives `legal_document` an `official_identifier` with "exact indexes on identifiers", and PRD
§15.2 requires legal status to be "derived from evidenced LegalEvents" — so a superseded circular
must become `SUPERSEDED` **on evidence**, not by assumption. That supersession chain is this group's
distinguishing deliverable.
...
1. `PT-SA` is a mandatory group and is attempted **in full** — never pre-selected for omission or
   reduced implementation, and never trimmed to make a release date easier. That applies to all eight
   payroll-tax jurisdictions equally, and to both guidance instrument classes the row names.
...
**Why `blocked_by SLEG-07`.** The rate is imposed under South Australian payroll-tax legislation,
which `LEG-SA` (`SLEG-07`, module `06-sources-legislation`) puts in the corpus. This ticket is ordered
...
and this ticket contributes **the SA row** across the three PRD §6.6 financial years (2026-27,
...
declared value re-parses from its own quoted source span. Where a jurisdiction's rate varies
continuously with payroll size rather than in fixed steps, the temptation to encode a formula is
strong — and a formula is a calculation that would then be cited as an official figure. Emit only what
the source prints.
...
Deliver the `PT-SA` source adapter under `pipelines/adapters/pt-sa/**`: the per-adapter
`registry.yaml`, `allowlist.yaml`, `licence.yaml` + immutable licence snapshot, an `adapter.py`
exposing `ADAPTER: SourceAdapter` with all eight PRD §40.7 boundaries over the RevenueSA payroll-tax
collections, **dated rate and threshold facts emitted as `SINS-01` `RateFact`s across the three PRD
§6.6 financial years with resolved `legislation_ref`s into `LEG-SA`**, rulings and circulars carried
with their official identifiers and an **evidenced supersession chain**, guides and change notices
captured as subordinate authority with citable nodes, a committed `fixtures/rate-matrix.json`
contributing the SA row of the PRD §44.2 `E13` matrix, and the complete PRD §40.8 fixture set — such
that `python -m <iroot>.conformance check pipelines/adapters/pt-sa` exits 0 in strict mode.

## Non-goals

- **No rate model changes.** `SINS-01` owns `_shared/rates/**` (plan §9 **R2**).
- **No South Australian legislation.** The payroll-tax Act and its instruments are `LEG-SA`
  (`SLEG-07`, module `06`). This ticket **reads** those nodes to resolve `legislation_ref`; it emits
  no legislation document and no legislation event.
- **No other jurisdiction.** `PT-NSW`…`PT-WA`, `PT-TAS`…`PT-NT` are `SINS-07`…`SINS-10`,
  `SINS-12`…`SINS-14`.
- **No employment-adjacent South Australian regulators** — SafeWork SA, Equal Opportunity SA,
  ReturnToWorkSA, labour-hire licensing and portable-LSL authorities are `ADJ-SA` (`SADJ-06`, module
  `09-sources-adjacent`).
- **No SA court or SAET decisions** — `CASE-SA` (`SCAS-10`, module `08-sources-cases`).
...
- `pipelines/adapters/pt-sa/**` — the whole group directory: `registry.yaml`, `allowlist.yaml`,
...
- Does not touch: `pipelines/adapters/pt-{nsw,vic,qld,wa,tas,act,nt}/**` — `SINS-07`…`SINS-10`,
  `SINS-12`…`SINS-14`.
- Does not touch: `pipelines/adapters/{fwc-docs,fwc-awards,fwc-agreements,fwo-guidance,ato-employment}/**`
  — `SINS-02`…`SINS-06`.
- Does not touch: `pipelines/adapters/leg-sa/**`, `adj-sa/**`, `case-sa/**`, `future-sa/**` — modules
  `06`, `09`, `08`, `10`. `leg-sa` is read-only from here.
...
`SLEG-07` have landed; all three are read-only from here. The seven sibling payroll-tax tickets run in
...
1. **`registry.yaml`** (`INGF-07` schema) with all nine PRD §6.1 attributes: `group_id: PT-SA`,
   `wave: 2`; `authority` = RevenueSA with `authority_type: REVENUE_OFFICE`, `jurisdiction: SA`,
   `official_url`; `official_endpoints` — one entry per payroll-tax collection actually used, each
   with `kind` and a PRD §40.5 `material_class` (`GUIDANCE` for guides, `GUIDANCE`/`POLICY` for
   rulings and circulars, `NEWS` for change notices) — never `LAW` or `OPERATIVE_INSTRUMENT`;
   `document_coverage.families` covering the row's required artifacts — **rates/thresholds, guides,
   rulings/circulars and changes** — with `financial_years: ['2024-25','2025-26','2026-27']`
...
2. **`allowlist.yaml`** (`INGF-02` schema): `schemes: [https]`, the RevenueSA host with
...
3. **`licence.yaml` + `licence-snapshots/`** via
   `python -m <iroot>.licensing capture pipelines/adapters/pt-sa`, stating all nine PRD §11.1 axes
...
4. **`adapter.py`** exposing `ADAPTER: SourceAdapter` with
   `AdapterMeta(group_id="PT-SA", adapter_key="pt-sa", jurisdiction="SA", …)` and all eight PRD §40.7
   boundaries. `discover` traverses the allowlisted collections with a `DiscoveryCursor` and honours
   `since`; `fetch` through `ctx.fetcher` with conditional-request validators; `parse` through
   `ctx.parser`.
5. **Ruling and circular identity (this row's distinguishing artifact).** `identify()` extracts the
   official identifier a ruling or circular prints on itself into
   `document_identity.official_identifier`, and assigns `document_type` from a **closed set**
   resolved against `packages/contracts` (sub-PRD **N1**) that keeps a ruling, a circular, a guide and
   a change notice distinguishable. Identifiers are **read**, never constructed or completed from
   model knowledge; an unparseable identifier quarantines rather than emits (`SRCH-004`, PRD §35.2
   "exact indexes on identifiers"). A module-level constant names the **forbidden** `document_type`
   set — legislation, legislative-instrument, operative-instrument — and a guard raises before
   emission.
6. **Evidenced supersession chain.** Where a ruling or circular states that it replaces, withdraws or
   supersedes an earlier one, emit a `legal_event` with `event_date` and `effective_date`
   distinguished and an `evidence_ref` to the node that states it, plus a `node_relation` with
   `derivation: DETERMINISTIC` and a non-model `confidence_state`; the superseded document's
   `legal_status` becomes `SUPERSEDED` **only** on that evidence (PRD §15.2, §6.7). Where supersession
   is merely implied — a newer document on the same topic — **nothing is asserted** (PRD §9.3;
   PRD §35.2: "`MODEL_SUGGESTED` cannot support definitive status").
7. **Dated rate and threshold facts through `SINS-01`.** For every rate and threshold the material
   publishes, emit a `RateFact` with `fact_key` in a documented namespace
   (`payroll-tax.sa.rate.<variant>`, `payroll-tax.sa.threshold.<variant>`, …), `jurisdiction: "SA"`,
   the correct `RateKind` and `Unit`, `applies_from`/`applies_to` from the dates the source states,
   `financial_year` where FY-periodised, `source_role: "OFFICIAL_GUIDANCE"` (or `"RULING"` where the
   value is stated in a ruling or circular), and `evidence` whose `quoted_text` is the exact span the
   number is printed in. Emission uses `rate_records()` — `SINS-01` deliverable 6's single mapping.
   **A rate that varies with payroll size is emitted as `RateValue.shape = "BRACKETED"` using the
   source's own published bounds and values; a formula or interpolation rule is never encoded as a
   fact** (PRD §40.3; the product is research, not a calculator).
8. **Resolved `legislation_ref`.** Every emitted `RateFact` carries a `legislation_ref` `NodeRef` into
   a `LEG-SA` node (`SLEG-07`), and `rate_records()` emits the `node_relation` of `SINS-01`
...
9. **Guides and change notices as citable nodes.** Emitted with their own headings and sections as
...
14. **Failure codes** with `register_failure_codes("pt-sa", …)`, each with a non-empty operator action
    (PRD §40.8 item 10, ADM-001) — at minimum: rate table not located, rate value not present in
    source, circular identifier unparseable, supersession target unresolved, effective date
    unparseable, legislation reference unresolved, document type would be operative, collection count
    anomaly.
15. **`README.md`** in the group directory: collections used, the ruling/circular identifier grammar
    and the supersession rule, the `fact_key` namespace, the `fact_key` → SA provision mapping, the
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml`, `licence.yaml` validate; `PT-SA` is
...
- [ ] `[fixture]` **DoD 12** — the replayed full run records non-zero `storage_bytes`,
      ...
      (PRD §39.2).
- [ ] `[machine]` `python -m <iroot>.conformance check pipelines/adapters/pt-sa` exits 0 in **strict**
      mode; the committed `conformance-report.json` shows no `FAIL` and no `NOT_AVAILABLE`
      (PRD §45.4).

**PRD §44.2 `E13` exit evidence:**

- [ ] `[fixture]` **The SA row of the eight-jurisdiction historical fixture matrix** —
      `assert_rate_matrix(pipelines/adapters/pt-sa)` passes: all three PRD §6.6 financial years have a
...
- [ ] `[machine]` **Ruling/circular identity (deliverable 5)** — a parser table over recorded fixtures
      ...
      record (`SRCH-004`).
- [ ] `[fixture]` **Evidenced supersession (deliverable 6)** — the superseding/superseded circular
      pair produces a `legal_event` with an `evidence_ref`, a deterministic `node_relation`, and
      `legal_status: SUPERSEDED` on the earlier document; a fixture where supersession is only implied
      asserts **nothing** (PRD §9.3, §15.2, §35.2).
...
- [ ] `[machine]` **No formula stands in for a published figure (deliverable 7)** — a size-varying
      rate is emitted as `BRACKETED` from the source's own bounds, and a test asserts no fact carries
      a computed value absent from its quoted span (PRD §40.3, sub-PRD D3).
...
- [ ] `[machine]` **Resolved `legislation_ref` (deliverable 8)** — every emitted `RateFact` carries a
      `legislation_ref` into a `LEG-SA` node and a deterministic `node_relation`; an
...
- [ ] `[machine]` The adapter imports no HTTP library and no HTML/XML/PDF parsing library —
      `INGF-01`'s AST scan over `pipelines/adapters/pt-sa/**` passes (PRD §37.4, SEC-002).
- [ ] `[machine]` `python -m <iroot>.registry validate pipelines/adapters/pt-sa` exits 0 and a
...
- [ ] `[human]` **`fact_key` → provision mapping and supersession review** — is each rate/threshold
      linked to the right SA provision, and is the supersession chain right? A wrongly superseded
      circular removes live guidance from answers; a wrongly live one keeps stale guidance in them.
      PRD §43.4 item 4 puts source adapter anomalies in the founder review queue.
...
Harness: `uv run pytest pipelines/adapters/pt-sa -q` plus the conformance CLI. All replays are offline
...
from `pipelines/adapters/_shared/rates/README.md` (`SINS-01`).

1. `uv sync --frozen && uv run pytest pipelines/adapters/pt-sa -q`.
2. `python -m <iroot>.registry validate pipelines/adapters/pt-sa` — exit 0.
3. `python -m <iroot>.conformance check pipelines/adapters/pt-sa --report conformance-report.json`
...
4. **`tests/test_rate_facts.py`** — for each of the three financial-year time points, assert
   ...
   `quoted_text`; the bracketed size-varying case; the "changed in code only" mutation asserting
...
8. **`tests/test_document_typing.py`** — ruling/circular/guide/notice typing and the forbidden-type
...
**Reviewer focus.** (a) Run `tests/test_circulars.py`'s implied-supersession fixture: an assumed
supersession silently removes live official guidance from every future answer, and PRD §9.3 permits
only evidenced assertions. (b) Run the "changed in code only" mutation. (c) Confirm no formula stands
in for a published figure — a size-varying rate must come from the source's own printed bounds.
(d) Confirm every fact resolves to a real `LEG-SA` node. (e) Confirm the recorded
`change_detection.capability` is backed by evidence in `fixtures/dry-run.json`. (f) If the group is
...
3. **Supersession is real but never stated in a machine-readable way** → record a `known_gaps` entry
   with `customer_visible: true` describing that circular status may lag, and leave the status
   unchanged rather than inferring it. PRD §15.2 makes status derive from evidence; PRD §6.7 provides
   `STATUS_UNCONFIRMED` for the honest case.
4. **A rate varies continuously with payroll size and the source publishes a formula, not a table** →
   capture the formula's **text** as a citable node and emit **no** derived fact. A computed rate
   presented as an official figure is an unsupported definitive claim, which PRD §43.3 gates to zero.
   Record a `known_gaps` entry and raise the modelling question against `SINS-01` if a bounded
   representation is genuinely possible.
...
PRD §44.4's two. If the three-financial-year matrix cannot be completed for South Australia, that also
puts PRD §26's Corpus Definition of Done at risk and must be raised before release, not at it.
