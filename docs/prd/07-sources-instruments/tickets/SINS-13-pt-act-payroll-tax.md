...
status: draft
date: 2026-08-03
blocked_by: [SINS-01, SLEG-09]
blocks: [GOLD-08, GOLD-16]
---

# SINS-13 — `PT-ACT` payroll tax

Implements PRD §40.3 (wave-2 source group `PT-ACT`), PRD §6.3 (state and territory scope) and PRD
§40.8 (adapter Definition of Done) <`ADM-001`, `SRCH-002`> — **No ADR — the decision is already made
in PRD §40.3; this is build ticket 13 of 14 against it.**
...
`SLEG-09` — `LEG-ACT`, module `06-sources-legislation`
...
**The related PRD §40.2 row matters here**, because it fixes what this group's `legislation_ref` can
point at:
...
So in the ACT the target of a rate's "applicable legislation" reference may be an **Act provision or a
subordinate/disallowable/notifiable instrument** node — both are in `LEG-ACT`'s scope (`SLEG-09`,
this ticket's blocker). The adapter must resolve to whichever the source actually cites and must
record the target's kind, not assume an Act section.

**Note what the §40.3 row does not say (sub-PRD D7).** It has no "Minimum adapter capability" column
...
1. `PT-ACT` is a mandatory group and is attempted **in full** — never pre-selected for omission or
   reduced implementation, and never trimmed to make a release date easier. All eight payroll-tax
   jurisdictions are treated equally; a small territory is not a candidate for reduced work.
...
**Circulars are the row's named guidance instrument.** They are typically numbered, dated and capable
of superseding an earlier one. PRD §35.2 gives `legal_document` an `official_identifier` with "exact
indexes on identifiers", and PRD §15.2 requires legal status to be "derived from evidenced
LegalEvents" — so a superseded circular becomes `SUPERSEDED` **on evidence**, never by assumption.
...
and this ticket contributes **the ACT row** across the three PRD §6.6 financial years (2026-27,
...
declared value re-parses from its own quoted source span. Territory figures are among the least
reliably recalled and most confidently asserted; the committed fixture is the only permitted source.

**Circulars and guidance are subordinate authority (PRD §9.1)** — level 6, and *"Guidance MUST NOT
silently override legislation, an operative instrument or binding authority."* Note the distinction
this group makes sharp: a **circular** is guidance, while the **instrument** it describes may be
operative law owned by `LEG-ACT`. The adapter emits `document_type` + `authority_key`, never a level;
`FND-10` computes the hierarchy (sub-PRD **D5**).
...
Deliver the `PT-ACT` source adapter under `pipelines/adapters/pt-act/**`: the per-adapter
`registry.yaml`, `allowlist.yaml`, `licence.yaml` + immutable licence snapshot, an `adapter.py`
exposing `ADAPTER: SourceAdapter` with all eight PRD §40.7 boundaries over the ACT Revenue Office
payroll-tax collections, **dated rate and threshold facts emitted as `SINS-01` `RateFact`s across the
three PRD §6.6 financial years with resolved `legislation_ref`s into `LEG-ACT` — resolving to an Act
provision or to a subordinate/disallowable/notifiable instrument node, whichever the source cites**,
circulars carried with their official identifiers and an evidenced supersession chain, guidance and
change notices captured as subordinate authority with citable nodes, a committed
`fixtures/rate-matrix.json` contributing the ACT row of the PRD §44.2 `E13` matrix, and the complete
PRD §40.8 fixture set — such that `python -m <iroot>.conformance check pipelines/adapters/pt-act`
exits 0 in strict mode.

## Non-goals

- **No rate model changes.** `SINS-01` owns `_shared/rates/**` (plan §9 **R2**).
- **No ACT legislation or instruments.** Acts, subordinate laws, disallowable/notifiable instruments,
  commencement notices and republications are `LEG-ACT` (`SLEG-09`, module `06`). This ticket
  **reads** those nodes to resolve `legislation_ref`; it emits no legislation document, no instrument
  and no register event.
- **No other jurisdiction.** `PT-NSW`…`PT-TAS`, `PT-NT` are `SINS-07`…`SINS-12`, `SINS-14`.
- **No employment-adjacent ACT regulators** — WorkSafe ACT, the ACT Human Rights Commission and the
  ACT Long Service Leave Authority are `ADJ-ACT` (`SADJ-08`, module `09-sources-adjacent`).
- **No ACT court or ACAT decisions** — `CASE-ACT` (`SCAS-12`, module `08-sources-cases`).
...
- `pipelines/adapters/pt-act/**` — the whole group directory: `registry.yaml`, `allowlist.yaml`,
...
- Does not touch: `pipelines/adapters/pt-{nsw,vic,qld,wa,sa,tas,nt}/**` — `SINS-07`…`SINS-12`,
  `SINS-14`.
- Does not touch: `pipelines/adapters/{fwc-docs,fwc-awards,fwc-agreements,fwo-guidance,ato-employment}/**`
  — `SINS-02`…`SINS-06`.
- Does not touch: `pipelines/adapters/leg-act/**`, `adj-act/**`, `case-act/**`, `future-act/**` —
  modules `06`, `09`, `08`, `10`. `leg-act` is read-only from here.
...
`SLEG-09` have landed; all three are read-only from here. The seven sibling payroll-tax tickets run in
...
1. **`registry.yaml`** (`INGF-07` schema) with all nine PRD §6.1 attributes: `group_id: PT-ACT`,
   `wave: 2`; `authority` = the ACT Revenue Office with `authority_type: REVENUE_OFFICE`,
   `jurisdiction: ACT`, `official_url`; `official_endpoints` — one entry per payroll-tax collection
   actually used, each with `kind` and a PRD §40.5 `material_class` (`GUIDANCE` for guidance and
   circulars, `POLICY` where the source labels a document as policy, `NEWS` for change notices) —
   never `LAW` or `OPERATIVE_INSTRUMENT`, because the instruments themselves belong to `LEG-ACT`;
   `document_coverage.families` covering the row's required artifacts — **rates/thresholds,
   circulars, guidance and changes** — with `financial_years: ['2024-25','2025-26','2026-27']`
   (PRD §6.6) or a `known_gaps` entry; `initial_index_tier: T1`; `change_detection.*` **as measured**;
   `known_gaps` with `customer_visible` flags; `evaluation_subset_ref`.
...
2. **`allowlist.yaml`** (`INGF-02` schema): `schemes: [https]`, the ACT Revenue Office host with
...
3. **`licence.yaml` + `licence-snapshots/`** via
   `python -m <iroot>.licensing capture pipelines/adapters/pt-act`, stating all nine PRD §11.1 axes
...
4. **`adapter.py`** exposing `ADAPTER: SourceAdapter` with
   `AdapterMeta(group_id="PT-ACT", adapter_key="pt-act", jurisdiction="ACT", …)` and all eight PRD
   ...
   through `ctx.parser`.
5. **Circular identity and evidenced supersession.** `identify()` extracts the official identifier a
   circular prints on itself into `document_identity.official_identifier` and assigns `document_type`
   from a **closed set** resolved against `packages/contracts` (sub-PRD **N1**) keeping a circular,
   guidance and a change notice distinguishable. Identifiers are **read**, never constructed;
   an unparseable identifier quarantines rather than emits (`SRCH-004`). Where a circular states that
   it replaces or withdraws an earlier one, emit a `legal_event` with `event_date` and
   `effective_date` distinguished and an `evidence_ref`, plus a `node_relation` with
   `derivation: DETERMINISTIC` and a non-model `confidence_state`; the superseded circular's
...
6. **Dated rate and threshold facts through `SINS-01`.** For every rate and threshold the material
   ...
   (`payroll-tax.act.rate.<variant>`, `payroll-tax.act.threshold.<variant>`, …),
   `jurisdiction: "ACT"`, the correct `RateKind` and `Unit`, `applies_from`/`applies_to` from the
   dates the source states, `financial_year` where FY-periodised,
   `source_role: "OFFICIAL_GUIDANCE"` (or `"RULING"` where the value is stated in a circular that the
   source itself treats as a ruling), and `evidence` whose `quoted_text` is the exact span the number
   is printed in. Emission uses `rate_records()` — `SINS-01` deliverable 6's single mapping. Every
   value comes from the committed fixture text.
7. **Resolved `legislation_ref`, Act **or** instrument (this group's distinguishing obligation).**
   Every emitted `RateFact` carries a `legislation_ref` `NodeRef` into a `LEG-ACT` node (`SLEG-09`) —
   an Act provision **or** a subordinate/disallowable/notifiable instrument node, whichever the source
   cites — and `rate_records()` emits the `node_relation` of `SINS-01` deliverable 6 step 4 with
   `derivation: DETERMINISTIC` and a non-model `confidence_state`. The group's committed `fact_key` →
   target mapping records the **kind** of each target (Act provision vs instrument) so a reviewer can
   check it; where the source cites no instrument, the mapping is used and reviewed as part of this
...
8. **Guidance and change notices as citable nodes.** Emitted with their own headings and sections as
...
10. **Fixtures (PRD §40.8 items 2, 4, 6, 7, 10).** `fixtures/discovery/`; `fixtures/dry-run.json`
    ...
    media type **including a superseding/superseded circular pair**, scrubbed of customer
...
13. **Failure codes** with `register_failure_codes("pt-act", …)`, each with a non-empty operator
    action (PRD §40.8 item 10, ADM-001) — at minimum: rate table not located, rate value not present
    in source, circular identifier unparseable, supersession target unresolved, effective date
    unparseable, legislation reference unresolved, document type would be operative, collection count
    anomaly.
14. **`README.md`** in the group directory: collections used, the circular identifier grammar and the
    supersession rule, the `fact_key` namespace, **the `fact_key` → ACT Act-provision-or-instrument
    mapping with each target's kind**, the three-financial-year evidence with its fixture references,
    the recorded change-detection capability with its evidence, the known gaps, and — if the group
    carries a `limitation` — the evidence, affected dates or collections and customer-visible warning
    behind it.
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml`, `licence.yaml` validate; `PT-ACT` is
...
- [ ] `[fixture]` **DoD 12** — the replayed full run records non-zero `storage_bytes`,
      ...
      (PRD §39.2).
- [ ] `[machine]` `python -m <iroot>.conformance check pipelines/adapters/pt-act` exits 0 in
      **strict** mode; the committed `conformance-report.json` shows no `FAIL` and no
      `NOT_AVAILABLE` (PRD §45.4).

**PRD §44.2 `E13` exit evidence:**

- [ ] `[fixture]` **The ACT row of the eight-jurisdiction historical fixture matrix** —
      `assert_rate_matrix(pipelines/adapters/pt-act)` passes: all three PRD §6.6 financial years have a
...
- [ ] `[machine]` **Circular identity (deliverable 5)** — a parser table over recorded fixtures
      extracts identifiers into `official_identifier` and assigns distinct `document_type`s to a
      circular, guidance and a change notice; a malformed identifier quarantines and emits no record
      (`SRCH-004`).
- [ ] `[fixture]` **Evidenced supersession (deliverable 5)** — the superseding/superseded circular
      pair produces a `legal_event` with an `evidence_ref`, a deterministic `node_relation` and
      `legal_status: SUPERSEDED` on the earlier document; the implied-supersession fixture asserts
...
- [ ] `[machine]` **Resolved `legislation_ref` to Act **or** instrument (deliverable 7)** — every
      emitted `RateFact` carries a `legislation_ref` into a `LEG-ACT` node and a deterministic
      `node_relation`; a test asserts the committed mapping records each target's kind (Act provision
      vs subordinate/disallowable/notifiable instrument, PRD §40.2's `LEG-ACT` families); an
...
- [ ] `[machine]` **Subordinate typing (PRD §9.1)** — no record carries a legislation,
      ...
      before emission. A circular describing an instrument is guidance; the instrument itself is
      `LEG-ACT`'s (deliverable 5, sub-PRD D5).
- [ ] `[machine]` The adapter imports no HTTP library and no HTML/XML/PDF parsing library —
      `INGF-01`'s AST scan over `pipelines/adapters/pt-act/**` passes (PRD §37.4, SEC-002).
- [ ] `[machine]` `python -m <iroot>.registry validate pipelines/adapters/pt-act` exits 0 and a
...
- [ ] `[human]` **`fact_key` → target mapping review, including target kind** — is each rate/threshold
      linked to the right ACT Act provision **or** instrument? Choosing the wrong one changes what the
      answer says the rate rests on, and it is a legal judgment rather than a test. PRD §43.4 item 4
      puts source adapter anomalies in the founder review queue.
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (`ADM-001`; supports `SRCH-002`,
      `SRCH-003`, `SRCH-004`, `SRCH-005`, PRD §44.2 `E13`); UAT IDs — **none owned**; supplies the
      corpus precondition for `UAT-SRCH-03`; schema/API/event compatibility (uses `SINS-01`'s
...
Harness: `uv run pytest pipelines/adapters/pt-act -q` plus the conformance CLI. All replays are
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/pt-act -q`.
2. `python -m <iroot>.registry validate pipelines/adapters/pt-act` — exit 0.
3. `python -m <iroot>.conformance check pipelines/adapters/pt-act --report conformance-report.json`
...
4. **`tests/test_rate_facts.py`** — for each of the three financial-year time points, assert
   ...
   delete-one-financial-year mutation asserting `RATE_FY_COVERAGE_INCOMPLETE` at `BLOCK`.
5. **`tests/test_circulars.py`** — identifier parsing table; the superseding/superseded pair producing
   an evidenced event, relation and `SUPERSEDED` status; the implied-supersession fixture asserting
   nothing.
6. **`tests/test_legislation_link.py`** — every fact carries a resolved `legislation_ref` and a
   deterministic relation; the mapping records each target's kind; the unresolvable-reference fixture
   produces `PT_RATE_LEGISLATION_LINK_MISSING` at `BLOCK` and emits no relation.
...
8. **`tests/test_document_typing.py`** — circular/guidance/notice typing and the forbidden-type
...
**Reviewer focus.** (a) Run `tests/test_legislation_link.py` and read the committed mapping: in the
ACT the target may be an instrument rather than an Act section, and pointing a rate at the wrong kind
of source changes what the answer claims the rate rests on. (b) Run the "changed in code only"
mutation. (c) Check the implied-supersession fixture asserts nothing — an assumed supersession removes
live guidance from every future answer (PRD §9.3). (d) Confirm the recorded
...
1. **The rate's source instrument is not yet in `LEG-ACT`** — e.g. `SLEG-09` scoped its title
   allowlist to Acts and the determination is a notifiable instrument → do **not** fabricate the
   reference and do **not** ingest the instrument here (it is `LEG-ACT`'s, plan §4). Record
   `PT_RATE_LEGISLATION_LINK_MISSING`, add a `known_gaps` entry, and raise the title-allowlist gap
   against `SLEG-09` in `docs/prd/06-sources-legislation/`, noting it in
   `docs/prd/07-sources-instruments/README.md`. PRD §40.2 already puts "disallowable/notifiable
   instruments" in `LEG-ACT`'s required families, so this is a scope correction there, not new work
   here.
2. **The site has no reliable delta mechanism** → record the true `change_detection.capability`, let
   ...
   customer-visible warning), and update this module's README. PRD §12.1 requires this "rather than a
   false guarantee".
...
4. **Supersession is real but never stated in a machine-readable way** → record a `known_gaps` entry
   with `customer_visible: true` and leave the status unchanged rather than inferring it. PRD §15.2
   makes status derive from evidence; PRD §6.7 provides `STATUS_UNCONFIRMED`.
5. **A historical financial year is not published any more** → do **not** reconstruct the value from
...
7. **A percentage anomaly rule is meaningless at this collection size** → tighten it in this group's
   `conformance.yaml` with the measured baseline as justification (`AnomalyPolicy` permits tightening
   only) and record the reasoning in the group `README.md`. Never loosen a rule locally (PRD §40.9;
   plan §8 **Q9**) — a genuine need for a looser percentage is a writeback to `INGF-05`.
...
PRD §44.4's two. If the three-financial-year matrix cannot be completed for the ACT, that also puts
