...
status: draft
date: 2026-08-03
blocked_by: [SINS-01, SLEG-04]
blocks: [GOLD-08, GOLD-16]
---

# SINS-08 — `PT-VIC` payroll tax

Implements PRD §40.3 (wave-2 source group `PT-VIC`), PRD §6.3 (state and territory scope) and PRD
§40.8 (adapter Definition of Done) <`ADM-001`, `SRCH-002`> — **No ADR — the decision is already made
in PRD §40.3; this is build ticket 8 of 14 against it.**
...
`SLEG-04` — `LEG-VIC`, module `06-sources-legislation`
...
1. `PT-VIC` is a mandatory group and is attempted **in full** — never pre-selected for omission or
   reduced implementation, and never trimmed to make a release date easier. That applies to all eight
   payroll-tax jurisdictions equally.
...
**This row is the only payroll-tax row that names history explicitly** — "Current/**historical**
rates". That is a direct source-side mandate for the point-in-time obligation PRD §6.6 imposes:

> "At MVP launch, point-in-time retrieval MUST support: 2026–27; 2025–26; 2024–25."

so the three financial years are both this group's PRD §40.8 item-6 time points (sub-PRD **D8**) and
its row of the PRD §44.2 `E13` matrix.
...
**Why `blocked_by SLEG-04`.** The "applicable legislation" half of the §40.3 sentence is not optional
for a payroll-tax rate: the rate is imposed under Victorian payroll-tax legislation, which `LEG-VIC`
(`SLEG-04`, module `06-sources-legislation`) puts in the corpus. This ticket is ordered after it in
plan §5.8 and §6.2 so every emitted rate fact can carry a **resolved** `legislation_ref` to a real
Victorian legislation node. For the eight payroll-tax groups an unresolved link is a **blocking**
finding of this adapter's own `validate()` — unlike `SINS-03`/`SINS-06`, which have no register edge
(sub-PRD **N6**).

**PRD §44.2 `E13` is this group's exit evidence** — *"Eight-jurisdiction historical fixture matrix"* —
and this ticket contributes **the Victorian row** via a committed `fixtures/rate-matrix.json` that
`SINS-01`'s `rate_matrix` harness reads. A missing cell is a failure, never a blank (PRD §44.4).

**No value may be hardcoded (`SINS-01` sub-PRD D3).** `RateFact` construction fails unless the
declared value re-parses from its own quoted source span. A rate or threshold the Builder "knows" but
the committed fixture does not contain **cannot** be emitted. Victoria's regional and metropolitan
distinctions make this group particularly easy to mis-recall.

**Rulings and guidance are subordinate authority (PRD §9.1).** A revenue ruling or guidance page sits
at level 6 ("Official regulator guidance, rulings, decision summaries and impact materials"), and
*"Guidance MUST NOT silently override legislation, an operative instrument or binding authority."*
The adapter emits `document_type` + `authority_key`, never a level; `FND-10` computes the hierarchy
(sub-PRD **D5**, PRD §45.2).
...
Deliver the `PT-VIC` source adapter under `pipelines/adapters/pt-vic/**`: the per-adapter
...
Victoria payroll-tax collections, **current and historical rate and threshold facts emitted as
`SINS-01` `RateFact`s across the three PRD §6.6 financial years with a resolved `legislation_ref` into
`LEG-VIC`**, rulings and guidance captured as subordinate authority with citable nodes, a committed
`fixtures/rate-matrix.json` contributing the Victorian row of the PRD §44.2 `E13` matrix, and the
complete PRD §40.8 fixture set — such that
`python -m <iroot>.conformance check pipelines/adapters/pt-vic` exits 0 in strict mode.

## Non-goals

- **No rate model changes.** `SINS-01` owns `_shared/rates/**`; extending it is a `SINS-01` ticket
  update, never a local variant (plan §9 **R2**).
- **No Victorian legislation.** The payroll-tax Act and its instruments are `LEG-VIC` (`SLEG-04`,
...
- **No other jurisdiction.** `PT-NSW`, `PT-QLD`…`PT-NT` are `SINS-07`, `SINS-09`…`SINS-14`.
- **No employment-adjacent Victorian regulators** — WorkSafe Victoria, VEOHRC, the Labour Hire
  Authority, the Portable Long Service Authority and the Wage Inspectorate are `ADJ-VIC` (`SADJ-03`,
  module `09-sources-adjacent`).
- **No Victorian court or tribunal decisions** — `CASE-VIC` (`SCAS-07`, module `08-sources-cases`).
...
- `pipelines/adapters/pt-vic/**` — the whole group directory: `registry.yaml`, `allowlist.yaml`,
...
- Does not touch: `pipelines/adapters/pt-{nsw,qld,wa,sa,tas,act,nt}/**` — `SINS-07`, `SINS-09`…
  `SINS-14`.
- Does not touch: `pipelines/adapters/{fwc-docs,fwc-awards,fwc-agreements,fwo-guidance,ato-employment}/**`
  — `SINS-02`…`SINS-06`.
- Does not touch: `pipelines/adapters/leg-vic/**`, `adj-vic/**`, `case-vic/**`, `future-vic/**` —
  modules `06`, `09`, `08`, `10`. `leg-vic` is read-only from here.
...
`SLEG-04` have landed; all three are read-only from here. The seven sibling payroll-tax tickets run in
...
lane, so there is no cross-lane write contention. The only potentially shared path is the optional
`pyproject.toml`, which is append-only.

## Deliverables

1. **`registry.yaml`** (`INGF-07` schema) with all nine PRD §6.1 attributes: `group_id: PT-VIC`,
   `wave: 2`; `authority` = State Revenue Office Victoria with `authority_type: REVENUE_OFFICE`,
   `jurisdiction: VIC`, `official_url`; `official_endpoints` — one entry per payroll-tax collection
   actually used, each with `kind` and a PRD §40.5 `material_class` (`GUIDANCE` for guidance,
   ...
   **current and historical rates, thresholds, rulings and guidance** — with
   `financial_years: ['2024-25','2025-26','2026-27']` (PRD §6.6) or a `known_gaps` entry explaining
   why not; `initial_index_tier: T1`; `change_detection.*` **as measured**; `known_gaps` with
...
2. **`allowlist.yaml`** (`INGF-02` schema): `schemes: [https]`, the SRO Victoria host with
...
3. **`licence.yaml` + `licence-snapshots/`** via
   `python -m <iroot>.licensing capture pipelines/adapters/pt-vic`, stating all nine PRD §11.1 axes
...
4. **`adapter.py`** exposing `ADAPTER: SourceAdapter` with
   `AdapterMeta(group_id="PT-VIC", adapter_key="pt-vic", jurisdiction="VIC", …)` and all eight PRD
...
5. **Subordinate document typing (PRD §9.1).** `identify()` assigns `document_type` from a **closed
   ...
   guidance and from a change notice, and sets `authority_key` to the State Revenue Office. A
...
6. **Dated rate and threshold facts through `SINS-01`.** For every rate, threshold, and any levy or
   ...
   (`payroll-tax.vic.rate.<variant>`, `payroll-tax.vic.threshold.<variant>`, …),
   `jurisdiction: "VIC"`, the correct `RateKind` and `Unit`, `applies_from`/`applies_to` from the
   ...
   `rate_records()` — `SINS-01` deliverable 6's single mapping — so records are shape-identical across
   all eight jurisdictions. **Where the source distinguishes rates or thresholds by a declared
   employer or location attribute, use `RateValue.shape = "DIMENSIONED"` with an explicit
   `Dimension`; where it varies by payroll size, use `"BRACKETED"`.** Every value comes from the
   committed fixture text; none is supplied from model knowledge.
7. **Historical series (this row's distinguishing artifact).** The §40.3 row requires
   "Current/historical rates", so the historical listing is a first-class collection: each historical
   entry produces its own dated `RateFact` with `applies_from`/`applies_to` read from the source's own
   period labels, and the resulting `RateSeries` covers all three PRD §6.6 financial years
   contiguously.
8. **Resolved `legislation_ref`.** Every emitted `RateFact` carries a `legislation_ref` `NodeRef` into
   a `LEG-VIC` node (`SLEG-04`), and `rate_records()` therefore emits the `node_relation` of `SINS-01`
   deliverable 6 step 4 with `derivation: DETERMINISTIC` and a non-model `confidence_state`.
   Resolution follows the citation the source itself gives; where the source cites no provision, the
   reference comes from the group's committed `fact_key` → provision mapping, reviewed as part of this
...
9. **Rulings and guidance as citable nodes.** Ruling and guidance documents are emitted with their own
   headings and sections as `document_node`/`node_version` records with exact offsets, so a
   conditional or negative conclusion can cite a pinpoint (PRD §15.3; PRD §36.6's validator checks
   exact offsets).
...
11. **Fixtures (PRD §40.8 items 2, 4, 6, 7, 10).** `fixtures/discovery/`; `fixtures/dry-run.json`
    ...
    PRD §6.6 financial years as its three legal dates** (sub-PRD D8); `fixtures/quarantine/` with one
...
14. **Failure codes** with `register_failure_codes("pt-vic", …)`, each with a non-empty operator
    action (PRD §40.8 item 10, ADM-001) — at minimum: rate table not located, rate value not present
    in source, historical period label unparseable, effective date unparseable, legislation reference
    unresolved, document type would be operative, collection count anomaly.
15. **`README.md`** in the group directory: collections used, the `fact_key` namespace and the
    dimension vocabulary, the `fact_key` → Victorian provision mapping used for `legislation_ref`, the
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml`, `licence.yaml` validate; `PT-VIC` is
...
- [ ] `[fixture]` **DoD 12** — the replayed full run records non-zero `storage_bytes`,
      ...
      (PRD §39.2).
- [ ] `[machine]` `python -m <iroot>.conformance check pipelines/adapters/pt-vic` exits 0 in
      **strict** mode; the committed `conformance-report.json` shows no `FAIL` and no
      `NOT_AVAILABLE` (PRD §45.4).

**PRD §44.2 `E13` exit evidence:**

- [ ] `[fixture]` **The Victorian row of the eight-jurisdiction historical fixture matrix** —
      `assert_rate_matrix(pipelines/adapters/pt-vic)` passes: all three PRD §6.6 financial years have a
...
- [ ] `[fixture]` **Rates as dated facts (PRD §40.3)** — every emitted rate/threshold is a `SINS-01`
      ...
      the declared value; records follow `SINS-01` deliverable 6's order (deliverable 6).
- [ ] `[fixture]` **Historical series (deliverable 7)** — the historical listing fixture produces one
      dated fact per published period, with `applies_from`/`applies_to` read from the source's own
      period labels and a contiguous, non-overlapping series across the three financial years.
- [ ] `[machine]` **Dimensioned values** — where the source distinguishes a rate or threshold by a
      declared attribute, the emitted fact uses `RateValue.shape = "DIMENSIONED"` with an explicit
      `Dimension`; a test asserts the dimension names come from the group's documented vocabulary and
      that no dimensioned fact collapses to a scalar (deliverable 6).
...
- [ ] `[machine]` **Resolved `legislation_ref` (deliverable 8)** — every emitted `RateFact` carries a
      `legislation_ref` into a `LEG-VIC` node and a corresponding deterministic `node_relation`; a
      fixture whose reference cannot be resolved produces `PT_RATE_LEGISLATION_LINK_MISSING` at
      severity `BLOCK` and emits **no** relation (PRD §40.3, §9.3).
- [ ] `[machine]` **Ruling/guidance pinpoints (deliverable 9)** — ruling and guidance sections are
      individually addressable nodes whose `canonical_text[start:end]` reproduces exactly
      (PRD §15.3, §36.6).
...
- [ ] `[machine]` The adapter imports no HTTP library and no HTML/XML/PDF parsing library —
      `INGF-01`'s AST scan over `pipelines/adapters/pt-vic/**` passes (PRD §37.4, SEC-002).
- [ ] `[machine]` `python -m <iroot>.registry validate pipelines/adapters/pt-vic` exits 0 and a
...
- [ ] `[human]` **`fact_key` → provision mapping and dimension-vocabulary review** — is each
      rate/threshold linked to the right Victorian provision, and does the dimension vocabulary
      reflect the distinctions the source actually draws? A wrong link or a collapsed distinction
      produces a confidently mis-grounded answer; PRD §43.4 item 4 puts source adapter anomalies in the
...
Harness: `uv run pytest pipelines/adapters/pt-vic -q` plus the conformance CLI. All replays are
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/pt-vic -q`.
2. `python -m <iroot>.registry validate pipelines/adapters/pt-vic` — exit 0.
3. `python -m <iroot>.conformance check pipelines/adapters/pt-vic --report conformance-report.json`
...
4. **`tests/test_rate_facts.py`** — for each of the three financial-year time points, assert
   ...
   offsets slice its `quoted_text`; the "changed in code only" mutation asserting
   `RateValueNotInSourceError`; the delete-one-financial-year mutation asserting
   `RATE_FY_COVERAGE_INCOMPLETE` at `BLOCK`.
5. **`tests/test_historical_series.py`** — the historical listing fixture: one fact per published
   period, contiguous and non-overlapping across the three years, period labels parsed from the
   source.
6. **`tests/test_dimensions.py`** — the dimensioned-value cases and the documented dimension
   vocabulary.
...
9. **`tests/test_document_typing.py`** — ruling/guidance/notice typing and the forbidden-type
...
sub-PRD **D3** is the only thing between that and a cited answer. (b) Check the historical series has
no gap silently filled and no interval synthesised — PRD §15.2 makes status derive from evidence.
(c) Confirm every fact resolves to a real `LEG-VIC` node; an unresolved link must block, not warn.
...
1. **The site has no reliable delta mechanism** → record the true `change_detection.capability`, let
   ...
   requires this "rather than a false guarantee". A payroll-tax rate that silently goes stale is a
...
3. **A historical financial year is not published any more** — least likely here, since this row
   explicitly requires "Current/historical rates", but if it happens: do **not** reconstruct the value
   from memory or a third-party site (PRD §6.1: only official public sources are eligible). Confirm by
...
5. **The `SINS-01` rate model cannot express this jurisdiction's structure** → extend `_shared/rates`
   **through a `SINS-01` ticket update** (its deliverable 3), update
   ...
   variant is the failure plan §9 **R2** exists to prevent — and with eight near-identical siblings it
   would immediately become eight variants.
6. **The source cites no provision and the mapping is ambiguous** → do not guess. Record
   ...
   deliverable 8. PRD §9.3 allows deterministic extraction only "when exact source evidence and parser
   version are retained".
7. **A sibling payroll-tax adapter needs the same helper** → put it in `_shared/rates` via a
   `SINS-01` ticket update, never in this directory and never copied (plan §9 **R2**).
...
safely warning or refusing. If the three-financial-year matrix cannot be completed for Victoria, that
