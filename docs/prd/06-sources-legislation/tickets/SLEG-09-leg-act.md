...
date: 2026-08-03
blocked_by: [SLEG-01]
blocks: [SINS-13, SADJ-08, SFUT-09, GOLD-09, GOLD-16]
---

# SLEG-09 — `LEG-ACT`

Implements PRD §40.2 (`LEG-ACT` source group), PRD §6.3 (state and territory scope) and PRD §40.8
(adapter Definition of Done) <SRCH-002, SRCH-003, SRCH-005; supports ADM-001, SEC-002> — no ADR — the
decision is already made in PRD §40.2; this is build ticket 9 of 10 against it.
...
**PRD §6.3 fixes what "employment-related" means for the ACT** — the ticket's subject scope, quoted in
full because `titles.yaml` is judged against it:
...
Only the **legislation** in that list is this ticket's: ACT Acts, subordinate laws, disallowable and
notifiable instruments and their commencement/amendment/repeal history. Rates and official guidance
are `07-sources-instruments` (`PT-ACT`), regulator material is `09-sources-adjacent` (`ADJ-ACT`), and
courts/tribunals — including ACAT — are `08-sources-cases` (`CASE-ACT`). All three build on the
legislation this ticket ingests (sub-PRD **D7**).
...
**What makes the ACT distinctive.** This is the only PRD §40.2 row whose minimum capability names
**register events** and **instrument relations**, and the only one that lists **republications** and
**commencement notices** as required families. Three consequences:

1. **The register's own event stream is the evidence.** PRD §15.2: "Legal status MUST be derived from
   evidenced LegalEvents." A register event (notification, commencement, republication, disallowance)
   is parsed into a `LegalEventRecord` through `SLEG-01`'s `build_event` with a resolvable
   `evidence_node_version_id`. `SLEG-01`'s vocabulary already carries `REPUBLICATION`,
   `COMMENCEMENT`, `COMMENCEMENT_PROCLAIMED`, `DISALLOWANCE`, `AMENDMENT`, `REPEAL` and
   `AS_MADE_REGISTERED` for exactly this.
2. **Instrument relations are `node_relation`/document relations, not prose.** PRD §35.2 defines
   `node_relation` with `relation_type`, `evidence_node_version_id`, `derivation` and
   `confidence_state`, and states that "`MODEL_SUGGESTED` cannot support definitive status". The
   relation between an Act and a disallowable or notifiable instrument made under it is recorded with
   the register's own evidence, never inferred. `SRCH-005`'s requirement — "Source/version pages
   expose timeline and relationships **without generation**" — is satisfied here or not at all.
3. **Republications are the version series.** Each republication is a `DocumentVersion` with its own
   `version_label` and effect interval, and the series must satisfy PRD §35.2's non-overlap rule;
   `SLEG-01`'s `assert_no_overlap` makes an overlap a **BLOCK** (PRD §40.9).
...
easier is not permitted. `LEG-ACT` is therefore built in full; a limited state is never a way to
...
**Downstream (plan §6.2): `SLEG-09 --> SINS-13 & SADJ-08 & SFUT-09 & GOLD-09 & GOLD-16`.**
`SINS-13` (`PT-ACT`, ACT Revenue Office) adds dated rates, thresholds, circulars and guidance on top
of the payroll-tax **legislation** this ticket ingests. `SADJ-08` (`ADJ-ACT`: WorkSafe ACT, ACT Human
Rights Commission, ACT Long Service Leave Authority and responsible public-sector authorities) adds
regulator material on top of the WHS, discrimination, compensation, portable-LSL and
privacy/public-sector **legislation** this ticket ingests. `SFUT-09` (`FUTURE-ACT`) adds future-status
events. `GOLD-09` authors state/territory evaluation cases against real corpus IDs; `GOLD-16`
reconciles the roster.
...
Deliver the `LEG-ACT` source adapter under `pipelines/adapters/leg-act/**`: the group's
`registry.yaml`, `allowlist.yaml`, `licence.yaml` + immutable `licence-snapshots/`, a `titles.yaml`
subject/title allowlist over the PRD §6.3 ACT employment scope, `conformance.yaml`, and an
`adapter.py` exposing `ADAPTER: SourceAdapter` that implements all eight PRD §40.7 boundaries over the
ACT Legislation Register — discovering Acts, subordinate laws and disallowable/notifiable
instruments, emitting a non-overlapping republication version series with exact-text round-trip,
turning register events (notification, commencement notice, republication, disallowance) into
evidenced `LegalEvent`s, and recording Act↔instrument relations from the register's own evidence —
together with the recorded fixtures that make
`python -m <root>.conformance check pipelines/adapters/leg-act` exit `0` in **strict** mode with all
...
- **No shared legislation primitive.** Point-in-time resolution, status derivation, node lineage, the
  event vocabulary, the `titles.yaml` schema and the FLAG/BLOCK table are `SLEG-01`'s. A helper this
  adapter wants is added **there** as a new sibling ticket, never copied here — plan §9 **R2**.
...
- **No ACT payroll-tax rates, thresholds, circulars or guidance** — `07-sources-instruments` /
  `SINS-13` (`PT-ACT`), which is `blocked_by` this ticket. This ticket ingests the payroll-tax
  **legislation** only (sub-PRD **D7**).
- **No WorkSafe ACT, ACT Human Rights Commission or ACT Long Service Leave Authority material** —
  `09-sources-adjacent` / `SADJ-08` (`ADJ-ACT`), which is `blocked_by` this ticket.
- **No ACT court or ACAT decisions** — `08-sources-cases` / `SCAS-12` (`CASE-ACT`).
- **No Bill documents, explanatory material, drafts, consultations or the current-vs-future separation
  model** — `10-sources-future` / `SFUT-09` (`FUTURE-ACT`), which is `blocked_by` this ticket. PRD
  §40.2 lists Bills among the register's document families, but PRD §40.6 assigns future status to
  `FUTURE-ACT` and PRD §6.5 requires future material to be separated and labelled. This adapter emits
  `ENACTED_NOT_IN_FORCE` for registered material whose commencement notice is not yet effective, and
  records a **disallowance** event where the register evidences one against an instrument it has
  already ingested — PRD §15.2 requires status to follow the evidence — and stops there (sub-PRD
  **D6**).
...
- `pipelines/adapters/leg-act/**` — the whole group directory, in the layout `INGF-07` deliverable 1
  fixes and `INGF-09` deliverable 7 demonstrates:

  ```text
  pipelines/adapters/leg-act/
...
- Does not touch: `pipelines/adapters/leg-{cth,nsw,vic,qld,wa,sa,tas,nt}/**` — `SLEG-02`…`SLEG-08`,
  `SLEG-10` (same module, concurrent wave-2 lanes).
- Does not touch: `pipelines/adapters/pt-act/**` (`SINS-13`), `pipelines/adapters/adj-act/**`
  (`SADJ-08`), `pipelines/adapters/future-act/**` (`SFUT-09`), `pipelines/adapters/case-act/**`
  (`SCAS-12`), `pipelines/adapters/_shared/{rates,caselaw,future}/**` — modules `07`–`10`.
...
touched `pipelines/adapters/leg-act/**`. The only writers anywhere under `pipelines/adapters/` before
...
1. **`registry.yaml` — the Source Coverage Registry row (PRD §40.8 item 1, PRD §6.1).** Validated by
   `python -m <root>.registry validate pipelines/adapters/leg-act`. All nine PRD §6.1 attributes:
   `group_id: LEG-ACT`, `wave: 1`; the register's responsible `authority` with `jurisdiction: ACT`
   and `official_url: https://www.legislation.act.gov.au/`; `official_endpoints` — the **exact
   collections/listings/event feeds this adapter actually calls**, each with `kind` and
   `material_class` (PRD §40.1); `document_coverage.families` covering the PRD §40.2 required
   families — Acts, subordinate laws, disallowable/notifiable instruments, commencement notices,
   republications — and `financial_years` covering at least `2024-25`, `2025-26`, `2026-27` (PRD
   ...
   the **measured** capability — a register event feed raises it (PRD §12.1); `known_gaps`;
   `evaluation_subset_ref` for `GOLD-09`/`GOLD-16`.
...
3. **`licence.yaml` + `licence-snapshots/` (PRD §40.8 item 1, PRD §11.1, PRD §35.3).** Capture the
   register's terms with `python -m <root>.licensing capture pipelines/adapters/leg-act --terms-url
...
4. **`titles.yaml` — the PRD §40.2 subject/title allowlist (`SLEG-01`'s schema, sub-PRD D5/D7).**
   `subjects` covering the PRD §6.3 ACT topics quoted above. One entry per included title with
   ...
   `AMENDING | COMMENCEMENT | TRANSITIONAL | INTERPRETATION | SUBORDINATE`) — **disallowable and
   notifiable instruments made under an included Act are the canonical `SUBORDINATE` dependency
   here**, and commencement notices the canonical `COMMENCEMENT` one. `adapter.discover()` refuses a
   title with no recorded reason via `unexplained_titles()`.

5. **`adapter.py` — `ADAPTER: SourceAdapter` implementing the eight PRD §40.7 boundaries.**
   `AdapterMeta(group_id="LEG-ACT", adapter_key="leg-act", jurisdiction="ACT", authority_id=…,
   ...
   - `discover(ctx, cursor, since)` — over the `official_endpoints`, enumerating each allowlisted
     title's republications and the register events attached to it (notification, commencement notice,
     republication, disallowance), with conditional requests where supported. Yields
     `RemoteDescriptor`s with a stable `descriptor_key`; paging through `DiscoveryCursor`.
   ...
   - `identify(ctx, artifact)` — `stable_source_key` is the register's permanent identifier for the
     title or instrument, stable across republications. Never a URL, never a display title, never a
     content hash.
   ...
   - `normalise(ctx, parsed, identity)` — builds `DocumentVersionRecord` + `NodeVersionRecord`s via
     `NodeTree`/`stable_node_key`, setting `version_label` (the republication identifier),
     `publication_date`, `effective_from`/`effective_to`, `content_hash`, `official_url`,
     `retrieved_at` and `legal_status` from `derive_status`.
   - `extract_events(ctx, normalised)` — `LegalEventRecord`s built through `build_event` from the
     register's own events: `AS_MADE_REGISTERED`/`ASSENT` on notification, `COMMENCEMENT` /
     `COMMENCEMENT_PROCLAIMED` from a commencement notice, `REPUBLICATION` on each republication,
     `AMENDMENT`, `REPEAL` and `DISALLOWANCE` where evidenced. Each carries a resolvable
     `evidence_node_version_id`; `event_date` and `effective_date` stay independent (PRD §15.2,
     §35.2).
   - `extract_relations(ctx, normalised)` — `NodeRelationRecord`s from `diff_nodes` **plus the
     Act↔instrument relations the register evidences**: an instrument made under an enabling provision
     is related to that provision with `derivation="DETERMINISTIC_STRUCTURAL"` and
     `confidence_state="EVIDENCED"`, never `MODEL_SUGGESTED` (PRD §35.2; PRD §40.2 "instrument
     relations"; `SRCH-005` "expose timeline and relationships without generation").
...
6. **`fixtures/discovery/` + `fixtures/dry-run.json` (DoD item 2).** ≥1 recorded discovery response
   per `official_endpoints` entry — including the register-event listing — so replay yields ≥1
   `RemoteDescriptor` with a non-empty `descriptor_key` and an allowlisted URL. `dry-run.json` carries
   `{run_at, descriptors_discovered, sample_urls, tool_versions}` from a **one-time live** run,
   `run_at` inside `DRY_RUN_MAX_AGE_DAYS = 180`.

7. **`fixtures/documents/` (DoD items 4 and 5).** Representative artefacts for every declared media
   type, **including at least one republication, one commencement notice and one disallowable or
   notifiable instrument with its enabling provision**. Public official material only, and **no
   customer data**: the item-4 scanner rejects TFN/ABN-with-name/email/phone/credential patterns,
   `Set-Cookie` or `Authorization` captures and `.env`-shaped content. Each fixture parses through
   `INGF-06`'s host, satisfies `assert_roundtrip()`, and yields a node hierarchy with one root, no
...
8. **`fixtures/timepoints/` (DoD item 6).** ≥3 legal dates, one in each PRD §6.6 financial year
   (2024–25, 2025–26, 2026–27), for at least one title with a real republication history — an ACT
   employment, WHS or payroll-tax Act is the natural choice, since `SINS-13` and `SADJ-08` depend on
   it. For each date: a republication whose interval brackets it, a `legal_status` from PRD §6.7's
   seven values backed by an evidence event, and `event_date`/`effective_date` distinguished. No two
   republications of one series may overlap (PRD §35.2, §40.9).
...
9. **Incremental scenarios (DoD item 7).** Four recorded scenarios: **no-change** (304 → zero fetched,
   ...
   unchanged); **changed** (a new republication emitted, prior republication's `effective_to` closed
...
11. **Freshness declaration (DoD item 9).** `registry.yaml.change_detection` plus recorded runs
    ...
    genuine event feed is a critical collection unless its capability says otherwise — in which case
    **D8** applies.
...
12. **`fixtures/quarantine/` and failure codes (DoD item 10).** ≥1 deliberately defective artefact per
    ...
    non-empty operator action. **Include an instrument whose enabling provision cannot be resolved**,
    which must produce a recorded finding rather than an inferred relation (PRD §35.2). Reuse
...
13. **`tests/test_conformance.py` — the five-line file `INGF-09` deliverable 1 fixes:**
    ...
    class TestLegAct(ConformanceTestCase):
...
14. **`conformance-report.json` attached to the PR** — from
    `python -m <root>.conformance check pipelines/adapters/leg-act`, `strict: true`, exit `0`.
...
- [ ] `[machine]` **DoD 1 — registry row, allowlist, licence.** All three files exist and validate
      through `INGF-07`/`INGF-02`/`INGF-04`; `group_id` is `LEG-ACT` and is in
...
- [ ] `[machine]` **Source Coverage Registry row is complete and composable.**
      `compose_registry(mode="release")` reports no `MANDATORY_GROUP_MISSING` for `LEG-ACT`, no
...
- [ ] `[fixture]` **DoD 3 — stable identity, versions, deletion/unavailability.** `identify()` is
      deterministic across two calls and stable across two republications of the same title; different
      titles and instruments yield different `stable_source_key`s; a removed descriptor produces a
      `REMOVED` finding and deletes no prior state (PRD §40.8 item 3).
- [ ] `[fixture]` **DoD 4 — representative fixtures without customer data**, covering every declared
      media type including a republication, a commencement notice and a disallowable/notifiable
      instrument; the no-customer-data scan finds nothing (PRD §40.8 item 4, §19.2, §35.3).
...
- [ ] `[fixture]` **DoD 6 — three time points**, one in each of 2024–25, 2025–26 and 2026–27, each
      resolving to a republication whose interval brackets it with a PRD §6.7 status backed by an
      evidence event and `event_date`/`effective_date` distinguished; no two republications of a
      series overlap (PRD §40.8 item 6, §6.6, §15.2, §35.2).
...
- [ ] `[fixture]` **DoD 10 — quarantine cases and operator recovery**, including the unresolvable
      enabling-provision case; every code has a non-empty `operator_action` (PRD §40.8 item 10;
      `ADM-001`).
...
- [ ] `[fixture]` **The kit as a whole.** `python -m <root>.conformance check
      pipelines/adapters/leg-act` exits `0` with `strict: true`, report schema-valid (PRD §45.4).

**Adapter behaviour beyond the kit.**

- [ ] `[machine]` `adapter.py` exposes module-level `ADAPTER` satisfying
      `isinstance(ADAPTER, SourceAdapter)`, and `AdapterMeta.adapter_key == "leg-act" ==
...
- [ ] `[machine]` **Register events are evidenced.** Every `REPUBLICATION`, `COMMENCEMENT`,
      `DISALLOWANCE`, `AMENDMENT` and `REPEAL` event carries a resolvable `evidence_node_version_id`
      into the parsed register event or notice, with `event_date` distinct from `effective_date`
      (PRD §15.2, §35.2; PRD §40.2 "Register events").
- [ ] `[machine]` **Instrument relations are evidenced, never inferred.** Every Act↔instrument relation
      carries `derivation="DETERMINISTIC_STRUCTURAL"` and `confidence_state="EVIDENCED"`; the string
      `MODEL_SUGGESTED` appears nowhere in this directory, and an instrument whose enabling provision
      cannot be resolved emits **no** relation (PRD §35.2 "`MODEL_SUGGESTED` cannot support definitive
      status"; `SRCH-005` "without generation"; PRD §40.2 "instrument relations").
...
- [ ] `[machine]` **Status is evidenced.** Every emitted version's `legal_status` traces to a
      `LegalEvent` with a resolvable `evidence_node_version_id`, or is `STATUS_UNCONFIRMED`
      (PRD §15.2, §35.2; `SRCH-005`).
- [ ] `[machine]` **Hard-filter fields are populated.** Every emitted `DocumentVersion` carries a
      ...
      the `ACT` jurisdiction through its `source`/`authority` (`SRCH-002`; PRD §30.2, §36.2).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (`SRCH-002`, `SRCH-003`,
      ...
      group's republication history makes possible, executed by `14-search-product`; schema/API/event
      ...
      `MANDATORY_GROUP_MISSING` for `LEG-ACT`, the correct honest state); known gaps and follow-up ids
      (`SINS-13`, `SADJ-08`, `SFUT-09`, `GOLD-09`, `GOLD-16`; sub-PRD **L3**, **L5**, **L7**, **L8**).
- [ ] `[human]` **Founder review of the coverage claim** — that `titles.yaml` genuinely covers the
      PRD §6.3 ACT topics (including the payroll-tax, WHS, discrimination, compensation, portable-LSL
      and privacy/public-sector legislation that `SINS-13` and `SADJ-08` will build on), that each
      inclusion reason is defensible, and that `known_gaps` states the limitations a customer would
      need to see (PRD §6.1, §41.3 step 1, §43.4 item 4; sub-PRD **L5**).
- [ ] `[human]` **Instrument-relation review** — spot-check one Act↔instrument relation against the
      register: the enabling provision and the instrument must genuinely correspond. `SRCH-005`
      renders these relations to customers "without generation", so a wrong relation is a wrong legal
      statement, and only a human can judge correspondence.
...
- [ ] `[human]` **Writeback obligation as an acceptance item** — if measured evidence shows the
      register cannot supply the PRD §40.2 minimum capability (register events, versions, instrument
      relations), the PR sets the limited `adapter_status`, adds the customer-visible gap, fills
...
Harness: `pytest`, run as `uv run pytest pipelines/adapters/leg-act -q`, plus the kit CLI. Everything
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/leg-act -q` — all green.
...
3. **The kit CLI** — `python -m <root>.conformance check pipelines/adapters/leg-act --report
...
4. **Register events** — `tests/test_register_events.py` asserts each event fixture produces the right
   `LegislationEventType` with a resolvable evidence node and distinct `event_date`/`effective_date`,
   and that a republication both emits a `REPUBLICATION` event and closes the prior version's
   `effective_to`.
5. **Instrument relations** — `tests/test_instrument_relations.py` asserts an instrument made under an
   included Act yields a relation with `confidence_state="EVIDENCED"` pointing at the enabling
   provision's node version; an instrument whose enabling provision is absent yields **no** relation
   and a recorded finding; a source scan asserts `MODEL_SUGGESTED` appears nowhere in the directory.
6. **Registry composition** — `python -m <root>.registry validate pipelines/adapters/leg-act` exits
   `0`; compose a fixture tree in `mode="release"` and assert no `MANDATORY_GROUP_MISSING` for
   `LEG-ACT` and that the five PRD §12.1 dates appear as five separate fields.
7. **Licence** — `python -m <root>.licensing check pipelines/adapters/leg-act` exits `0`; snapshot
   hash matches; `INGF-04`'s gate returns the expected `LicenceDecision` for each of the six
   `IntendedUse` values.
8. **Adapter unit tests** — `tests/test_identity.py` (determinism, stability across republications),
...
`DEFERRED` is legitimate only for item 11; (b) **the instrument relations** — confirm each names its
evidence and that none is inferred, because `SRCH-005` shows these to customers without generation;
(c) confirm every register event names its evidence and keeps notification and effect dates apart
(PRD §15.2); (d) confirm the republication series does not overlap and each financial-year date
resolves to exactly one version (PRD §35.2, §6.6); (e) confirm `adapter_status` matches what the
fixtures prove (PRD §7, §12.1, §44.4); (f) confirm the fixtures contain no cookies, tokens or personal
data and that nothing opens a socket or a parser directly (PRD §37.4, `SEC-002`).
...
(CLAUDE.md, issue #53). Five tickets are `blocked_by` this one (`SINS-13`, `SADJ-08`, `SFUT-09`,
...
1. **The register does not provide the PRD §40.2 minimum capability** — no machine-readable register
   events, no republication history, or no instrument relations → **writeback**, never a silent
...
2. **An instrument's enabling provision cannot be resolved deterministically** → emit **no** relation
   and record the finding. PRD §35.2 forbids `MODEL_SUGGESTED` from supporting definitive status, and
   `SRCH-005` renders relations to customers without generation; a guessed relation is a wrong legal
   statement shown as fact. If the gap is systematic, record it in `known_gaps` with
   `customer_visible: true`.
3. **A shared helper is missing from `_shared/legislation`** — for example a needed event type is not
   in `LegislationEventType` → do **not** define a local enum and do **not** copy `SLEG-01`'s code.
   Plan §9 **R2**: add a new sibling ticket in this module, recorded in
...
6. **Republications do not cover the PRD §6.6 window** → sub-PRD **D10**: emit what the register
   publishes and record a `DATE_LIMITED` `known_gaps` entry with `customer_visible: true`. **Never**
   reconstruct a consolidation — PRD §6.1 admits only official public sources. Overturning D10
   requires `docs/adr/NNNN-no-synthesised-consolidations.md` plus the D10/L6 rows in the sub-PRD,
   escalated first.
7. **Disallowance tracking pulls toward bills and future status** → record a `DISALLOWANCE` event only
   where the register evidences it against material already ingested here. Bills, drafts,
   consultations and the current-vs-future separation model are `SFUT-09`'s (sub-PRD **D6**;
   PRD §40.6, §6.5).
8. **A title `SINS-13` or `SADJ-08` needs is not in `titles.yaml`** → add it here with its inclusion
   reason; that is this ticket's scope under sub-PRD **D7**.

**Escalation rule.** If the twelve-item Definition of Done cannot be met for `LEG-ACT`, stop and
