...
date: 2026-08-03
blocked_by: [SLEG-01]
blocks: [SINS-10, SADJ-05, SFUT-06, GOLD-09, GOLD-16]
---

# SLEG-06 — `LEG-WA`

Implements PRD §40.2 (`LEG-WA` source group), PRD §6.3 (state and territory scope) and PRD §40.8
(adapter Definition of Done) <SRCH-002, SRCH-003, SRCH-005; supports ADM-001, SEC-002> — no ADR — the
decision is already made in PRD §40.2; this is build ticket 6 of 10 against it.
...
**PRD §6.3 fixes what "employment-related" means for Western Australia** — the ticket's subject scope,
...
Only the **legislation** in that list is this ticket's: WA Acts, subsidiary legislation and their
commencement/amendment/repeal history. Rates and official guidance are `07-sources-instruments`
(`PT-WA`), regulator material is `09-sources-adjacent` (`ADJ-WA`), and courts/tribunals — including
the WAIRC — are `08-sources-cases` (`CASE-WA`). All three build on the legislation this ticket
ingests (sub-PRD **D7**).
...
**What makes Western Australia distinctive.** The PRD §40.2 row is the only one that names **stable
identity** in the minimum-capability column, and the only one that qualifies history with **"where
official"**. Three consequences:

1. **DoD item 3 is the load-bearing check for this group.** `identify()` must be deterministic and
   stable across versions, and `stable_source_key` must be the register's own permanent identifier —
   never a URL, never a display title, never a content hash. PRD §35.2 makes
   `(source_id, stable_source_key)` unique on `legal_document`, and PRD §40.9 makes "any duplicate
   stable identity" a **BLOCK**. `SRCH-005`'s evidence is "Historical stable link survives later
   release", which is exactly this.
2. **"Historical versions where official" is a scope boundary, not an aspiration.** Where the register
   does not publish an official historical version, none is invented — sub-PRD **D10**: no synthesised
   consolidations, because PRD §6.1 admits "Only official public sources". The correct outcome is a
   `DATE_LIMITED` `known_gaps` entry with `customer_visible: true`.
3. **Gazettes are commencement evidence.** PRD §40.2 lists them as a required family for this group,
   and PRD §15.2 requires status to be derived from evidenced events — so a gazette notice is a parse
   target that produces a `COMMENCEMENT`/`COMMENCEMENT_PROCLAIMED` `LegalEvent` with a resolvable
   `evidence_node_version_id`, not a link in a footnote.
...
easier is not permitted. `LEG-WA` is therefore built in full; a limited state is never a way to
...
**Downstream (plan §6.2): `SLEG-06 --> SINS-10 & SADJ-05 & SFUT-06 & GOLD-09 & GOLD-16`.**
`SINS-10` (`PT-WA`, RevenueWA) adds dated rates, thresholds, the employer guide and rulings on top of
the payroll-tax **legislation** this ticket ingests. `SADJ-05` (`ADJ-WA`: WorkSafe WA, Equal
Opportunity Commission, WorkCover WA, MyLeave and responsible labour-relations/public-sector
authorities — PRD §40.5 adds "only regimes actually applicable") adds regulator material on top of the
WHS, discrimination, compensation and portable-LSL **legislation** this ticket ingests. `SFUT-06`
(`FUTURE-WA`) adds future-status events. `GOLD-09` authors state/territory evaluation cases against
...
Deliver the `LEG-WA` source adapter under `pipelines/adapters/leg-wa/**`: the group's `registry.yaml`,
`allowlist.yaml`, `licence.yaml` + immutable `licence-snapshots/`, a `titles.yaml` subject/title
allowlist over the PRD §6.3 WA employment scope, `conformance.yaml`, and an `adapter.py` exposing
`ADAPTER: SourceAdapter` that implements all eight PRD §40.7 boundaries over WA Legislation —
discovering Acts, subsidiary legislation and officially published historical versions, deriving a
**provably stable** `stable_source_key` from the register's own permanent identifier, parsing HTML and
PDF through `INGF-06`'s isolated host with exact-text round-trip, and emitting evidenced `LegalEvent`s
including gazette-backed commencements — together with the recorded fixtures that make
`python -m <root>.conformance check pipelines/adapters/leg-wa` exit `0` in **strict** mode with all
...
- **No WA payroll-tax rates, thresholds, employer guide or rulings** — `07-sources-instruments` /
  `SINS-10` (`PT-WA`, RevenueWA), which is `blocked_by` this ticket. This ticket ingests the
  payroll-tax **legislation** only (sub-PRD **D7**).
- **No WorkSafe WA, Equal Opportunity Commission, WorkCover WA or MyLeave material** —
  `09-sources-adjacent` / `SADJ-05` (`ADJ-WA`), which is `blocked_by` this ticket.
- **No WAIRC decisions, orders, awards or agreements, and no WA court judgments** —
  `08-sources-cases` / `SCAS-09` (`CASE-WA`), whose PRD §40.4 row covers "WAIRC decisions/orders/
  awards/agreements".
- **No Bills, draft instruments, consultations or the current-vs-future separation model** —
  `10-sources-future` / `SFUT-06` (`FUTURE-WA`), which is `blocked_by` this ticket. PRD §40.2 lists
  Bills among the register's document families, but PRD §40.6 assigns future status to `FUTURE-WA` and
...
- `pipelines/adapters/leg-wa/**` — the whole group directory, in the layout `INGF-07` deliverable 1
  fixes and `INGF-09` deliverable 7 demonstrates:

  ```text
  pipelines/adapters/leg-wa/
...
- Does not touch: `pipelines/adapters/leg-{cth,nsw,vic,qld,sa,tas,act,nt}/**` — `SLEG-02`…`SLEG-05`,
  `SLEG-07`…`SLEG-10` (same module, concurrent wave-2 lanes).
- Does not touch: `pipelines/adapters/pt-wa/**` (`SINS-10`), `pipelines/adapters/adj-wa/**`
  (`SADJ-05`), `pipelines/adapters/future-wa/**` (`SFUT-06`), `pipelines/adapters/case-wa/**`
  (`SCAS-09`), `pipelines/adapters/_shared/{rates,caselaw,future}/**` — modules `07`–`10`.
...
touched `pipelines/adapters/leg-wa/**`. The only writers anywhere under `pipelines/adapters/` before
...
1. **`registry.yaml` — the Source Coverage Registry row (PRD §40.8 item 1, PRD §6.1).** Validated by
   `python -m <root>.registry validate pipelines/adapters/leg-wa`. All nine PRD §6.1 attributes:
   `group_id: LEG-WA`, `wave: 1`; the register's responsible `authority` with `jurisdiction: WA` and
   `official_url: https://www.legislation.wa.gov.au/`; `official_endpoints` — the **exact
   collections/listings this adapter actually calls**, each with `kind` and `material_class` (PRD
   §40.1); `document_coverage.families` covering the PRD §40.2 required families — Acts, subsidiary
   legislation, gazettes and officially published historical versions — and `financial_years` covering
   at least `2024-25`, `2025-26`, `2026-27` (PRD §6.6) or a `customer_visible: true` gap;
   `licence_ref`; `allowlist_ref`; `adapter_status` — `ACTIVE`, or one of PRD §7's four limited
   ...
   **measured** capability (PRD §12.1); `known_gaps` — **including any period for which the register
   publishes no official historical version**, marked `DATE_LIMITED` and `customer_visible: true`
   (sub-PRD **D10**); `evaluation_subset_ref` for `GOLD-09`/`GOLD-16`.
...
2. **`allowlist.yaml` (PRD §40.8 item 1, `SEC-002`, PRD §37.4).** `schemes: [https]` only; the
   ...
   `approved_max_bytes_reason` (PDF Acts and gazettes can exceed `INGF-02`'s 50 MiB default; PRD §37.4
   allows only a documented "source-specific approved limit"). The adapter reaches the network only
   through `ctx.fetcher`; `ReplayFetcher` refuses a fixture URL outside this file.

3. **`licence.yaml` + `licence-snapshots/` (PRD §40.8 item 1, PRD §11.1, PRD §35.3).** Capture the
   register's terms with `python -m <root>.licensing capture pipelines/adapters/leg-wa --terms-url
...
4. **`titles.yaml` — the PRD §40.2 subject/title allowlist (`SLEG-01`'s schema, sub-PRD D5/D7).**
   `subjects` covering the PRD §6.3 WA topics quoted above. One entry per included title with
   ...
   refuses a title with no recorded reason via `unexplained_titles()`. Where PRD §40.5's "only regimes
   actually applicable" means a WA regime has no equivalent legislation, the absence is recorded in
   `known_gaps`, not left implicit.

5. **`adapter.py` — `ADAPTER: SourceAdapter` implementing the eight PRD §40.7 boundaries.**
   `AdapterMeta(group_id="LEG-WA", adapter_key="leg-wa", jurisdiction="WA", authority_id=…,
   adapter_version="0.1.0", supported_content_types=[…HTML and PDF media types…],
   declared_quarantine_reasons=[… including `IDENTITY_CONFLICT`, `IDENTITY_DUPLICATE`,
   `PARSE_FAILED`, `OCR_DEFECT` …])`.
   - `discover(ctx, cursor, since)` — over the `official_endpoints`, enumerating each allowlisted
     title's officially published version history and the gazette notices that evidence its
   ...
   - `identify(ctx, artifact)` — **the load-bearing boundary for this group.**
     `stable_source_key` is the register's own permanent identifier for the title. A documented
     derivation rule lives in the module docstring, and the rule must be stable under: a changed
     display title, a changed URL, a new version of the same title, and a re-fetch of identical
     content. Never a URL, never a display title, never a content hash.
   - `parse(ctx, artifact)` — through `ctx.parser` (`INGF-06`) only, for HTML and PDF; `ParsedBlock`
     offsets satisfy the exact-text round-trip.
   ...
   - `extract_events(ctx, normalised)` — `LegalEventRecord`s for commencement (**with the gazette
     ...
     `effective_date` (commencement) stay independent — PRD §15.2 distinguishes publication from
     effective time.
   ...
   - `validate(ctx, candidate, prior)` — returns `legislation_findings(...)`, which makes a duplicate
     stable identity a **BLOCK** (PRD §40.9).
...
7. **`fixtures/documents/` (DoD items 4 and 5).** Representative artefacts for every declared media
   type — HTML and PDF — **including at least one gazette notice**. Public official material only, and
   **no customer data**: the item-4 scanner rejects TFN/ABN-with-name/email/phone/credential patterns,
...
8. **`fixtures/identity/` and `fixtures/timepoints/` (DoD items 3 and 6).**
   - `fixtures/identity/` — the same title captured twice with a **changed display title** and a
     **changed URL**, plus two genuinely different titles, plus two versions of one title. These are
     the inputs that prove `stable_source_key` stability and non-collision (PRD §40.8 item 3;
     `SRCH-005`'s "Historical stable link survives later release").
   - `fixtures/timepoints/` — ≥3 legal dates, one in each PRD §6.6 financial year (2024–25, 2025–26,
     2026–27), for at least one title with official version history — a WA employment, WHS or
     payroll-tax Act is the natural choice, since `SINS-10` and `SADJ-05` depend on it. For each date:
...
12. **`fixtures/quarantine/` and failure codes (DoD item 10).** ≥1 deliberately defective artefact per
    ...
    non-empty operator action. **Include a duplicate-stable-identity fixture** mapping to
    `IDENTITY_DUPLICATE` (PRD §40.9's critical identity failure). Reuse `SLEG-01`'s `legislation`
...
13. **`tests/test_conformance.py` — the five-line file `INGF-09` deliverable 1 fixes:**
    ...
    class TestLegWa(ConformanceTestCase):
...
14. **`conformance-report.json` attached to the PR** — from
    `python -m <root>.conformance check pipelines/adapters/leg-wa`, `strict: true`, exit `0`.
...
- [ ] `[machine]` **DoD 1 — registry row, allowlist, licence.** All three files exist and validate
      through `INGF-07`/`INGF-02`/`INGF-04`; `group_id` is `LEG-WA` and is in
...
- [ ] `[machine]` **Source Coverage Registry row is complete and composable.**
      `compose_registry(mode="release")` reports no `MANDATORY_GROUP_MISSING` for `LEG-WA`, no
...
- [ ] `[fixture]` **DoD 3 — stable identity, versions, deletion/unavailability.** **The load-bearing
      check for this group.** Using `fixtures/identity/`: `identify()` returns the same
      `stable_source_key` across two calls, across a changed display title, across a changed URL and
      across two versions of one title; two genuinely different titles never collide; a removed
      descriptor produces a `REMOVED` finding and deletes no prior state (PRD §40.8 item 3, §35.2,
      §40.9; `SRCH-005` "Historical stable link survives later release").
- [ ] `[fixture]` **DoD 4 — representative fixtures without customer data**, covering every declared
      media type including a gazette notice; the no-customer-data scan finds nothing
      (PRD §40.8 item 4, §19.2, §35.3).
- [ ] `[fixture]` **DoD 5 — parser/node hierarchy and exact-text round-trip** for every fixture in
      both formats; one root, no cycles, contiguous sibling ordinals, recomputable `text_hash`es
...
- [ ] `[fixture]` **DoD 6 — three time points**, one in each of 2024–25, 2025–26 and 2026–27, each
      ...
      event and `event_date`/`effective_date` distinguished; no two consolidated versions overlap
...
- [ ] `[fixture]` **DoD 10 — quarantine cases and operator recovery**, including the
      duplicate-stable-identity fixture mapping to `IDENTITY_DUPLICATE`; every code has a non-empty
      `operator_action` (PRD §40.8 item 10, §40.9; `ADM-001`).
...
- [ ] `[fixture]` **The kit as a whole.** `python -m <root>.conformance check
      pipelines/adapters/leg-wa` exits `0` with `strict: true`, report schema-valid (PRD §45.4).

**Adapter behaviour beyond the kit.**

- [ ] `[machine]` `adapter.py` exposes module-level `ADAPTER` satisfying
      `isinstance(ADAPTER, SourceAdapter)`, and `AdapterMeta.adapter_key == "leg-wa" ==
...
- [ ] `[machine]` **Architecture**: the scanner reports this directory clean — no `httpx`, `requests`,
      ...
      import; the network is reached only via `ctx.fetcher` and parsing only via `ctx.parser`
...
- [ ] `[machine]` **Gazette evidence.** Every `COMMENCEMENT`/`COMMENCEMENT_PROCLAIMED` event emitted
      from a gazette fixture carries a resolvable `evidence_node_version_id` into that gazette's
      parsed nodes, with `event_date` (gazettal) distinct from `effective_date` (commencement)
      (PRD §15.2, §35.2; PRD §40.2 "gazettes").
...
- [ ] `[machine]` **Hard-filter fields are populated.** Every emitted `DocumentVersion` carries a
      ...
      the `WA` jurisdiction through its `source`/`authority` (`SRCH-002`; PRD §30.2, §36.2).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (`SRCH-002`, `SRCH-003`,
      ...
      group's official version history makes possible, executed by `14-search-product`;
      schema/API/event compatibility (none — fills existing schemas); tenant/PII/security impact (none
      — public official material; fixtures scanned by DoD item 4); **source/licence impact**
      (assessment, `max_quote_chars`, and any `DATE_LIMITED` gap where the register publishes no
      official historical version); cost/memory/latency impact (DoD item 12 against PRD §39.2's 2 GiB
      budget); rollback path (delete the group directory; `compose_registry` then reports
      `MANDATORY_GROUP_MISSING` for `LEG-WA`, the correct honest state); known gaps and follow-up ids
      (`SINS-10`, `SADJ-05`, `SFUT-06`, `GOLD-09`, `GOLD-16`; sub-PRD **L3**, **L5**, **L7**, **L8**).
- [ ] `[human]` **Founder review of the coverage claim** — that `titles.yaml` genuinely covers the
      PRD §6.3 WA topics (including the payroll-tax, WHS, discrimination, compensation and
      portable-LSL legislation that `SINS-10` and `SADJ-05` will build on), that each inclusion reason
      is defensible, and that `known_gaps` states the limitations a customer would need to see —
      particularly any period with no official historical version (PRD §6.1, §41.3 step 1, §43.4
      item 4; sub-PRD **L5**, **D10**).
- [ ] `[human]` **Stable-identity review** — confirm the documented `stable_source_key` derivation is
      the register's own permanent identifier and would survive a site redesign. `SRCH-005`'s
      acceptance evidence is "Historical stable link survives later release", and `GOLD-09`'s gold
      authorities are written against these keys; a later change invalidates them.
...
- [ ] `[human]` **Writeback obligation as an acceptance item** — if measured evidence shows the
      register cannot supply the PRD §40.2 minimum capability (discovery, stable identity,
      PDF/history), the PR sets the limited `adapter_status`, adds the customer-visible gap, fills
...
Harness: `pytest`, run as `uv run pytest pipelines/adapters/leg-wa -q`, plus the kit CLI. Everything
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/leg-wa -q` — all green.
...
3. **The kit CLI** — `python -m <root>.conformance check pipelines/adapters/leg-wa --report
...
4. **Identity suite** — `tests/test_identity.py` over `fixtures/identity/`: same key across two calls,
   a changed display title, a changed URL and two versions; different keys for different titles; a
   deliberately duplicated key produces a **BLOCK** `IDENTITY_DUPLICATE` finding.
5. **Registry composition** — `python -m <root>.registry validate pipelines/adapters/leg-wa` exits
   `0`; compose a fixture tree in `mode="release"` and assert no `MANDATORY_GROUP_MISSING` for
   `LEG-WA` and that the five PRD §12.1 dates appear as five separate fields.
6. **Licence** — `python -m <root>.licensing check pipelines/adapters/leg-wa` exits `0`; snapshot hash
...
8. **Adapter unit tests** — `tests/test_normalise.py` (node tree, round-trip in both formats),
   `tests/test_timepoints.py` (the three financial-year dates, non-overlap).
...
`DEFERRED` is legitimate only for item 11; (b) **the identity suite** — this is the group the PRD
singles out for stable identity, and `GOLD-09`'s gold authorities depend on these keys; (c) confirm
gazette-backed commencements resolve to evidence and keep gazettal and commencement dates apart
(PRD §15.2); (d) confirm `adapter_status` and any `DATE_LIMITED` gap match what the fixtures prove —
"historical versions **where official**" is a boundary, not a target (PRD §7, §6.1, sub-PRD **D10**);
(e) confirm the fixtures contain no cookies, tokens or personal data; (f) confirm nothing in the
directory opens a socket or a parser directly (PRD §37.4, `SEC-002`).
...
(CLAUDE.md, issue #53). Five tickets are `blocked_by` this one (`SINS-10`, `SADJ-05`, `SFUT-06`,
...
1. **The register does not provide the PRD §40.2 minimum capability** — no structured discovery, no
   stable identifier, or no official history → **writeback**, never a silent downgrade. Set
...
2. **The register has no permanent identifier and only URLs distinguish titles** → do **not** silently
   fall back to a URL or a content hash; that would make `SRCH-005`'s "Historical stable link survives
   later release" false and break `GOLD-09`'s gold authorities on the first site change. Document the
   best available derivation, record the fragility in `known_gaps` with `customer_visible: true`, and
   raise it in `docs/prd/06-sources-legislation/README.md` — a group whose identity cannot be
   stabilised is a candidate for a PRD §7 limited status under plan §8 **Q10**'s confirmed policy —
   which admits that status only with the measured evidence, affected scope and customer-visible
   warning in a `limitation` block, and never as a reason to drop the group.
3. **The register publishes no official historical version for part of the PRD §6.6 window** ("where
   official") → sub-PRD **D10**: emit what the register publishes and record a `DATE_LIMITED`
...
7. **A title `SINS-10` or `SADJ-05` needs is not in `titles.yaml`** → add it here with its inclusion
...
**Escalation rule.** If the twelve-item Definition of Done cannot be met for `LEG-WA`, stop and
