...
Implements PRD §40.4 (wave 3 roster, the `CASE-TAS` row), PRD §6.3/§6.4 (state scope; case law and
...
build ticket 11 of 13 against it.
...
**The row names three publisher classes and no URL at all** — Tasmanian courts, TASCAT, and the
industrial collections. PRD §40.4's closing paragraph therefore makes the decomposition this ticket's
first and largest deliverable:
...
discovered at the official publishers' own sites during the recording run, verified there, and
recorded in `registry.yaml` and `allowlist.yaml`. Multiple publishers are expressed inside one
`registry.yaml` (sub-PRD **Q9**).

**"operative instruments" is explicit in this row.** PRD §9.1 ranks decisions and instruments *"with
operative effect"* (rank 4) above ordinary persuasive decisions (rank 5), and PRD §36.2's eligibility
predicate begins *"requested date ∈ effective interval"* — so an operative instrument carries a
structured effective interval and a PRD §6.7 `legal_status`, never prose.

**Relevance is a recorded criterion, not a hidden filter** (PRD §40.2's analogous rule for
legislation; PRD §44.4). PRD §6.3 requires, for Tasmania, *"employment and industrial-relations
...
**Downstream.** Plan §6.2: `SCAS-11 --> GOLD-16`.
...
initial defaults, tightenable after a representative baseline), **Q9**. Small jurisdictions publish
less and less consistently; PRD §7's explicit limited states exist precisely for this case, and using
one honestly — with the measured evidence **D15** requires — is a success, not a failure. It is never
a licence to reduce this group's scope.

## Goal

Deliver `pipelines/adapters/case-tas/` as a complete PRD §40.8-conforming source group: a
`registry.yaml` that **decomposes the Tasmanian court, TASCAT and industrial collections into exact
official endpoints**, distinguishing decisions from operative instruments and recording the
employment-relevance criterion; the URL allowlist; licence snapshot(s) and assessment; an `adapter.py`
implementing PRD §40.7's eight boundaries over `SCAS-01`'s primitives with structured effective
intervals on instruments; recorded offline fixtures for discovery, documents, three time points,
incremental change and quarantine; a count/hash baseline; and the five-line `ConformanceTestCase`
subclass — such that `python -m <root>.conformance check pipelines/adapters/case-tas` exits `0` in
strict mode with a committed `conformance-report.json`, and `CASE-TAS` composes into the Source
...
- **No Tasmanian legislation** — `06-sources-legislation` / `SLEG-08` (`LEG-TAS`). An instrument made
  under an Act is this group's; the Act is not.
- **No Tasmanian regulator, WHS, equal-opportunity, compensation or portable-LSL material** —
  `09-sources-adjacent` / `SADJ-07` (`ADJ-TAS`).
- **No other decision group** — `SCAS-02`…`SCAS-10`, `SCAS-12`, `SCAS-13` (sub-PRD **D9**).
...
- `pipelines/adapters/case-tas/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: any sibling `pipelines/adapters/case-*/**` — `SCAS-02`…`SCAS-10`, `SCAS-12`,
  `SCAS-13`.
- Does not touch: `pipelines/adapters/leg-tas/**` (`SLEG-08`), `adj-tas/**` (`SADJ-07`), `pt-tas/**`
  (`SINS-12`).
...
and none of them writes. Modules `06`, `07` and `09` tickets covering Tasmania write their own group
...
1. **`registry.yaml` — the collection decomposition (DoD item 1; the ticket's primary deliverable).**
   `group_id: CASE-TAS`, `wave: 3`, `initial_index_tier: T2` (the row's value; `CRPS-04` owns final
   tiering). `official_endpoints[]` enumerates the **exact** official collections of the Tasmanian
   courts, TASCAT and the industrial collections, each with `kind` and `material_class` — decisions are
   `DECISION`; operative instruments are `OPERATIVE_INSTRUMENT`. `document_coverage` records the
   employment-relevance criterion and separates the decision and instrument families;
   ...
   *"solely because it is older than three financial years"*. `change_detection` states the real
   observed capability per collection — which in a small jurisdiction may honestly be `NONE`, deriving
   `FRESHNESS_LIMITED` (PRD §12.1). `evaluation_subset_ref` names the evaluation ids.
...
2. **`allowlist.yaml` (DoD item 1; `INGF-02`'s schema).** Only official Tasmanian publisher hosts
   verified during the recording run. No aggregator, no commercial reporter (**D13**) — the temptation
   is highest in the smallest jurisdictions, where an aggregator's archive is more complete than the
   official one; PRD §6.1 and §40.4 forbid it regardless.
...
4. **`adapter.py` — `ADAPTER: SourceAdapter`** (`INGF-01` deliverables 4 and 9) implementing all eight
   ...
   court/tribunal or instrument identifier, else URL path with `weak=True`; small jurisdictions
   frequently lack neutral citations, so the `weak` path and its `CASE_IDENTITY_WEAK` finding matter
   here), `parse` (`ctx.parser`), `normalise` (`build_judgment_nodes()` for decisions; the same node
   machinery with `document_type = OPERATIVE_INSTRUMENT` plus `effective_from`/`effective_to` and a
   PRD §6.7 `legal_status` for instruments), `extract_events` (`case_event()` with evidence spans or a
   named official field), `extract_relations` (`resolve_citations()` → `CITES`; `assert_treatment()`
   only with a resolvable evidence span), `validate` (`case_validation()` plus the instrument-interval
   and relevance-basis rules). `AdapterMeta` declares `group_id="CASE-TAS"`, `adapter_key="case-tas"`,
   `jurisdiction="TAS"`.
5. **Operative instruments are modelled, not flattened.** Structured effective intervals and PRD §6.7
   statuses; an overlapping consolidated series is a `BLOCK` finding (PRD §40.9, §35.2); no
   determinable effective date → quarantine, never a default to publication date.
6. **Court-code registration.** `register_court_codes("CASE-TAS", {...})` with `CourtFacts` for every
   Tasmanian court/tribunal code ingested, each with `authority_type`, the contract `court_level` and
...
8. **Recorded fixtures (DoD items 2, 4, 6, 7, 10) under `fixtures/`.** Captured from the official
   ...
   including at least one court decision, one TASCAT decision, **one operative instrument**, and one
   decision with in-text citations), `timepoints/` (≥3 legal dates, one inside and one outside the
   instrument's interval), `incremental/` (304, changed, removed, transient 5xx), `quarantine/`
   (≥1 per declared reason code, including a document with no determinable identity), `baseline.json`.
...
11. **`tests/`** — the five-line conformance subclass plus group-specific unit tests: per-publisher
    ...
    weak-identity handling, relevance-basis recording, court-code/level mapping, paragraph round-trip,
    citation offsets, treatment evidence, identity stability.
12. **`README.md`** — which collections were decomposed and why, the decision/instrument split, what is
    out of scope (Tasmanian legislation → `SLEG-08`; regulator material → `SADJ-07`), the licence
    position per publisher, the observed change-detection capability, and every `known_gaps` entry.
    Read by `GOLD-16` and the Founder at Gate 2 (PRD §41.3 step 1).
13. **`conformance-report.json`** committed at the group root — the PRD §45.4 evidence artifact.

## Acceptance checklist (classified)

The twelve PRD §40.8 items, in order, each proved for `CASE-TAS`:

- [ ] `[fixture]` **DoD 1 — registry row, URL allowlist, licence snapshot/assessment.** All three files
      validate; `group_id` is in `MANDATORY_SOURCE_GROUPS`; directory name is `case-tas`; each
...
- [ ] `[machine]` **PRD §40.4 decomposition.** `official_endpoints[]` lists exact collections for the
      courts, TASCAT and the industrial collections with a `material_class` each; no bare homepage; no
...
- [ ] `[fixture]` **DoD 3 — stable identity and deletion/unavailability.** Deterministic and stable
      across two recorded versions; no collisions; a document without a neutral citation still gets a
      stable key with `weak=True` and a `CASE_IDENTITY_WEAK` finding; a removed descriptor yields
      `REMOVED`, retains the prior version and deletes no state (PRD §40.8 item 3).
...
- [ ] `[fixture]` **DoD 6 — three time points.** The instrument resolves correctly inside and outside
      its interval; decisions resolve at their decision date; PRD §6.7 statuses are used; events
      distinguish `event_date` from `effective_date`; no overlapping intervals (PRD §40.8 item 6,
      §15.2, §35.2, §36.2, §40.9).
...
- [ ] `[machine]` **DoD 9 — freshness schedule with last-check/last-ingest separation.** A replayed 304
      run and a replayed content run write different fields; if the declared capability is `NONE`, the
      composed `freshness_status` is `FRESHNESS_LIMITED` regardless of recency (PRD §40.8 item 9,
      §12.1).
...
- [ ] `[machine]` `python -m <root>.conformance check pipelines/adapters/case-tas` exits `0` in
      **strict** mode; the committed report shows `summary.pass == 12` (or item 11 `DEFERRED`) and
      `"strict": true` (PRD §45.4).
- [ ] `[machine]` `INGF-07`'s composer includes `CASE-TAS` with all nine PRD §6.1 attributes, a
...
- [ ] `[machine]` The adapter imports no HTTP or parser library, no corpus/app database module, and
      nothing from `pipelines/adapters/_shared/legislation/**` (sub-PRD **Q7**) — `INGF-01`
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**SRCH-004**, **SRCH-005**,
      **ADM-001**; supports **SRCH-002**/**SRCH-003**) and UAT IDs (**none** directly; the instrument
      time-point behaviour is the property `UAT-SRCH-03` exercises for legislation); schema/API/event
      compatibility (none, unless **Q7**/**Q9** forced a change, which must be linked);
      tenant/PII/security impact (public official text; **D14**); **source/licence impact** (one
      ...
      directory — the registry then fails with `MANDATORY_GROUP_MISSING`); known gaps (every entry,
      verbatim).
- [ ] `[human]` **Founder review of the decomposition and coverage claim** — whether the recorded
      collections genuinely cover *"Relevant published decisions and operative instruments"* for
      Tasmania, and — because this is a small jurisdiction where official publication may genuinely be
      partial — whether the limited status and gap entries are honest rather than optimistic
      (PRD §40.4, §7, §43.4 items 4–5, §44.4).
- **No further `[fixture]` classes** beyond the recorded transcripts above, and **no additional
  `[human]` criteria**. Declared explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/case-tas/tests -q`, fully offline, replaying only committed
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/case-tas/tests -q`.
2. `python -m <root>.conformance check pipelines/adapters/case-tas` — strict, exit `0`, report
...
4. **`test_instruments.py`** — interval and PRD §6.7 status; resolution at three dates; overlapping
   series → `BLOCK`; no determinable effective date → quarantine.
5. **`test_identity_weak.py`** — a decision without a neutral citation gets a stable key, `weak=True`
   and the `CASE_IDENTITY_WEAK` finding; two such decisions never collide.
...
7. **`test_metadata.py`** — court/tribunal, level, date, file number, citation; unregistered code
   quarantines.
8. **`test_paragraphs.py`** — round-trip for decisions and instruments; `para/NNNN` keys; `[N]` labels;
...
10. **`test_incremental.py`** — the four DoD item 7 scenarios via `ReplayFetcher`; the
...
Reviewer focus: (a) the decomposition is real for all three publisher classes — this row names no URL,
so a homepage entry is the likely shortcut and is not acceptable; (b) `allowlist.yaml` is
official-only even where an aggregator would be more complete (**D13**); (c) the weak-identity path is
implemented rather than silently dropping citation-less decisions; (d) instruments carry structured
intervals; (e) fixtures are recorded transcripts (**D12**).
...
1. **Official publication is genuinely partial** (a class or historical range simply is not published)
   → `registry.yaml` records `SOURCE_UNAVAILABLE` or date-limited coverage with a
   `customer_visible: true` gap entry, exactly as PRD §40.4 directs, and the shortfall is recorded in
   `docs/prd/08-sources-cases/README.md`. This is the **correct** outcome, not a failure: PRD §7
   provides the vocabulary and PRD §44.4 forbids the alternative.
2. **An aggregator has a more complete archive than the official publisher** → it is still not a
   source (PRD §6.1, §40.4; **D13**). Record the gap; escalate through `GOLD-16` → `LNCH-05` if the
   coverage shortfall is material.
3. **Operative instruments need `_shared/legislation`'s point-in-time machinery** (sub-PRD **Q7**) →
   **do not import it**: no `blocked_by` edge orders `SLEG-01` before this ticket. The writeback is a
   plan change — add `SLEG-01` to this ticket's `blocked_by` in `docs/prd/breakdown-plan.md` §5.9 and
   the matching edge in §6.2, update `docs/prd/08-sources-cases/README.md` **Q7** and this ticket's
   frontmatter, re-publish, then implement.
4. **`INGF-07`'s schema cannot express three authorities in one group** (sub-PRD **Q9**) → keep one
   file per group and express the multiplicity inside it; if the schema must change, open a docs PR
   against
...
5. **No reliable delta mechanism** → declare `capability: NONE` honestly; `INGF-07` derives
...
8. **Decisions carry no neutral citation at all** → use the weak-identity path with its finding; do not
   invent a citation, and do not import one from an aggregator (which would attribute a third party's
   citation scheme to an official source). If the whole collection lacks citations, record it as a
   `known_gaps` entry — `SRCH-004`'s exact-match behaviour depends on citations existing, and pretending
   otherwise would misstate the product's capability.
9. **`SCAS-01` lacks a primitive this jurisdiction needs** → update
