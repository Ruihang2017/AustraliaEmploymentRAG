...
Implements PRD §40.4 (wave 3 roster, the `CASE-NT` row), PRD §6.3/§6.4 (territory scope; case law and
...
discovered at the NT courts' and NTCAT's own sites during the recording run, verified there, and
...
legislation; PRD §44.4). PRD §6.3 requires, for the Northern Territory, *"employment and
industrial-relations legislation and guidance … and relevant regulators, courts and tribunals"*;
PRD §6.4 requires *"Relevant state and territory courts and tribunals"*. NTCAT hears a broad range of
matters, so the criterion is what keeps the corpus honest in both directions — nothing irrelevant,
...
**Downstream.** Plan §6.2: `SCAS-13 --> GOLD-16` — this ticket, with its eleven siblings, is part of
what makes `GOLD-16`'s claim *"Every mandatory group is ACTIVE or explicitly limited — never silently
omitted"* true or false for wave 3.
...
(multi-authority groups in one `registry.yaml`). The NT is the smallest jurisdiction in the roster and
may publish partially; PRD §7's explicit limited states exist for exactly that case, and using one
honestly — with the measured evidence **D15** requires — is a success, not a failure. It is never a
licence to reduce this group's scope.

## Goal

Deliver `pipelines/adapters/case-nt/` as a complete PRD §40.8-conforming source group: a
`registry.yaml` that **decomposes the NT court and NTCAT collections into exact official endpoints**
...
`python -m <root>.conformance check pipelines/adapters/case-nt` exits `0` in strict mode with a
committed `conformance-report.json`, and `CASE-NT` composes into the Source Coverage Registry as
...
- **No NT legislation** — `06-sources-legislation` / `SLEG-10` (`LEG-NT`).
- **No NT regulator, WorkSafe, Anti-Discrimination Commission or portable-LSL material** —
  `09-sources-adjacent` / `SADJ-09` (`ADJ-NT`).
- **No other decision group** — `SCAS-02`…`SCAS-12` (sub-PRD **D9**).
...
- `pipelines/adapters/case-nt/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: any sibling `pipelines/adapters/case-*/**` — `SCAS-02`…`SCAS-12`.
- Does not touch: `pipelines/adapters/leg-nt/**` (`SLEG-10`), `adj-nt/**` (`SADJ-09`), `pt-nt/**`
  (`SINS-14`).
...
and none of them writes. Modules `06`, `07` and `09` tickets covering the NT write their own group
...
1. **`registry.yaml` — the collection decomposition (DoD item 1; the ticket's primary deliverable).**
   `group_id: CASE-NT`, `wave: 3`, `initial_index_tier: T2` (the row's value; `CRPS-04` owns final
   tiering). `official_endpoints[]` enumerates the **exact** official collections of the NT courts and
   of NTCAT, each with `kind` and `material_class` (`DECISION`; `OPERATIVE_INSTRUMENT` for anything
   published with operative effect). `document_coverage` records the employment-relevance criterion —
   for NTCAT, the specific lists/divisions that hear employment-related matters — and
   ...
   *"solely because it is older than three financial years"*. `change_detection` states the real
   observed capability per collection — which may honestly be `NONE`, deriving `FRESHNESS_LIMITED`
   (PRD §12.1). `evaluation_subset_ref` names the evaluation ids.
...
2. **`allowlist.yaml` (DoD item 1; `INGF-02`'s schema).** Only official NT court and NTCAT publisher
   hosts verified during the recording run. No aggregator, no commercial reporter (**D13**) — the
   temptation is highest in the smallest jurisdiction, where an aggregator's archive may be more
   complete than the official one; PRD §6.1 and §40.4 forbid it regardless.
...
4. **`adapter.py` — `ADAPTER: SourceAdapter`** (`INGF-01` deliverables 4 and 9) implementing all eight
   ...
   court/tribunal file number, else URL path with `weak=True`; a small jurisdiction may publish without
   neutral citations, so the weak path and its `CASE_IDENTITY_WEAK` finding matter here), `parse`
   (`ctx.parser` for every declared media type), `normalise` (`build_judgment_nodes()` — root, parts,
   one node per numbered paragraph `para/NNNN`, display `[N]`), `extract_events` (`case_event()` with
   evidence spans or a named official field), `extract_relations` (`resolve_citations()` → `CITES`;
   `assert_treatment()` only with a resolvable evidence span), `validate` (`case_validation()` plus the
   relevance-basis rule). `AdapterMeta` declares `group_id="CASE-NT"`, `adapter_key="case-nt"`,
   `jurisdiction="NT"`.
5. **Court-code registration.** `register_court_codes("CASE-NT", {...})` with `CourtFacts` for every NT
   court/tribunal neutral-citation code ingested (including appellate levels where the source
   distinguishes them), each with `authority_type`, the contract `court_level` and `is_appellate`. An
...
7. **Recorded fixtures (DoD items 2, 4, 6, 7, 10) under `fixtures/`.** Captured from the official
   ...
   including at least one NT court judgment, one NTCAT decision from an employment-relevant list, and
   one decision with in-text citations), `timepoints/` (≥3 legal dates), `incremental/` (304, changed,
   removed, transient 5xx), `quarantine/` (≥1 per declared reason code, including a document with no
   determinable identity), `baseline.json`.
...
10. **`tests/`** — the five-line conformance subclass plus group-specific unit tests: per-publisher
    discovery, relevance-basis recording (including an out-of-scope NTCAT list excluded by the recorded
    criterion), weak-identity handling, court-code/level mapping, paragraph round-trip, citation
    offsets, treatment evidence, identity stability.
11. **`README.md`** — which collections were decomposed and why, which NTCAT lists are in scope and
    which are not, what is out of scope entirely (NT legislation → `SLEG-10`; regulator material →
    `SADJ-09`), the licence position per publisher, the observed change-detection capability, and every
...
The twelve PRD §40.8 items, in order, each proved for `CASE-NT`:

- [ ] `[fixture]` **DoD 1 — registry row, URL allowlist, licence snapshot/assessment.** All three files
      validate; `group_id` is in `MANDATORY_SOURCE_GROUPS`; directory name is `case-nt`; each
...
- [ ] `[machine]` **PRD §40.4 decomposition.** `official_endpoints[]` lists exact collections for the
      NT courts and NTCAT with a `material_class` each; no bare homepage; no aggregator host in
...
- [ ] `[fixture]` **DoD 3 — stable identity and deletion/unavailability.** Deterministic and stable
      across two recorded versions; no collisions; a decision without a neutral citation still gets a
      stable key with `weak=True` and a `CASE_IDENTITY_WEAK` finding; a removed descriptor yields
      `REMOVED`, retains the prior version and deletes no state (PRD §40.8 item 3; **D14**).
...
- [ ] `[machine]` `python -m <root>.conformance check pipelines/adapters/case-nt` exits `0` in
      **strict** mode; the committed report shows `summary.pass == 12` (or item 11 `DEFERRED`) and
      `"strict": true` (PRD §45.4).
- [ ] `[machine]` `INGF-07`'s composer includes `CASE-NT` with all nine PRD §6.1 attributes, a
...
- [ ] `[machine]` **Relevance basis is evidenced.** Every ingested decision records the criterion that
      put it in scope, and an out-of-scope NTCAT list is excluded by that recorded criterion rather
      than by undocumented code (deliverable 6; PRD §44.4).
...
- [ ] `[human]` **Founder review of the decomposition, the NTCAT list selection and the coverage
      claim** — whether the recorded collections genuinely cover *"Relevant published
      judgments/decisions"* for NT employment matters, and — because this is the smallest jurisdiction
      and official publication may genuinely be partial — whether the limited status and gap entries are
      honest rather than optimistic (PRD §40.4, §7, §43.4 items 4–5, §44.4).
- **No further `[fixture]` classes** beyond the recorded transcripts above, and **no additional
  `[human]` criteria**. Declared explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/case-nt/tests -q`, fully offline, replaying only committed
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/case-nt/tests -q`.
2. `python -m <root>.conformance check pipelines/adapters/case-nt` — strict, exit `0`, report
...
4. **`test_relevance.py`** — an in-scope NTCAT list decision is ingested with its basis; an out-of-scope
...
6. **`test_metadata.py`** — court/tribunal, level, date, file number, citation; unregistered code
   quarantines.
7. **`test_paragraphs.py`** — round-trip, `para/NNNN` keys, `[N]` labels, contiguous ordinals, and the
...
9. **`test_incremental.py`** — the four DoD item 7 scenarios via `ReplayFetcher`; the
...
homepage entry is the likely shortcut and is not acceptable; (b) `allowlist.yaml` is official-only even
where an aggregator would be more complete (**D13**); (c) the weak-identity path is implemented rather
than silently dropping citation-less decisions; (d) fixtures are recorded transcripts (**D12**); (e) the
registry status and gap entries match what the fixtures actually prove — this is the last of the twelve
wave-3 groups and `GOLD-16`'s roster claim is only as honest as the weakest row.
...
1. **Official publication is genuinely partial** (a class or historical range is not published) →
...
2. **An aggregator has a more complete archive than the official publisher** → it is still not a source
   (PRD §6.1, §40.4; **D13**). Record the gap and escalate through `GOLD-16` → `LNCH-05` if the
   shortfall is material to NT coverage claims.
3. **NTCAT publishes no list/division field to filter on** → record the criterion actually available,
   state the residual uncertainty as a `known_gaps` entry with `customer_visible: true`, and note it in
   `docs/prd/08-sources-cases/README.md`. Do not guess from prose.
4. **`INGF-07`'s schema cannot express two authorities in one group** (sub-PRD **Q9**) → keep one file
...
8. **Decisions carry no neutral citation** → use the weak-identity path with its finding; do not invent
   a citation and do not import one from an aggregator. If the whole collection lacks citations, record
   it as a `known_gaps` entry — `SRCH-004`'s exact-match behaviour depends on citations existing.
