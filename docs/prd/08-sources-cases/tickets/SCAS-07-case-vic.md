...
Implements PRD §40.4 (wave 3 roster, the `CASE-VIC` row), PRD §6.3/§6.4 (state scope; case law and
...
discovered at the official courts' and tribunals' own sites during the recording run, verified there,
and recorded in `registry.yaml` and `allowlist.yaml`.

**Four court/tribunal classes are named:** Supreme, County, VCAT and industrial-related published
decisions. Each is a separate official collection with its own listing, formats and possibly its own
terms, expressed **inside one `registry.yaml`** (sub-PRD **Q9**). PRD §6.3 requires, for Victoria,
*"employment and industrial-relations legislation and guidance … and relevant regulators, courts and
tribunals"*; PRD §6.4 requires *"Relevant state and territory courts and tribunals"*.

**Relevance is a recorded criterion, not a hidden filter.** The row says *"Relevant … published
decisions"*; PRD §40.2's analogous rule for legislation is *"A maintained subject/title allowlist plus
dependency expansion records why each title is included"*, and PRD §44.4 forbids implying coverage
that does not exist. The employment-relevance criterion is written into `registry.yaml` and every
ingested decision records the basis on which it qualified.
...
**Downstream.** Plan §6.2: `SCAS-07 --> GOLD-16` (*"Every mandatory group is ACTIVE or explicitly
limited — never silently omitted"*).
...
Deliver `pipelines/adapters/case-vic/` as a complete PRD §40.8-conforming source group: a
`registry.yaml` that **decomposes the Victorian Supreme, County, VCAT and industrial-related judgment
collections into exact official endpoints** with a recorded employment-relevance criterion, the URL
allowlist, licence snapshot(s) and assessment, an `adapter.py` implementing PRD §40.7's eight
boundaries over `SCAS-01`'s primitives (neutral citations for the courts and tribunals actually
ingested, stable identity, paragraph-exact nodes, court/tribunal facts, evidenced `CITES`/treatment
relations), recorded offline fixtures for discovery, documents, three time points, incremental change
and quarantine, a count/hash baseline, and the five-line `ConformanceTestCase` subclass — such that
`python -m <root>.conformance check pipelines/adapters/case-vic` exits `0` in strict mode with a
committed `conformance-report.json`, and `CASE-VIC` composes into the Source Coverage Registry as
...
- **No Victorian legislation** — `06-sources-legislation` / `SLEG-04` (`LEG-VIC`).
- **No Victorian regulator, OHS, labour-hire, portable-LSL or wage-inspectorate material** —
  `09-sources-adjacent` / `SADJ-03` (`ADJ-VIC`). This group is decisions.
- **No other decision group** — `SCAS-02`…`SCAS-06`, `SCAS-08`…`SCAS-13`. A relation whose `from_ref`
...
- `pipelines/adapters/case-vic/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: any sibling `pipelines/adapters/case-*/**` — `SCAS-02`…`SCAS-06`, `SCAS-08`…`SCAS-13`.
- Does not touch: `pipelines/adapters/leg-vic/**` (`SLEG-04`), `adj-vic/**` (`SADJ-03`), `pt-vic/**`
  (`SINS-08`) — other modules' Victorian material.
...
and none of them writes. Tickets in modules `06`, `07` and `09` covering Victoria write their own group
directories and never this one. The single shared path is `pipelines/adapters/pyproject.toml`,
append-only by plan §1.1.

## Deliverables

1. **`registry.yaml` — the collection decomposition (DoD item 1; the ticket's primary deliverable).**
   `group_id: CASE-VIC`, `wave: 3`, `initial_index_tier: T2` (the row's value; `CRPS-04` owns final
   tiering). `official_endpoints[]` enumerates the **exact** official collections of the Supreme Court,
   County Court, VCAT and the industrial-related publishers, each with `kind` and `material_class`
   (decisions `DECISION`; any operative instrument `OPERATIVE_INSTRUMENT`). The multi-authority shape
   is expressed inside this one file (sub-PRD **Q9**). `document_coverage` records the
   employment-relevance criterion (court/list/division, subject or listing category);
   ...
   *"solely because it is older than three financial years"*. `change_detection` states the real
   observed capability and cadence per collection; `evaluation_subset_ref` names the evaluation ids.
...
2. **`allowlist.yaml` (DoD item 1; `INGF-02`'s schema).** Only official Victorian court/tribunal
   publisher hosts verified during the recording run. No aggregator, no commercial reporter (**D13**).
3. **`licence.yaml` + `licence-snapshots/<date>-<hash>.<ext>` (DoD item 1; `INGF-04`).** Terms captured
   ...
   making `adapter_status` `METADATA_AND_LINK_ACTIVE` with a customer-visible gap. Where publishers'
   terms differ, the **more restrictive** assessment governs material whose provenance is ambiguous.
   Any limited status set here carries deliverable 1's `limitation` block (`reason_code: LICENSING_RESTRICTION`, with the licence
...
4. **`adapter.py` — `ADAPTER: SourceAdapter`** (`INGF-01` deliverables 4 and 9) implementing all eight
   ...
   declared media type — Victorian collections publish both HTML and PDF judgments), `normalise`
   (`build_judgment_nodes()` — root, parts, one node per numbered paragraph `para/NNNN`, display
   `[N]`), `extract_events` (`case_event()` with evidence spans or a named official field),
   `extract_relations` (`resolve_citations()` → `CITES`; `assert_treatment()` only with a resolvable
   evidence span), `validate` (`case_validation()` plus the relevance-basis rule).
   `AdapterMeta` declares `group_id="CASE-VIC"`, `adapter_key="case-vic"`, `jurisdiction="VIC"`.
5. **Court-code registration.** `register_court_codes("CASE-VIC", {...})` with `CourtFacts` for every
   Victorian court/tribunal neutral-citation code this group ingests, each with `authority_type`, the
   contract `court_level` and `is_appellate` (a Court of Appeal judgment is not a first-instance one).
   An unregistered code quarantines — a wrong court level is a wrong authority level (PRD §9.2
   bullet 1, §36.3 item 3).
6. **Relevance basis recorded per document.** Every ingested decision stores the criterion that put it
   in scope, and the run report summarises the counts. A decision with no determinable basis fails the
   run rather than being included or dropped silently (PRD §44.4).
7. **Recorded fixtures (DoD items 2, 4, 6, 7, 10) under `fixtures/`.** Captured from the official
   ...
   (≥1 per collection), `dry-run.json` (≤180 days old), `documents/` (one per declared media type,
   including at least one court judgment, one VCAT decision, and one decision with in-text citations),
   `timepoints/` (≥3 legal dates), `incremental/` (304, changed, removed, transient 5xx),
   `quarantine/` (≥1 per declared reason code), `baseline.json` (per-collection counts and hash set).
...
10. **`tests/`** — the five-line conformance subclass plus group-specific unit tests: per-collection
    discovery, relevance-basis recording, court-code/level mapping, paragraph round-trip on both HTML
    and PDF sources, citation offsets, treatment evidence, identity stability.
11. **`README.md`** — which collections were decomposed and why, what is out of scope (Victorian
    legislation → `SLEG-04`; regulator material → `SADJ-03`), the licence position per publisher, the
    observed change-detection capability, and every `known_gaps` entry. Read by `GOLD-16` and the
    Founder at Gate 2 (PRD §41.3 step 1).
12. **`conformance-report.json`** committed at the group root — the PRD §45.4 evidence artifact.

## Acceptance checklist (classified)

The twelve PRD §40.8 items, in order, each proved for `CASE-VIC`:

- [ ] `[fixture]` **DoD 1 — registry row, URL allowlist, licence snapshot/assessment.** All three files
      validate; `group_id` is in `MANDATORY_SOURCE_GROUPS`; directory name is `case-vic`; each
...
- [ ] `[machine]` **PRD §40.4 decomposition.** `official_endpoints[]` lists exact collections for the
      Supreme, County, VCAT and industrial-related publishers with a `material_class` each; no entry is
      a bare homepage standing in for a collection; no aggregator host appears in `allowlist.yaml`
      (PRD §40.4 closing paragraph, §6.1; **D13**).
- [ ] `[fixture]` **DoD 2 — discovery fixture and live dry-run evidence.** Replaying `discovery/`
      yields ≥1 `RemoteDescriptor` per collection with an allowlisted URL; `dry-run.json` present,
...
- [ ] `[fixture]` **DoD 5 — parser/node hierarchy and exact-text round-trip.** `assert_roundtrip()`
      passes for every fixture **including the PDF one**; one root, no cycles, contiguous sibling
...
- [ ] `[machine]` `python -m <root>.conformance check pipelines/adapters/case-vic` exits `0` in
      **strict** mode; the committed report shows `summary.pass == 12` (or item 11 `DEFERRED`) and
      `"strict": true` (PRD §45.4).
- [ ] `[machine]` `INGF-07`'s composer includes `CASE-VIC` with all nine PRD §6.1 attributes, a
...
- [ ] `[human]` **Founder review of the decomposition and coverage claim** — whether the recorded
      collections genuinely cover *"Relevant Supreme/County/VCAT/industrial-related published
      decisions"* for Victorian employment matters, and whether every shortfall is a customer-visible
      limitation rather than silence (PRD §40.4, §43.4 items 4–5, §44.4).
- **No further `[fixture]` classes** beyond the recorded transcripts above, and **no additional
  `[human]` criteria**. Declared explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/case-vic/tests -q`, fully offline, replaying only committed
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/case-vic/tests -q`.
2. `python -m <root>.conformance check pipelines/adapters/case-vic` — strict, exit `0`, report
...
3. **`test_decomposition.py`** — every `official_endpoints` entry has a `material_class` and passes the
   allowlist; all four named classes are represented; no host outside the official set appears.
...
6. **`test_paragraphs.py`** — round-trip on HTML **and** PDF fixtures, `para/NNNN` keys, `[N]` labels,
   contiguous ordinals, and the duplicate-number `BLOCK` finding.
...
8. **`test_incremental.py`** — the four DoD item 7 scenarios via `ReplayFetcher`.
...
Reviewer focus: (a) the decomposition is real — exact collections for each named court/tribunal class,
not a homepage; (b) the PDF round-trip is byte-exact (Victorian collections publish authoritative PDFs,
and PRD §15.3's citation offsets depend on it); (c) `allowlist.yaml` contains only official Victorian
hosts (**D13**); (d) fixtures are recorded transcripts, not hand-written HTML (**D12**).
...
1. **`INGF-07`'s `registry.yaml` schema cannot express several authorities in one group** (sub-PRD
   **Q9**) → keep **one file per group** and express the multiplicity inside it, exactly as `INGF-07`'s
   ...
   **Q9**, and re-publish. Never introduce a cross-group shared file (plan §2.1 **A2**).
2. **A named collection is not published in a machine-retrievable form, or a class/date range is
   ...
   exactly as PRD §40.4 directs, recorded in `docs/prd/08-sources-cases/README.md`. PRD §44.4 forbids
   silently calling the category covered; substituting an aggregator is never the answer (**D13**).
3. **No reliable delta mechanism** → set `change_detection.capability` honestly; `INGF-07` derives
   `FRESHNESS_LIMITED`; add a customer-visible gap entry and record it in
   `docs/prd/08-sources-cases/README.md` (PRD §12.1: *"rather than a false guarantee"*).
4. **Licensing is unclear, restrictive, or differs between collections** → `METADATA_AND_LINK_ONLY`
   (PRD §11.1) → `METADATA_AND_LINK_ACTIVE` or `LICENSING_RESTRICTED` with a customer-visible gap,
   recorded per publisher in `docs/prd/08-sources-cases/README.md`. Never ingest full text "pending
   clarification".
...
6. **PDF paragraph extraction is unreliable for a collection** → quarantine the affected artifacts with
   a declared reason code and record the class as a `known_gaps` entry rather than emitting nodes whose
   offsets do not round-trip. PRD §15.3 makes exact offsets a citation precondition; a wrong offset is
   a wrong citation, which `EVID-05` will reject at answer time anyway.
