...
status: draft
date: 2026-08-03
blocked_by: [SFUT-01, SLEG-08]
blocks: [GOLD-16]
---

# SFUT-08 — `FUTURE-TAS`

Implements PRD §40.6 (wave 5 — `FUTURE-TAS`), PRD §6.5 (future and proposed law) and PRD §40.8
(adapter Definition of Done) &lt;SRCH-002, ADM-001&gt; — no ADR — the decision is already made in PRD
§40.6; this is build ticket 8 of 10 against it.
...
and `SLEG-08` — `LEG-TAS`, module `06-sources-legislation`, file-scope
`pipelines/adapters/leg-tas/**` (`docs/prd/06-sources-legislation/tickets/`).
...
**The register anchor.** PRD §40.2's `LEG-TAS` row names Tasmanian Legislation Online —
<https://www.legislation.tas.gov.au/> — with families *"Acts/statutory rules, point-in-time,
as-made/uncommenced, Bills links"* and capability *"Point-in-time extraction; events"*. That register
is `SLEG-08`'s. PRD §40.6 adds *"Parliament of Tasmania and official consultations"*. **PRD §40.1
...
formats, counts, date bounds, schedules and gaps."* The PRD names no parliament or consultation URL;
the Builder discovers the exact official endpoints and records them in `registry.yaml` — an entry
name is not an endpoint.

**Jurisdiction-specific note — "Bills links" and "as-made/uncommenced".** The `LEG-TAS` row is the
only §40.2 row that says *"Bills **links**"* rather than "Bills": the register points at bill texts
rather than necessarily hosting them, so this group's bill documents are expected to come from the
Parliament of Tasmania endpoint. If the link target is not machine-discoverable — no feed, no stable
listing — that is a real limitation and must be declared, not worked around: use
`change_detection.capability: NONE` (→ `FRESHNESS_LIMITED`) or `SOURCE_UNAVAILABLE` for that
collection, with a `known_gaps` entry marked `customer_visible: true` (PRD §7, §12.1).

The same row names *"as-made/uncommenced"* material on the register. Uncommenced provisions are the
`ENACTED_NOT_IN_FORCE` evidence for this jurisdiction — and, exactly as in Queensland, the register's
uncommenced annotation is **evidence for a `FUTURE-TAS` status event**, while the as-made Act itself
remains `LEG-TAS`'s document (sub-PRD **D1**).

**The ownership cut (sub-PRD D1) — read this before writing `identify()`.** `LEG-TAS` (`SLEG-08`)
owns Acts and statutory rules, point-in-time versions and as-made/uncommenced register material.
`FUTURE-TAS` owns the **pre-enactment pipeline**: bill texts and prints, explanatory material,
...
must **not** emit a `document_version` for an Act or statutory rule; `SFUT-01`'s separation validator
enforces that mechanically (check `S2`). Plan §5.7's `SLEG-08` goal is *"Tasmanian point-in-time
extraction and events"*; plan §5.11's goal for this ticket is *"Tasmanian future-status events"*. If
the merged `leg-tas` adapter already emits bill documents, stop and follow *Feedback obligation*
item 4 — sub-PRD open question **F4**.
...
No mandatory source group is pre-selected for omission or reduced implementation; `FUTURE-TAS` must be
...
Implement the `FUTURE-TAS` source adapter under `pipelines/adapters/future-tas/**`: a
`registry.yaml` row decomposing "Tasmanian Legislation, Parliament of Tasmania and official
consultations" into exact official collections — declaring honestly where the register only *links*
to bills — an `allowlist.yaml`, a licence snapshot and assessment per authority, and an `adapter.py`
exposing `ADAPTER: SourceAdapter` that implements PRD §40.7's eight boundaries over Tasmanian bills,
explanatory material, exposure drafts/consultations and proclamation/commencement notices — emitting
only proposal documents with PRD §6.7 non-current statuses derived from evidenced events via
`_shared/future`, with recorded fixtures sufficient for all twelve PRD §40.8 DoD items to pass
offline through `INGF-09`'s conformance kit and for `SFUT-01`'s separation suite to prove zero
current-law eligibility at three legal dates.

## Non-goals

- **No Acts, statutory rules, point-in-time versions or as-made register documents** — `SLEG-08`
  (`leg-tas`), sub-PRD **D1**.
...
- **No screen-scraping around a missing bill listing.** If the official link target cannot be
  discovered reliably, declare the limitation (see *Feedback obligation* item 1); do not crawl an
  unofficial mirror (PRD §6.1: *"Only official public sources are eligible"*).
...
- `pipelines/adapters/future-tas/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: `pipelines/adapters/future-{cth,nsw,vic,qld,wa,sa,act,nt}/**` — the eight sibling
  `SFUT` tickets.
...
**Serial safety.** First decomposition; nothing merged, nothing in flight. `SFUT-01` and `SLEG-08`
...
1. **`registry.yaml`** (schema and validator: `INGF-07`) with `group_id: FUTURE-TAS`, `wave: 5`, the
   authority list (the Tasmanian legislation-register authority, the Parliament of Tasmania and the
   ...
   documents and proclamation/commencement notices; `financial_years` covers at least PRD §6.6's three
   or a `known_gaps` entry says why not. `initial_index_tier: T3` (sub-PRD **D8**).
   `change_detection` per sub-PRD **D9** — **declare honestly** if the bill collection is only
   reachable through register links. `evaluation_subset_ref` non-empty (F8). `adapter_status` is
   `ACTIVE` or a PRD §7 limited state with a customer-visible gap (PRD §40.1).
...
2. **`allowlist.yaml`** (schema: `INGF-02`) — exact official hosts/path prefixes for the register and
   the Parliament, **excluding** any third-party-submission repository and any non-official mirror
   (sub-PRD **D6**; check `S8`; PRD §6.1).
...
4. **`adapter.py`** exposing module-level `ADAPTER: SourceAdapter` (`INGF-01` deliverable 9),
   implementing PRD §40.7's eight boundaries: `discover` (register listings plus the Parliament's bill
   collection, filtered to the PRD §6.3 employment scope through `_shared/legislation`'s title
   allowlist; a register "Bills link" is followed only to an allowlisted official host), `fetch`
   (through `ctx.fetcher` with conditional-request validators), `identify` (deterministic identity
   stable across bill prints, never colliding with a `leg-tas` identity), `parse` (through
   `ctx.parser`), `normalise` (`document_type` from `PROPOSAL_DOCUMENT_TYPES`, `legal_status` from
   `derive_future_status()`), `extract_events` (`INTRODUCED`, `PASSED`, `ASSENTED`,
   `PROCLAMATION_MADE`, `COMMENCEMENT`, `DRAFT_PUBLISHED`, `CONSULTATION_OPENED/CLOSED`, `LAPSED`,
   `WITHDRAWN`, `REPEAL_SCHEDULED`, each with an `evidence_ref` into this document's own parsed text —
   including the register's uncommenced annotation as commencement evidence), `extract_relations`
...
6. **Discovery fixtures** + `fixtures/dry-run.json` — DoD item 2, including one recorded
   register→Parliament link-following transcript.
7. **Identity/deletion rules** — DoD item 3, including `REMOVED` behaviour that retains prior
   versions, and the **link-rot case**: a register link whose target 404s produces `UNAVAILABLE`, not
   a deletion.
...
10. **Three time points** — DoD item 6 — spanning **(a)** before introduction, **(b)** after assent
    and before commencement (the register's "uncommenced" state), **(c)** after the evidenced
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml` and `licence.yaml` exist and validate;
      `FUTURE-TAS` is in `MANDATORY_SOURCE_GROUPS`; directory name `future-tas`; licence-snapshot
...
- [ ] `[fixture]` **DoD 2** — recorded discovery replays through `discover()` yielding ≥1
      `RemoteDescriptor` with a non-empty `descriptor_key` and an allowlisted URL, including one
      register→Parliament link-follow; `dry-run.json` present and within `DRY_RUN_MAX_AGE_DAYS`.
- [ ] `[fixture]` **DoD 3** — `identify()` deterministic across two calls and two prints; distinct
      documents distinct keys; `REMOVED` deletes no prior state; **a dead register link yields
      `UNAVAILABLE`, not a deletion**.
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/future-tas` exits 0 in strict
...
- [ ] `[machine]` **An "uncommenced" register annotation produces a status event with an
      `evidence_ref`, not an as-made document version** (jurisdiction note; sub-PRD D1).
...
- [ ] `[machine]` A commencement expressed as a day to be fixed by proclamation produces
      `resolved_date: null` and status `ENACTED_NOT_IN_FORCE` (check `S4`; sub-PRD D4; PRD §43.3).
...
- [ ] `[machine]` The allowlist admits no third-party submission repository, no non-official mirror,
      and no record's `provenance.official_url` matches one (check `S8`; sub-PRD D6; PRD §6.1, §10.1).
- [ ] `[machine]` `assert_no_cross_adapter_import()` passes — `_shared/**` and the ingestion framework
      only, never `pipelines/adapters/leg-tas` or a sibling group (sub-PRD D7).
- [ ] `[machine]` `python -m <root>.registry validate pipelines/adapters/future-tas` exits 0 and
...
- [ ] `[machine]` No HTTP, document-parsing or database import — `INGF-01`'s architecture scan is
      green over `pipelines/adapters/future-tas/**` (PRD §37.4, §39.1, §40.7; SEC-002).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**SRCH-002** data-side,
      **ADM-001** registry row); UAT IDs (`UAT-SRCH-02`; screen behaviour is `FIND-04`'s);
      ...
      gaps (`known_gaps` rows including any bill-listing limitation, sub-PRD F1–F4, F8).
- [ ] `[human]` **Founder review of the coverage claim** — if the bill collection is reachable only
      through register links, is the declared status (`ACTIVE` vs `FRESHNESS_LIMITED` vs
      `SOURCE_UNAVAILABLE`) and its customer-visible gap an honest description? PRD §6.1, §7, §44.4,
      §43.4 item 4. Irreducibly human judgment.
- **No further `[fixture]` classes** beyond the twelve DoD items and the separation replay. Declared
  explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/future-tas -q`, fully offline, with `INGF-09`'s
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/future-tas -q`.
...
5. **`tests/test_link_following.py`** — the register→Parliament link-follow stays on an allowlisted
   official host; a link to a non-allowlisted host is refused by the fetcher; a dead link yields
   `UNAVAILABLE` (the jurisdiction-specific risk here, and an SSRF-adjacent path — SEC-002).
6. **`tests/test_commencement.py`** — proclamation-unset → `resolved_date is None`; the register's
   "uncommenced" annotation yields the status event; the `S4` negative control fails.
7. **`tests/test_identity.py`** — determinism across two prints; no collision with a `leg-tas`-shaped
...
Reviewer focus (security- and correctness-sensitive paths): the link-following path first — a
register link is untrusted input, and the fetcher's allowlist must be the thing that stops an
off-allowlist fetch, not adapter code (PRD §37.4, SEC-002). Then the `S1`–`S8` mutations, then confirm
(a) the "uncommenced" annotation produces evidence rather than a duplicate document, (b) no
commencement date is produced without evidence, and (c) `conformance.yaml` defers item 11 and nothing
else.
...
1. **The bill collection is reachable only through register links with no stable listing or feed** —
   the most likely limitation in this jurisdiction → declare
   `change_detection.capability: NONE` (→ **`FRESHNESS_LIMITED`**) or `SOURCE_UNAVAILABLE` for that
   collection, with a `known_gaps` entry marked `customer_visible: true`, and record it in
   `docs/prd/10-sources-future/README.md`. PRD §12.1: *"Sources without reliable delta mechanisms MUST
   show `FRESHNESS_LIMITED` rather than a false guarantee"*; PRD §7 fixes the vocabulary; PRD §44.4
   forbids a silent downgrade. Do **not** substitute an unofficial mirror (PRD §6.1).
...
4. **`leg-tas` already emits bill documents**, contradicting sub-PRD **D1** (open question **F4**) →
