...
status: draft
date: 2026-08-03
blocked_by: [SFUT-01, SLEG-03]
blocks: [GOLD-16]
---

# SFUT-03 — `FUTURE-NSW`

Implements PRD §40.6 (wave 5 — `FUTURE-NSW`), PRD §6.5 (future and proposed law) and PRD §40.8
(adapter Definition of Done) &lt;SRCH-002, ADM-001&gt; — no ADR — the decision is already made in PRD
§40.6; this is build ticket 3 of 10 against it.
...
and `SLEG-03` — `LEG-NSW`, module `06-sources-legislation`, file-scope
`pipelines/adapters/leg-nsw/**` (`docs/prd/06-sources-legislation/tickets/`).
...
This is the row every other state/territory row in §40.6 refers back to: `FUTURE-VIC` … `FUTURE-NT`
all read *"Same controlled future-status events"*, i.e. the list above.
...
requests historical, future or proposed material."* The §40.6 row states the same requirement in its
own words — *"without contaminating current-law answers"* — and `SFUT-01` is where it becomes
mechanical.

**The register anchor.** PRD §40.2's `LEG-NSW` row names NSW legislation —
<https://legislation.nsw.gov.au/> — with families *"In-force/repealed/as-made Acts/instruments,
point-in-time versions, Bills, commencement tables"* and capability *"XML/bulk where permitted;
feeds; versions/events"*. That register is `SLEG-03`'s. PRD §40.6 adds *"NSW Parliament and official
consultations"* as this group's further official entries. **PRD §40.1 requires this ticket to expand
...
and gaps."* The PRD names no parliament or consultation URL; the Builder discovers the exact official
endpoints and records them in `registry.yaml` — an entry name is not an endpoint.

**The ownership cut (sub-PRD D1) — read this before writing `identify()`.** `LEG-NSW` (`SLEG-03`)
owns Acts and instruments, their point-in-time versions and the register's commencement tables.
`FUTURE-NSW` owns the **pre-enactment pipeline**: bill texts and prints, explanatory material,
exposure drafts and draft instruments, official consultation documents, and separately published
proclamation/commencement notices — plus the §40.6 status events on those documents. This adapter
must **not** emit a `document_version` for an Act or a statutory instrument; `SFUT-01`'s separation
validator enforces that mechanically (check `S2`). The `LEG-NSW` row lists "Bills" among the
register's families precisely because the register *hosts* them; hosting is not ownership — plan
§5.7's `SLEG-03` goal is *"NSW point-in-time Acts/instruments and commencement tables"* and plan
§5.11's goal for this ticket is *"NSW bill/draft/proclamation status events"*. If the merged
`leg-nsw` adapter already emits bill documents, stop and follow *Feedback obligation* item 4 —
sub-PRD open question **F4**.

**Commencement by proclamation.** NSW Acts commonly commence on a day appointed by proclamation.
`SFUT-01`'s `CommencementSpec` records the mechanism and `resolve_commencement()` returns `None` for
`BY_PROCLAMATION_UNSET` — rendered as "not yet fixed", never substituted (sub-PRD **D4**; PRD §43.3
makes date critical errors a zero-tolerance gate).
...
subject/title allowlist plus dependency expansion"* — `SLEG-01`'s primitive, reused here to decide
which proposals are tracked and to record why.
...
**Downstream.** This ticket `blocks` `GOLD-16` — full-roster coverage, licence and freshness
reconciliation, whose goal is *"Every mandatory group is ACTIVE or explicitly limited — never
silently omitted"* (plan §5.22, PRD §44.4).

**Carried caveats.** DoD item 11 may be `DEFERRED(GOLD-16)` with a recorded `conformance.yaml`
reason and nothing else may be deferred (`INGF-09` deliverable 4; sub-PRD **F8**).
...
No mandatory source group is pre-selected for omission or reduced implementation; `FUTURE-NSW` must be
...
Implement the `FUTURE-NSW` source adapter under `pipelines/adapters/future-nsw/**`: a
`registry.yaml` row decomposing "NSW legislation register, NSW Parliament and official consultations"
into exact official collections, an `allowlist.yaml`, a licence snapshot and assessment per
authority, and an `adapter.py` exposing `ADAPTER: SourceAdapter` that implements PRD §40.7's eight
boundaries over NSW bills, explanatory material, exposure drafts/consultations and
proclamation/commencement notices — emitting only proposal documents with PRD §6.7 non-current
statuses derived from evidenced events via `_shared/future`, with recorded fixtures sufficient for
all twelve PRD §40.8 DoD items to pass offline through `INGF-09`'s conformance kit and for
`SFUT-01`'s separation suite to prove zero current-law eligibility at three legal dates.

## Non-goals

- **No Acts, instruments, point-in-time versions or commencement tables** — `SLEG-03` (`leg-nsw`),
  sub-PRD **D1**.
...
- **No third-party consultation submissions**, Hansard, committee reports or media releases —
  sub-PRD **D6**; PRD §6.1 (*"Only official public sources are eligible"*), PRD §10.1.
...
- `pipelines/adapters/future-nsw/**` — the whole group directory, in the layout `INGF-07`
...
- Does not touch: `pipelines/adapters/future-{cth,vic,qld,wa,sa,tas,act,nt}/**` — `SFUT-02`,
  `SFUT-04`…`SFUT-10`.
...
wave 1) and `SLEG-03` have landed. The eight sibling adapter tickets may run concurrently: each
writes only its own `pipelines/adapters/future-<juris>/` directory, so the write-sets are disjoint by
directory. The nine share exactly two paths — `_shared/future/**`, which they only read and which is
owned by `SFUT-01` (all nine are `blocked_by` it), and the append-only
`pipelines/adapters/pyproject.toml`.

## Deliverables

1. **`registry.yaml`** (schema and validator: `INGF-07`) with `group_id: FUTURE-NSW`, `wave: 5`, the
   authority list (the NSW legislation-register authority, the NSW Parliament and the official
   consultation publisher, D10 — multiplicity inside one file), and `official_endpoints` decomposing
   PRD §40.6's three named entries into exact collections, each with `kind` and `material_class`.
   `document_coverage.families` covers bills and bill prints, explanatory material, exposure
   drafts/draft instruments, consultation documents and proclamation/commencement notices;
   `financial_years` covers at least PRD §6.6's three (`2024-25`, `2025-26`, `2026-27`) or a
   `known_gaps` entry with `customer_visible: true` says why not. `initial_index_tier: T3` (sub-PRD
   **D8**). `change_detection` per sub-PRD **D9** — the `LEG-NSW` row records *"feeds"* capability, so
   prefer a feed/conditional-request capability over scraping and declare it honestly.
   `evaluation_subset_ref` non-empty (F8). `adapter_status` is `ACTIVE` or one of PRD §7's four
   limited states with a customer-visible gap (PRD §40.1).
...
2. **`allowlist.yaml`** (schema: `INGF-02`) listing the exact official hosts/path prefixes and
   **excluding** any third-party-submission repository (sub-PRD **D6**; enforced by check `S8`).
3. **`licence.yaml` + `licence-snapshots/`** (schema and capture CLI: `INGF-04`) — one
   `LicenceAssessment` per authority across PRD §11.1's nine decision axes; unclear rights default to
   *"metadata, limited quotation and official links"*.
4. **`adapter.py`** exposing module-level `ADAPTER: SourceAdapter` (`INGF-01` deliverable 9),
   implementing PRD §40.7's eight boundaries: `discover` (official listings/feeds filtered to the
   PRD §6.3 employment scope through `_shared/legislation`'s title allowlist), `fetch` (through
   `ctx.fetcher` with conditional-request validators), `identify` (deterministic identity stable
   across bill prints, never colliding with a `leg-nsw` identity), `parse` (through `ctx.parser`,
   exact-offset blocks), `normalise` (`document_type` from `PROPOSAL_DOCUMENT_TYPES`, `legal_status`
   from `derive_future_status()`), `extract_events` (`INTRODUCED`, `PASSED`, `ASSENTED`,
   `PROCLAMATION_MADE`, `COMMENCEMENT`, `DRAFT_PUBLISHED`, `CONSULTATION_OPENED/CLOSED`, `LAPSED`,
   `WITHDRAWN`, `REPEAL_SCHEDULED` — each with an `evidence_ref` into this document's own parsed
   text), `extract_relations` (intra-document only; "amends" travels as `future.amends`, sub-PRD
   **D5**), `validate` (`_shared/legislation` checks plus `FutureSeparationValidator`).
5. **`future.*` metadata** on every emitted record via
...
6. **Discovery fixtures** under `fixtures/discovery/` plus `fixtures/dry-run.json` — DoD item 2.
7. **Identity/deletion rules** — DoD item 3, including `REMOVED` behaviour that retains prior
   versions.
8. **Document fixtures** under `fixtures/documents/` covering every declared media type, free of
   personal data, cookies, `Authorization` headers and credentials — DoD item 4.
9. **Parser/node round-trip** — DoD item 5, via `INGF-06`'s `assert_roundtrip()`.
10. **Three time points** under `fixtures/timepoints/` — DoD item 6 — spanning **(a)** before
    introduction, **(b)** after passage/assent and before commencement, **(c)** after the evidenced
    commencement or proclamation. At all three the emitted records stay non-current (sub-PRD **D2**).
11. **Incremental matrix** — DoD item 7 (no-change / changed / removed / transient failure).
12. **`fixtures/baseline.json`** — DoD item 8, plus tighten-only `anomaly_overrides` (plan §8 Q9).
13. **Freshness schedule** — DoD item 9: a replayed 304 run and a replayed content run write
    different fields (PRD §12.1's last-check/last-ingest separation).
14. **Quarantine cases** under `fixtures/quarantine/` — DoD item 10, one per declared reason code,
    each with a defined operator action.
15. **`conformance.yaml`** — `resource_ceilings`, `anomaly_overrides` and `deferred_items: [11]` with
    a recorded reason (`"Evaluation cases authored in GOLD-16"`) — the only permitted deferral.
16. **Measured resources** — DoD item 12, recorded in the committed `conformance-report.json`.
17. **`tests/test_conformance.py`** — the five-line `ConformanceTestCase` subclass — and
    **`tests/test_separation.py`**, calling
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml` and `licence.yaml` exist and validate;
      `FUTURE-NSW` is in `MANDATORY_SOURCE_GROUPS`; the directory name is `future-nsw`; every licence
...
- [ ] `[fixture]` **DoD 4** — fixtures cover every declared media type and the no-customer-data scan
      is clean.
- [ ] `[fixture]` **DoD 5** — every document fixture parses; `assert_roundtrip()` passes; hierarchy
      has one root, no cycles, contiguous sibling ordinals; every `text_hash` recomputes (PRD §15.3).
...
- [ ] `[fixture]` **DoD 7** — the four incremental scenarios each produce their expected run counts
      and status; the transient case creates **no** content quarantine item.
- [ ] `[fixture]` **DoD 8** — the replayed run reproduces `baseline.json` exactly; `anomaly_overrides`
      pass `INGF-05`'s tighten-only policy.
...
- [ ] `[fixture]` **DoD 11** — `evaluation_subset_ref` is non-empty and well-formed; recorded as
      `DEFERRED` with the `conformance.yaml` reason until `evals/**` exists — the only permitted
      deferral (sub-PRD F8).
- [ ] `[fixture]` **DoD 12** — storage, parse time, index-size estimate and peak RSS are recorded and
      within the declared ceilings (PRD §39.2).
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/future-nsw` exits 0 in strict
...
- [ ] `[fixture]` `assert_future_separation()` over this group's own replayed fixtures at all three
      ...
      `INCLUDE_FUTURE` returns them (PRD §6.5, §6.7, §36.2; the §40.6 row's *"without contaminating
      current-law answers"*).
- [ ] `[machine]` No emitted record carries `legal_status: IN_FORCE`, `REPEALED` or `SUPERSEDED`, and
      no emitted `document_type` is outside `PROPOSAL_DOCUMENT_TYPES` — checks `S1`/`S2` (sub-PRD
      **D1**, **D2**).
...
- [ ] `[machine]` A commencement expressed as a day to be appointed by proclamation produces
      `resolved_date: null` and status `ENACTED_NOT_IN_FORCE`; no fixture path fabricates a date
      (check `S4`; sub-PRD D4; PRD §43.3).
...
- [ ] `[machine]` `future.amends` links are emitted only with exact evidence offsets and a
      deterministic derivation (check `S7`; PRD §40.6, §35.2).
...
- [ ] `[machine]` `assert_no_cross_adapter_import()` passes: this adapter imports `_shared/**` and the
      ingestion framework only, never `pipelines/adapters/leg-nsw` or a sibling group (sub-PRD D7).
- [ ] `[machine]` `python -m <root>.registry validate pipelines/adapters/future-nsw` exits 0 and the
...
- [ ] `[machine]` This adapter imports no HTTP or document-parsing library and no database module —
      `INGF-01`'s architecture scan is green over `pipelines/adapters/future-nsw/**` (PRD §37.4,
      §39.1, §40.7; SEC-002).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**SRCH-002** data-side,
      **ADM-001** registry row); UAT IDs (`UAT-SRCH-02` — this group supplies future material the
      script needs; the screen behaviour is `FIND-04`'s); schema/API/event compatibility (none);
      tenant/PII/security impact (submission material excluded, fixtures scanned, fetches
      allowlisted); source/licence impact (new licence snapshot + assessment per authority);
      cost/memory/latency impact (the DoD-12 measurements); rollback path (remove the group directory;
      the registry composer then reports `MANDATORY_GROUP_MISSING`, the correct loud failure); known
...
- [ ] `[human]` **Founder review of the coverage claim**: are the decomposed collections, date bounds
      and `known_gaps` an honest description of NSW future/proposed employment law? PRD §6.1 forbids
      claiming complete coverage and PRD §44.4 forbids silently calling a category covered; PRD §43.4
      item 4 puts source anomalies in the founder review queue. Irreducibly human judgment.
- **No further `[fixture]` classes** beyond the twelve DoD items and the separation replay. Declared
  explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/future-nsw -q`, fully offline. Recorded responses are
served by `INGF-09`'s `ReplayFetcher` (which also applies this group's `allowlist.yaml`) with
`ReplayClock` and an in-memory `RecordSink` inside `replay_context`. Copy the construction pattern
from `INGF-09`'s reference adapter
(`pipelines/ingestion/src/<root>/conformance/reference/demo-registry/`) — it exists precisely so an
adapter Builder need not read another adapter.

1. `uv sync --frozen && uv run pytest pipelines/adapters/future-nsw -q`.
2. **`tests/test_conformance.py`** `[fixture]` — the inherited twelve `test_dod_NN_*` methods, all
   `PASS` except item 11 `DEFERRED`; then
   `python -m <root>.conformance check pipelines/adapters/future-nsw --report conformance-report.json`
   exits 0 and the report validates against `conformance-report.schema.json`.
3. **`tests/test_separation.py`** — `assert_future_separation()` at the three DoD-6 legal dates, then
   per-check assertions for `S1`–`S8` using locally mutated copies of this group's fixtures in
   `tmp_path` (never mutating the committed fixtures).
...
5. **`tests/test_commencement.py`** — the proclamation-unset fixture: `resolved_date is None`; the
   `S4` negative control (a commencement date without evidence) fails.
6. **`tests/test_identity.py`** — determinism across two prints; no collision with a `leg-nsw`-shaped
   key; `REMOVED` behaviour.
7. **`tests/test_registry.py`** — `python -m <root>.registry validate pipelines/adapters/future-nsw`
   exits 0; `adapter_status` is releasable; a limited status carries a customer-visible gap.
8. **`tests/test_architecture.py`** — the forbidden-import scan and `assert_no_cross_adapter_import()`.
...
`document_type` overlaps `leg-nsw`'s Acts/instruments, (c) the allowlist contains no submission
...
1. **An official entry named in PRD §40.6 has no machine-readable change-detection capability** →
   declare `change_detection.capability: NONE`, which `INGF-07` derives to **`FRESHNESS_LIMITED`**,
   and add a `known_gaps` entry with `customer_visible: true`. Record it in
   `docs/prd/10-sources-future/README.md`. PRD §12.1: *"Sources without reliable delta mechanisms MUST
   show `FRESHNESS_LIMITED` rather than a false guarantee"*; PRD §7 fixes the vocabulary. Never a
   silent downgrade — PRD §44.4: *"It is not permitted to silently call an unimplemented source
   category covered."*
2. **Licensing does not permit storage, indexing or quotation of a collection** → the assessment
   becomes `METADATA_AND_LINK_ONLY` (or `UNCLEAR_RESTRICTED`/`REVIEW_REQUIRED`, PRD §11.1) and the
   group's `adapter_status` becomes **`LICENSING_RESTRICTED`** with a customer-visible gap, recorded
   in `docs/prd/10-sources-future/README.md`. Never substitute a non-official source (PRD §6.1).
3. **A whole collection named in the §40.6 row is unavailable** → `SOURCE_UNAVAILABLE` for that
   endpoint with a customer-visible gap. If the entire group is unavailable, that is a group-level
   `SOURCE_UNAVAILABLE` carrying its full `limitation` block under the confirmed plan §8 **Q10**
   policy — `GOLD-16` consolidates the evidence, `LNCH-05` verifies the disclosure, and Gate 2 is the
   Founder's verification and sign-off, not a scope decision. Do not invent a status value.
4. **`leg-nsw` already emits bill documents**, contradicting sub-PRD **D1** (open question **F4**) →
...
5. **A §40.6 status event has no canonical `event_type` member** (sub-PRD **F3**) → add it under
   `FND-03`'s rules (`packages/contracts`, the `prd-enums` fixture,
   `docs/prd/00-foundation/README.md` D6) and map it in `SFUT-01`'s `future.events`. Never emit a
...
6. **The "amends" link cannot be expressed as `future.amends` metadata** → sub-PRD **F2**: writeback
   ...
   Do not add a cross-source `node_relation` locally.
7. **A DoD item genuinely does not apply to this group** → `NOT_APPLICABLE` **with a recorded reason**
   in `conformance.yaml` (`INGF-09` deliverable 4). Deferral remains permitted for item 11 only. If an
   item is impossible for a whole class of sources, that is a change to PRD §40.8 — escalate under
   PRD §45.5.
8. **Resource ceilings (DoD 12) are exceeded** → raise this group's ceiling in `conformance.yaml`
   with the measurement as justification; if the 52-group aggregate threatens PRD §39.2's 2 GiB
   budget, that is `RLSE-11`'s benchmark decision (plan §8 Q3).
