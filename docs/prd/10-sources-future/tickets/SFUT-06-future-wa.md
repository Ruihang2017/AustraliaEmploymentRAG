...
status: draft
date: 2026-08-03
blocked_by: [SFUT-01, SLEG-06]
blocks: [GOLD-16]
---

# SFUT-06 — `FUTURE-WA`

Implements PRD §40.6 (wave 5 — `FUTURE-WA`), PRD §6.5 (future and proposed law) and PRD §40.8
(adapter Definition of Done) &lt;SRCH-002, ADM-001&gt; — no ADR — the decision is already made in PRD
§40.6; this is build ticket 6 of 10 against it.
...
and `SLEG-06` — `LEG-WA`, module `06-sources-legislation`, file-scope
`pipelines/adapters/leg-wa/**` (`docs/prd/06-sources-legislation/tickets/`).
...
**The register anchor.** PRD §40.2's `LEG-WA` row names Western Australian Legislation —
<https://www.legislation.wa.gov.au/> — with families *"Acts, subsidiary legislation, Bills, gazettes
and historical versions where official"* and capability *"Discovery; stable identity; PDF/history"*.
That register is `SLEG-06`'s. PRD §40.6 adds *"Parliament of Western Australia and official
...
**Jurisdiction-specific note — the Gazette.** The `LEG-WA` row is one of two §40.2 rows that name
*"gazettes"*. Where commencement is effected by a proclamation published in the Government Gazette,
that gazette notice is a **separately published document this group owns** (sub-PRD **D1**) and is the
`evidence_ref` for the `PROCLAMATION_MADE`/`COMMENCEMENT` event. Without the gazette notice the
commencement is unevidenced and `resolve_commencement()` must return `None`
(`BY_PROCLAMATION_UNSET`), not a guess. The row's *"PDF/history"* capability means gazette and bill
material is likely PDF: all parsing goes through `INGF-06`'s resource-limited `ParserHost`
subprocess (PRD §37.4), and DoD item 5's exact-text round-trip must hold on the PDF path
(PRD §15.3).

**The ownership cut (sub-PRD D1) — read this before writing `identify()`.** `LEG-WA` (`SLEG-06`)
owns Acts, subsidiary legislation and their historical versions. `FUTURE-WA` owns the
**pre-enactment pipeline**: bill texts and prints, explanatory material, exposure drafts and draft
instruments, official consultation documents, and separately published gazette
proclamation/commencement notices — plus the §40.6 status events on those documents. This adapter
must **not** emit a `document_version` for an Act or subsidiary legislation; `SFUT-01`'s separation
validator enforces that mechanically (check `S2`). The `LEG-WA` row lists "Bills" because the
register *hosts* them; hosting is not ownership — plan §5.7's `SLEG-06` goal is *"WA Acts/subsidiary
legislation with stable identity"* and plan §5.11's goal for this ticket is *"WA future-status
events"*. If the merged `leg-wa` adapter already emits bill or gazette-proclamation documents, stop
...
subject/title allowlist plus dependency expansion"* — `SLEG-01`'s primitive, reused here. Note PRD
§40.5's `ADJ-WA` row caveat — *"only regimes actually applicable"* — as a reminder that WA's
industrial-relations landscape differs from the other states; track only proposals in the PRD §6.3
scope and record why each is tracked.
...
No mandatory source group is pre-selected for omission or reduced implementation; `FUTURE-WA` must be
...
Implement the `FUTURE-WA` source adapter under `pipelines/adapters/future-wa/**`: a `registry.yaml`
row decomposing "WA Legislation, Parliament of Western Australia and official consultations" into
exact official collections, an `allowlist.yaml`, a licence snapshot and assessment per authority, and
an `adapter.py` exposing `ADAPTER: SourceAdapter` that implements PRD §40.7's eight boundaries over
WA bills, explanatory material, exposure drafts/consultations and gazette proclamation/commencement
notices — emitting only proposal documents with PRD §6.7 non-current statuses derived from evidenced
events via `_shared/future`, with recorded fixtures (including PDF) sufficient for all twelve PRD
...
- **No Acts, subsidiary legislation or historical versions** — `SLEG-06` (`leg-wa`), sub-PRD **D1**.
...
- **No general gazette ingestion** — only gazette notices that are commencement/proclamation evidence
  for a tracked in-scope proposal. A whole-of-gazette crawl is outside PRD §6.3's subject scope and
  would breach the title-allowlist discipline of PRD §40.2.
...
- `pipelines/adapters/future-wa/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: `pipelines/adapters/future-{cth,nsw,vic,qld,sa,tas,act,nt}/**` — the eight sibling
  `SFUT` tickets.
...
**Serial safety.** First decomposition; nothing merged, nothing in flight. `SFUT-01` and `SLEG-06`
...
1. **`registry.yaml`** (schema and validator: `INGF-07`) with `group_id: FUTURE-WA`, `wave: 5`, the
   authority list (the WA legislation-register authority, the Parliament of Western Australia, the
   Gazette publisher and the official consultation publisher, D10 — multiplicity inside one file), and
   ...
   proclamation/commencement notices; `financial_years` covers at least PRD §6.6's three or a
   `known_gaps` entry says why not. `initial_index_tier: T3` (sub-PRD **D8**). `change_detection` per
   sub-PRD **D9**. `evaluation_subset_ref` non-empty (F8). `adapter_status` is `ACTIVE` or a PRD §7
   limited state with a customer-visible gap (PRD §40.1).
...
4. **`adapter.py`** exposing module-level `ADAPTER: SourceAdapter` (`INGF-01` deliverable 9),
   ...
   §6.3 employment scope through `_shared/legislation`'s title allowlist, plus targeted gazette
   lookups for tracked proposals), `fetch` (through `ctx.fetcher` with conditional-request
   validators), `identify` (deterministic identity stable across bill prints, never colliding with a
   `leg-wa` identity), `parse` (through `ctx.parser` — HTML and **PDF**), `normalise` (`document_type`
   from `PROPOSAL_DOCUMENT_TYPES`, `legal_status` from `derive_future_status()`), `extract_events`
   ...
   `evidence_ref` into this document's own parsed text — the gazette notice being the commencement
   evidence), `extract_relations` (intra-document only; "amends" travels as `future.amends`, sub-PRD
...
7. **Identity/deletion rules** — DoD item 3, including `REMOVED` behaviour that retains prior
   versions.
8. **Document fixtures** covering every declared media type — at least one PDF bill print and one PDF
   gazette proclamation notice — free of personal data, cookies, `Authorization` headers and
   credentials — DoD item 4.
...
10. **Three time points** — DoD item 6 — spanning **(a)** before introduction, **(b)** after assent
    and before the gazette proclamation, **(c)** after the evidenced proclamation. At all three the
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml` and `licence.yaml` exist and validate;
      `FUTURE-WA` is in `MANDATORY_SOURCE_GROUPS`; directory name `future-wa`; licence-snapshot
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/future-wa` exits 0 in strict
...
- [ ] `[machine]` **A commencement asserted without a gazette proclamation notice fails**: the
...
- [ ] `[machine]` `assert_no_cross_adapter_import()` passes — `_shared/**` and the ingestion framework
      only, never `pipelines/adapters/leg-wa` or a sibling group (sub-PRD D7).
- [ ] `[machine]` `python -m <root>.registry validate pipelines/adapters/future-wa` exits 0 and
...
- [ ] `[machine]` No HTTP, document-parsing or database import — `INGF-01`'s architecture scan is
      green over `pipelines/adapters/future-wa/**` (PRD §37.4, §39.1, §40.7; SEC-002).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**SRCH-002** data-side,
      **ADM-001** registry row); UAT IDs (`UAT-SRCH-02`; screen behaviour is `FIND-04`'s);
      ...
      including PDF parse cost); rollback path (remove the group directory; the composer then reports
...
- [ ] `[human]` **Founder review of the coverage claim** — are the decomposed collections, date bounds
      and `known_gaps` an honest description of WA future/proposed employment law, given WA's distinct
      industrial-relations coverage (PRD §40.5's *"only regimes actually applicable"*)? PRD §6.1,
      §44.4, §43.4 item 4. Irreducibly human judgment.
- **No further `[fixture]` classes** beyond the twelve DoD items and the separation replay. Declared
  explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/future-wa -q`, fully offline, with `INGF-09`'s
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/future-wa -q`.
...
5. **`tests/test_gazette_commencement.py`** — the gazette-evidenced proclamation resolves a date; the
   same fixture with the notice removed resolves `None` and keeps `ENACTED_NOT_IN_FORCE`; the
   jurisdiction-specific risk here.
6. **`tests/test_pdf_roundtrip.py`** — PDF exact-text round-trip and offset stability.
7. **`tests/test_identity.py`** — determinism across two prints; no collision with a `leg-wa`-shaped
...
confirm (a) no commencement date is produced without a parsed gazette notice, (b) PDF offsets
round-trip exactly, (c) the gazette crawl is targeted rather than whole-of-gazette, (d) the allowlist
contains no submission repository or non-official host, and (e) `conformance.yaml` defers item 11 and
nothing else.
...
4. **`leg-wa` already emits bill or gazette-proclamation documents**, contradicting sub-PRD **D1**
   (open question **F4**) → stop before emitting. Writeback to
   `docs/prd/10-sources-future/README.md` **D1** and `docs/prd/06-sources-legislation/README.md`; a
   module-boundary move is a `docs/prd/breakdown-plan.md` §4/§5 change and an escalation.
...
7. **Gazette notices cannot be discovered per proposal without a whole-of-gazette crawl** → record a
   `known_gaps` entry and declare the commencement evidence limited (`FRESHNESS_LIMITED` for that
   endpoint) rather than widening the crawl beyond PRD §6.3's subject scope. If targeted discovery is
   genuinely impossible, raise it in `docs/prd/10-sources-future/README.md` as a new open question
