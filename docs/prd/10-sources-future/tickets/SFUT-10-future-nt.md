...
status: draft
date: 2026-08-03
blocked_by: [SFUT-01, SLEG-10]
blocks: [GOLD-16]
---

# SFUT-10 — `FUTURE-NT`

Implements PRD §40.6 (wave 5 — `FUTURE-NT`), PRD §6.5 (future and proposed law) and PRD §40.8
(adapter Definition of Done) &lt;SRCH-002, ADM-001&gt; — no ADR — the decision is already made in PRD
§40.6; this is build ticket 10 of 10 against it.
...
and `SLEG-10` — `LEG-NT`, module `06-sources-legislation`, file-scope
`pipelines/adapters/leg-nt/**` (`docs/prd/06-sources-legislation/tickets/`).
...
**The register anchor.** PRD §40.2's `LEG-NT` row names Northern Territory Legislation —
<https://legislation.nt.gov.au/> — with families *"In-force/historical Acts and subordinate law,
Bills, as-made, gazettes"* and capability *"Discovery; API/PDF; versions/events"*. That register is
`SLEG-10`'s. PRD §40.6 adds *"NT Legislative Assembly and official consultations"*. **PRD §40.1
requires this ticket to expand those named entries into exact collections:** *"The live Source
Coverage Registry will expand each group into exact collections/endpoints, licence snapshots,
formats, counts, date bounds, schedules and gaps."* The PRD names no Assembly or consultation URL;
the Builder discovers the exact official endpoints and records them in `registry.yaml` — an entry
name is not an endpoint.

**Jurisdiction-specific notes.**

- **Gazettes.** The `LEG-NT` row is one of two §40.2 rows that name *"gazettes"*. Where commencement
  is effected by a notice published in the Gazette, that notice is a **separately published document
  this group owns** (sub-PRD **D1**) and is the `evidence_ref` for the
  `PROCLAMATION_MADE`/`COMMENCEMENT` event. Without it the commencement is unevidenced and
  `resolve_commencement()` must return `None` (`BY_PROCLAMATION_UNSET`), not a guess.
- **An API is declared.** The row's capability is *"Discovery; API/PDF; versions/events"* — the only
  §40.2 row naming an API. Prefer it for discovery and change detection over scraping, and declare
  the resulting `change_detection.capability` honestly (`API` rather than `UPDATED_LISTING`) so the
  freshness derivation in `INGF-07` is not over-optimistic.
- **A single chamber.** The NT Legislative Assembly is unicameral, so the passage sequence produces
  **one** `PASSED` event. The adapter must not synthesise a second-chamber event;
  `derive_future_status()` depends only on which events are evidenced.

**The ownership cut (sub-PRD D1) — read this before writing `identify()`.** `LEG-NT` (`SLEG-10`)
owns Acts and subordinate law, in-force and historical versions and as-made material. `FUTURE-NT`
owns the **pre-enactment pipeline**: bill texts and prints, explanatory material, exposure drafts and
draft instruments, official consultation documents, and separately published gazette
commencement/proclamation notices — plus the §40.6 status events on those documents. This adapter
must **not** emit a `document_version` for an Act or subordinate law; `SFUT-01`'s separation
validator enforces that mechanically (check `S2`). The `LEG-NT` row lists "Bills" because the
register *hosts* them; hosting is not ownership — plan §5.7's `SLEG-10` goal is *"NT
in-force/historical law with versions and events"* and plan §5.11's goal for this ticket is *"NT
future-status events"*. If the merged `leg-nt` adapter already emits bill or gazette-notice
documents, stop and follow *Feedback obligation* item 4 — sub-PRD open question **F4**.
...
limited — never silently omitted"* (plan §5.22, PRD §44.4). It is the last of the nine wave-5 groups,
so its delivery completes the PRD §26 corpus item *"All five source waves have active or explicitly
limited registry status"* on the ingestion side.
...
No mandatory source group is pre-selected for omission or reduced implementation; `FUTURE-NT` must be
...
Implement the `FUTURE-NT` source adapter under `pipelines/adapters/future-nt/**`: a `registry.yaml`
row decomposing "NT Legislation, NT Legislative Assembly and official consultations" into exact
official collections, an `allowlist.yaml`, a licence snapshot and assessment per authority, and an
`adapter.py` exposing `ADAPTER: SourceAdapter` that implements PRD §40.7's eight boundaries over NT
bills, explanatory material, exposure drafts/consultations and gazette commencement/proclamation
notices — emitting only proposal documents with PRD §6.7 non-current statuses derived from evidenced
events via `_shared/future`, with recorded fixtures sufficient for all twelve PRD §40.8 DoD items to
pass offline through `INGF-09`'s conformance kit and for `SFUT-01`'s separation suite to prove zero
current-law eligibility at three legal dates.

## Non-goals

- **No Acts, subordinate law, in-force/historical versions or as-made register documents** —
  `SLEG-10` (`leg-nt`), sub-PRD **D1**.
...
- **No general gazette ingestion** — only gazette notices that are commencement/proclamation evidence
  for a tracked in-scope proposal, per PRD §6.3's subject scope and PRD §40.2's title-allowlist
  discipline.
...
- `pipelines/adapters/future-nt/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: `pipelines/adapters/future-{cth,nsw,vic,qld,wa,sa,tas,act}/**` — the eight sibling
  `SFUT` tickets.
...
**Serial safety.** First decomposition; nothing merged, nothing in flight. `SFUT-01` and `SLEG-10`
...
1. **`registry.yaml`** (schema and validator: `INGF-07`) with `group_id: FUTURE-NT`, `wave: 5`, the
   authority list (the NT legislation-register authority, the NT Legislative Assembly, the Gazette
   publisher and the official consultation publisher, D10 — multiplicity inside one file), and
   `official_endpoints` decomposing PRD §40.6's three named entries into exact collections with `kind`
   and `material_class`. `document_coverage.families` covers bills and bill prints, explanatory
   material, exposure drafts/draft instruments, consultation documents and gazette
   commencement/proclamation notices; `financial_years` covers at least PRD §6.6's three or a
   `known_gaps` entry says why not. `initial_index_tier: T3` (sub-PRD **D8**). `change_detection` per
   sub-PRD **D9**, declaring `capability: API` where the register's API is genuinely usable.
...
2. **`allowlist.yaml`** (schema: `INGF-02`) — exact official hosts/path prefixes including the Gazette
   host, **excluding** any third-party-submission repository (sub-PRD **D6**; check `S8`).
...
4. **`adapter.py`** exposing module-level `ADAPTER: SourceAdapter` (`INGF-01` deliverable 9),
   implementing PRD §40.7's eight boundaries: `discover` (the register API/listings plus the
   Assembly's bill collection, filtered to the PRD §6.3 employment scope through
   `_shared/legislation`'s title allowlist, plus targeted gazette lookups for tracked proposals),
   `fetch` (through `ctx.fetcher` with conditional-request validators), `identify` (deterministic
   identity stable across bill prints, never colliding with a `leg-nt` identity), `parse` (through
   `ctx.parser` — HTML and **PDF**), `normalise` (`document_type` from `PROPOSAL_DOCUMENT_TYPES`,
   `legal_status` from `derive_future_status()`), `extract_events` (`INTRODUCED`, a single `PASSED`,
   `ASSENTED`, `PROCLAMATION_MADE`, `COMMENCEMENT`, `DRAFT_PUBLISHED`, `CONSULTATION_OPENED/CLOSED`,
   `LAPSED`, `WITHDRAWN`, `REPEAL_SCHEDULED`, each with an `evidence_ref` into this document's own
   parsed text — the gazette notice being the commencement evidence), `extract_relations`
   (intra-document only; "amends" travels as `future.amends`, sub-PRD **D5**), `validate`
   (`_shared/legislation` checks plus `FutureSeparationValidator`).
5. **`future.*` metadata** on every emitted record via `build_future_metadata()`.
6. **Discovery fixtures** + `fixtures/dry-run.json` — DoD item 2, including one recorded API response
   if the API is used.
7. **Identity/deletion rules** — DoD item 3, including `REMOVED` behaviour that retains prior
   versions.
8. **Document fixtures** covering every declared media type — at least one PDF — free of personal
   data, cookies, `Authorization` headers and credentials — DoD item 4.
9. **Parser/node round-trip** — DoD item 5 via `assert_roundtrip()`, exercised on the PDF path.
10. **Three time points** — DoD item 6 — spanning **(a)** before introduction, **(b)** after assent
    and before the gazette commencement notice, **(c)** after the evidenced notice. At all three the
    emitted records stay non-current (sub-PRD **D2**).
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml` and `licence.yaml` exist and validate;
      `FUTURE-NT` is in `MANDATORY_SOURCE_GROUPS`; directory name `future-nt`; licence-snapshot
...
- [ ] `[fixture]` **DoD 3** — `identify()` deterministic across two calls and two prints; distinct
      documents distinct keys; `REMOVED` deletes no prior state.
- [ ] `[fixture]` **DoD 4** — fixtures cover every declared media type, **including PDF**; the
      no-customer-data scan is clean.
- [ ] `[fixture]` **DoD 5** — every fixture parses through `ParserHost`; `assert_roundtrip()` passes
      on the PDF path as well as HTML; one root, no cycles, contiguous sibling ordinals; every
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/future-nt` exits 0 in strict
...
- [ ] `[machine]` **A commencement asserted without a gazette notice fails**: the
      `COMMENCEMENT`/`PROCLAMATION_MADE` event requires an `evidence_ref` into a parsed notice, and
      the unevidenced fixture yields `resolved_date: null` with `BY_PROCLAMATION_UNSET`
...
- [ ] `[machine]` `assert_no_cross_adapter_import()` passes — `_shared/**` and the ingestion framework
      only, never `pipelines/adapters/leg-nt` or a sibling group (sub-PRD D7).
- [ ] `[machine]` `python -m <root>.registry validate pipelines/adapters/future-nt` exits 0 and
...
- [ ] `[machine]` No HTTP, document-parsing or database import — `INGF-01`'s architecture scan is
      green over `pipelines/adapters/future-nt/**` (PRD §37.4, §39.1, §40.7; SEC-002).
...
- [ ] `[human]` PR body states the PRD §45.4 items: requirement IDs (**SRCH-002** data-side,
      **ADM-001** registry row); UAT IDs (`UAT-SRCH-02`; screen behaviour is `FIND-04`'s);
      ...
      authority, including the Gazette publisher); cost/memory/latency impact (DoD-12 measurements);
      rollback path (remove the group directory; the composer then reports `MANDATORY_GROUP_MISSING`,
      the correct loud failure); known gaps (`known_gaps` rows, sub-PRD F1–F4, F8).
- [ ] `[human]` **Founder review of the coverage claim** — are the decomposed collections, date bounds
      and `known_gaps` an honest description of NT future/proposed employment law? This is also the
      ninth and last wave-5 group, so it is the natural point to sanity-check that PRD §26's *"All
      five source waves have active or explicitly limited registry status"* is genuinely reachable
      (PRD §6.1, §44.4, §43.4 item 4). Irreducibly human judgment.
- **No further `[fixture]` classes** beyond the twelve DoD items and the separation replay. Declared
  explicitly.

## Test plan

Harness: `uv run pytest pipelines/adapters/future-nt -q`, fully offline, with `INGF-09`'s
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/future-nt -q`.
...
4. **`tests/test_status.py`** — before introduction → `STATUS_UNCONFIRMED`; after introduction →
   `BILL_NOT_ENACTED`; after assent → `ENACTED_NOT_IN_FORCE`; `IN_FORCE` never appears; a single
   `PASSED` event is sufficient (unicameral).
5. **`tests/test_gazette_commencement.py`** — the gazette-evidenced notice resolves a date; the same
   fixture with the notice removed resolves `None` and keeps `ENACTED_NOT_IN_FORCE`.
6. **`tests/test_discovery_api.py`** — the recorded API response drives discovery and the declared
   `change_detection.capability` matches what is actually used (an over-optimistic declaration would
   make `INGF-07`'s freshness derivation dishonest — PRD §12.1).
7. **`tests/test_identity.py`** — determinism across two prints; no collision with a `leg-nt`-shaped
...
confirm (a) no commencement date is produced without a parsed gazette notice, (b) the declared
change-detection capability matches the mechanism actually used, (c) the gazette crawl is targeted
rather than whole-of-gazette, (d) the allowlist contains no submission repository or non-official
host, and (e) `conformance.yaml` defers item 11 and nothing else.
...
1. **The declared API does not cover bills or consultations, or has no delta mechanism** → declare the
   real `change_detection.capability` (down to `NONE` → **`FRESHNESS_LIMITED`**) plus a `known_gaps`
   entry with `customer_visible: true`, recorded in `docs/prd/10-sources-future/README.md`. PRD §12.1:
   *"Sources without reliable delta mechanisms MUST show `FRESHNESS_LIMITED` rather than a false
   guarantee"*; PRD §7 fixes the vocabulary; PRD §44.4 forbids a silent downgrade. An over-declared
   capability is worse than a limited one, because it produces a false freshness guarantee.
2. **Licensing does not permit storage, indexing or quotation** (the Gazette is a frequent case) →
   `METADATA_AND_LINK_ONLY` (or `UNCLEAR_RESTRICTED`/`REVIEW_REQUIRED`, PRD §11.1) and
   `adapter_status: LICENSING_RESTRICTED` with a customer-visible gap, recorded in the sub-PRD. Never
   substitute a non-official source (PRD §6.1).
...
4. **`leg-nt` already emits bill or gazette-notice documents**, contradicting sub-PRD **D1** (open
   question **F4**) → stop before emitting. Writeback to `docs/prd/10-sources-future/README.md` **D1**
   and `docs/prd/06-sources-legislation/README.md`; a module-boundary move is a
...
7. **Gazette notices cannot be discovered per proposal without a whole-of-gazette crawl** → record a
   `known_gaps` entry and declare the commencement evidence limited rather than widening the crawl
   beyond PRD §6.3's subject scope. If targeted discovery is genuinely impossible, raise it in
   `docs/prd/10-sources-future/README.md` as a new open question before expanding the allowlist.
