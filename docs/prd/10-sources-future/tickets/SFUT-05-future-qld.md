...
status: draft
date: 2026-08-03
blocked_by: [SFUT-01, SLEG-05]
blocks: [GOLD-16]
---

# SFUT-05 — `FUTURE-QLD`

Implements PRD §40.6 (wave 5 — `FUTURE-QLD`), PRD §6.5 (future and proposed law) and PRD §40.8
(adapter Definition of Done) &lt;SRCH-002, ADM-001&gt; — no ADR — the decision is already made in PRD
§40.6; this is build ticket 5 of 10 against it.
...
and `SLEG-05` — `LEG-QLD`, module `06-sources-legislation`, file-scope
`pipelines/adapters/leg-qld/**` (`docs/prd/06-sources-legislation/tickets/`).
...
**The register anchor.** PRD §40.2's `LEG-QLD` row names Queensland Legislation —
<https://www.legislation.qld.gov.au/> — with families *"Bills, Acts, subordinate legislation,
point-in-time reprints, future annotations"* and capability *"HTML/XML/PDF discovery;
versions/events"*. That register is `SLEG-05`'s. PRD §40.6 adds *"Queensland Parliament and official
...
**Jurisdiction-specific note — "future annotations".** The `LEG-QLD` row is the only §40.2 row that
names *"future annotations"*: the register itself annotates amendments that are enacted but not yet
commenced. Those annotations are the strongest available **evidence** for an `ENACTED_NOT_IN_FORCE`
status event, and the boundary matters: the annotation supports a `FUTURE-QLD` status event (with an
`evidence_ref` into the annotation text this adapter parsed), while the annotated Act reprint remains
`LEG-QLD`'s document (sub-PRD **D1**). Do not emit the reprint.

**The ownership cut (sub-PRD D1) — read this before writing `identify()`.** `LEG-QLD` (`SLEG-05`)
owns Acts, subordinate legislation and point-in-time reprints. `FUTURE-QLD` owns the
**pre-enactment pipeline**: bill texts and prints, explanatory notes, exposure drafts and draft
...
`document_version` for an Act, subordinate legislation or a reprint; `SFUT-01`'s separation validator
enforces that mechanically (check `S2`). The `LEG-QLD` row lists "Bills" because the register *hosts*
them; hosting is not ownership — plan §5.7's `SLEG-05` goal is *"Queensland reprints and future
annotations"* and plan §5.11's goal for this ticket is *"Queensland future-status events"*. If the
merged `leg-qld` adapter already emits bill documents, stop and follow *Feedback obligation* item 4 —
sub-PRD open question **F4**.

**Jurisdiction-specific note — a single chamber.** The Queensland Parliament is unicameral, so the
passage sequence is introduction → passage → assent with **one** `PASSED` event, not two. The
adapter must not synthesise a second-chamber event to match another jurisdiction's shape;
`derive_future_status()` (`SFUT-01` deliverable 5) depends only on which events are evidenced, so a
single `PASSED` is correct and complete.

**Commencement.** Queensland Acts commence on assent, on a fixed day, or on a day fixed by
proclamation. `SFUT-01`'s `CommencementSpec` covers all three and `resolve_commencement()` returns
`None` when the day is not yet fixed — rendered as "not yet fixed", never substituted (sub-PRD
**D4**; PRD §43.3 makes date critical errors a zero-tolerance gate).
...
No mandatory source group is pre-selected for omission or reduced implementation; `FUTURE-QLD` must be
...
Implement the `FUTURE-QLD` source adapter under `pipelines/adapters/future-qld/**`: a
`registry.yaml` row decomposing "Queensland Legislation, Queensland Parliament and official
...
§40.7's eight boundaries over Queensland bills, explanatory notes, exposure drafts/consultations,
proclamation/commencement notices and the register's future annotations as status evidence —
emitting only proposal documents with PRD §6.7 non-current statuses derived from evidenced events via
...
- **No Acts, subordinate legislation or point-in-time reprints** — `SLEG-05` (`leg-qld`), sub-PRD
  **D1**.
...
- `pipelines/adapters/future-qld/**` — the whole group directory in `INGF-07` deliverable 1's layout:
...
- Does not touch: `pipelines/adapters/future-{cth,nsw,vic,wa,sa,tas,act,nt}/**` — the eight sibling
  `SFUT` tickets.
...
**Serial safety.** First decomposition; nothing merged, nothing in flight. `SFUT-01` and `SLEG-05`
...
1. **`registry.yaml`** (schema and validator: `INGF-07`) with `group_id: FUTURE-QLD`, `wave: 5`, the
   authority list (the Queensland legislation-register authority, the Queensland Parliament and the
   ...
   covers bills and bill prints, explanatory notes, exposure drafts/draft instruments, consultation
   documents, proclamation/commencement notices and future-annotation listings;
   `financial_years` covers at least PRD §6.6's three or a `known_gaps` entry says why not.
   `initial_index_tier: T3` (sub-PRD **D8**). `change_detection` per sub-PRD **D9**.
...
4. **`adapter.py`** exposing module-level `ADAPTER: SourceAdapter` (`INGF-01` deliverable 9),
   ...
   across bill prints, never colliding with a `leg-qld` identity), `parse` (through `ctx.parser` —
   HTML/XML/PDF per the `LEG-QLD` capability row), `normalise` (`document_type` from
   `PROPOSAL_DOCUMENT_TYPES`, `legal_status` from `derive_future_status()`), `extract_events`
   (`INTRODUCED`, a single `PASSED`, `ASSENTED`, `PROCLAMATION_MADE`, `COMMENCEMENT`,
   `DRAFT_PUBLISHED`, `CONSULTATION_OPENED/CLOSED`, `LAPSED`, `WITHDRAWN`, `REPEAL_SCHEDULED`, each
   with an `evidence_ref` into this document's own parsed text — including future annotations as
...
7. **Identity/deletion rules** — DoD item 3, including `REMOVED` behaviour that retains prior
   versions.
8. **Document fixtures** covering every declared media type (HTML/XML/PDF), free of personal data,
   cookies, `Authorization` headers and credentials — DoD item 4.
...
10. **Three time points** — DoD item 6 — spanning **(a)** before introduction, **(b)** after assent
    and before commencement (the future-annotation state), **(c)** after the evidenced commencement.
    At all three the emitted records stay non-current (sub-PRD **D2**).
...
- [ ] `[fixture]` **DoD 1** — `registry.yaml`, `allowlist.yaml` and `licence.yaml` exist and validate;
      `FUTURE-QLD` is in `MANDATORY_SOURCE_GROUPS`; directory name `future-qld`; licence-snapshot
...
- [ ] `[fixture]` `python -m <root>.conformance check pipelines/adapters/future-qld` exits 0 in strict
...
- [ ] `[machine]` **A future annotation is used as evidence, not as a document**: the annotation
      produces a `COMMENCEMENT`/`ENACTED_NOT_IN_FORCE` status event with an `evidence_ref`, and no
      reprint `document_version` is emitted (jurisdiction note; sub-PRD D1).
...
- [ ] `[machine]` `assert_no_cross_adapter_import()` passes — `_shared/**` and the ingestion framework
      only, never `pipelines/adapters/leg-qld` or a sibling group (sub-PRD D7).
- [ ] `[machine]` `python -m <root>.registry validate pipelines/adapters/future-qld` exits 0 and
...
- [ ] `[machine]` No HTTP, document-parsing or database import — `INGF-01`'s architecture scan is
      green over `pipelines/adapters/future-qld/**` (PRD §37.4, §39.1, §40.7; SEC-002).
...
- [ ] `[human]` **Founder review of the coverage claim** — are the decomposed collections, date bounds
      and `known_gaps` an honest description of Queensland future/proposed employment law? PRD §6.1,
...
Harness: `uv run pytest pipelines/adapters/future-qld -q`, fully offline, with `INGF-09`'s
...
1. `uv sync --frozen && uv run pytest pipelines/adapters/future-qld -q`.
...
5. **`tests/test_future_annotation.py`** — the register's future annotation yields a status event with
   an `evidence_ref` and **no** reprint document; the jurisdiction-specific D1 risk.
6. **`tests/test_commencement.py`** — proclamation-unset → `resolved_date is None`; the `S4` negative
   control fails.
7. **`tests/test_identity.py`** — determinism across two prints; no collision with a `leg-qld`-shaped
...
confirm (a) the future-annotation path emits evidence, not a duplicate reprint, (b) no fixture path
produces a commencement date without evidence, (c) the allowlist contains no submission repository or
non-official host, and (d) `conformance.yaml` defers item 11 and nothing else.
...
4. **`leg-qld` already emits bill documents or treats future annotations as its own documents**,
   contradicting sub-PRD **D1** (open question **F4**) → stop before emitting. Writeback to
