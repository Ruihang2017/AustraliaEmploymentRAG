...
| D11 | **Python src-layout under one import root.** Plan §5.6 writes ticket scopes as `src/<area>/**`; under Python src-layout that path is `pipelines/ingestion/src/<import-root>/<area>/**`. The area names, their one-to-one mapping to tickets and their disjointness are unchanged. The import root is whatever `FND-01`'s committed `pipelines/ingestion/pyproject.toml` declares; if it declares none, `aer_ingestion` (see M4). | PRD §20.1 layout; plan §1.1 (module owns its members' manifests, append-only). | `INGF-01` |
...
| M4 | The Python import root for `pipelines/ingestion`. | `00-foundation` (**`FND-01`** creates every member manifest, plan §1.1) | `INGF-01` reads the committed `pyproject.toml` and follows it; falls back to `aer_ingestion` | Every import path in modules `06`–`10`. Resolved before `INGF-01` starts (`FND-01` is wave 1). |
