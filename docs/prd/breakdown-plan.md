...
| `agent` | `builder` for all 238 tickets. Standing "Why `builder`" line: *"a bounded change inside one module's declared file-scope against a fixed contract — not a new subsystem decision."* |
...
|---|---|---|---|---|---:|---|
| `00-foundation` | `FND` | Monorepo skeleton, pinned toolchains, CI gates, canonical enums/IDs, OpenAPI + event schema roots, framework-free domain rules | E01–E03 | DEV-001; underpins all | 12 | — |
...
238 tickets. For every module below: **`lane` = the module directory name, `agent` = `builder`** for
...
### 5.1 `00-foundation` — lane `00-foundation` · agent `builder` · 12 tickets
...
| FND-12 | Repair the package-purity import scanner | S | `packages/contracts/test/enums/package-purity.test.ts` | FND-04 | §39.1, §45.2, §20.1, §20.3, §45.3, E02 | The purity scan reports only statically knowable specifiers: generated import text is not an undeclared import, and a real undeclared import still fails. |
...
All 523 edges from §5, written as `blocker --> dependents` (one line per blocker, `&`-joined). It is
...
- `00-foundation` absorbed `FND-11` at eleven tickets without changing its shape (3 min waves /
  7 max useful lanes / 7 peak lanes): `FND-11` is blocked only by `FND-01` and joined the existing
  wave 2. **`FND-12` does change it**, and the figures in the table above are re-measured with
  `dag-core.laneProfile`, not carried over. At twelve tickets the module is **4 min waves / 4 max
  useful lanes / 4 peak lanes**, because `FND-12` is `blocked_by FND-04` and therefore extends the
  module's critical path to `FND-01 → FND-03 → FND-04 → FND-12`; a fourth wave is unavoidable. The
  dependency waves at unlimited concurrency are
  `[FND-01] → [FND-02, FND-03, FND-11] → [FND-04 … FND-10] → [FND-12]`, and those same four waves are
  already reached at concurrency **4** — the seven wave-3-eligible tickets simply spread over the last
  two waves, `[FND-01] → [FND-02, FND-03, FND-11] → [FND-04, FND-05, FND-06, FND-07] → [FND-08,
  FND-09, FND-10, FND-12]` — so the honest "max useful lanes" figure falls from 7 to 4. **That is a
  reporting change, not a decomposition defect:** no ticket lost a lane, no write-set became shared,
  and the module is nowhere near fully serial. A repair whose input is `FND-04`'s merged file cannot
  start before `FND-04`; widening the module again would mean either re-basing `FND-12` on `FND-03`
  (false — the file it repairs is `FND-04`'s) or splitting a wave-3 ticket, and nothing here
  justifies either.

Across the whole PRD the flat DAG has a **critical path of 17 waves** with wave widths
`1, 4, 12, 12, 10, 6, 13, 13, 19, 53, 52, 24, 8, 5, 3, 2, 1` — the two 50-wide waves are the 52
...
the 25 sub-PRDs and 238 tickets still resolve; what changed is that every entry now carries an
...
- v0.5 — 2026-08-08 — FND-12 added (238 tickets): repairs the `packages/contracts` package-purity import scanner (`packages/contracts/test/enums/package-purity.test.ts`), whose text regexes match inside a template literal and reported `FND-05`'s **generated import text** — the literal string `${specifier}` emitted by `packages/contracts/src/events/codegen/emit.mjs` — as an undeclared import, so any correct rebase of `FND-05` onto `main` failed with `undeclared import in packages/contracts tooling: … -> ${specifier}`. No manifest entry can ever satisfy `${specifier}`, so repairing the scanner is the only correct fix; the guard's rule (PRD §39.1/§45.2, sub-PRD D22c) is unchanged and explicitly not weakened. §5.1 gains the `FND-12` row, §6.2 gains the `FND-04 --> FND-12` edge (523 edges), and the counts in §1, §1.1, §3, §5, §8 and §9 R3 are refreshed. §7 is **re-measured with `dag-core.laneProfile`, not carried over**: `00-foundation` moves from 11 tickets / 3 min waves / 7 max useful lanes / 7 peak lanes to **12 / 4 / 4 / 4**, because `FND-12` is `blocked_by FND-04` and lengthens the module critical path to `FND-01 → FND-03 → FND-04 → FND-12`; the whole-PRD wave-width vector's wave 4 widens from 11 to 12, with the 17-wave critical path and the concurrency-38 figure unchanged. Second instance of the defect class `FND-11` repaired — a repository-wide guard written from one ticket's viewpoint that misfires on a later ticket.
